...
> ⚠️ Die Quelle wird von Hand gepflegt — das Blatt ist nur so frisch
> wie der letzte Eintrag in `hub.json`, nicht so frisch wie das Repo.

**Stand:** 2026-08-23 · **10 von 34 Schritten abgenommen** · 6 gebaut, Urteil offen · 18 offen

`████████████▓▓▓▓▓▓▓░░░░░░░░░░░░░░░░░░░░░` 29 % abgenommen, 47 % gebaut

**Bei dir liegen:** 10 Punkt(e) — Details auf der Briefing-Seite (https://nakama-briefing.philipld.chatgpt.site)

## Phasen auf einen Blick

| Phase | Fortschritt | abgenommen | gebaut, Urteil offen | offen |
|---|---|---:|---:|---:|
...
- ■ **S0** — Beweis-Runner, Manifest-Vorlage, Basislinie. Heute 18 Prüfbeine. (abgenommen · 20.08.)
...
- ■ **S1** `SONDE-004a` — Wegwerf-Messgerät für Termin A: zwei Nebenwege (Aux) und Latenzausgleich messbar machen. (abgenommen · 20.08.)
- ■ **Termin A** `FL, du` — Nebenwege und Latenzausgleich in FL gemessen: geht, samplegenau, überlebt Speichern/Laden. (abgenommen · 22.08.)
- ■ **S2** `SONDE-001/002` — Identität eingefroren: Kennungen aller drei Plugins festgeschrieben und mit 63 Prüfungen bewacht, damit FL alte Projekte weiter erkennt. (abgenommen · 20.08.)
- ■ **S3** `SONDE-003` — Hostbrücke: ein Patch am Plugin-Rahmenwerk macht sichtbar, was FL liefert (Transport, Latenz, Automationspunkte). 91 Prüfungen. (abgenommen · 21.08.)
- ■ **S3b** `Nachtrag` — Messgerät für Termin B (Host-Probe): zeichnet Zeitsprünge, Render, Automation als JSON auf. 85 Prüfungen. (abgenommen · 21.08.)
- ■ **Termin B** `FL, du + Claude` — Hostzeit und Automation in FL gemessen (12:45–13:27): Kontext in allen 259 298 Blöcken, Sprünge/Schleifen/Render/Automation sauber getrennt gemeldet; FL liefert nur float. Du hast aufgebaut, Claude ist über den FL-MCP gefahren. (abgenommen · 22.08.)
...
- ■ **G0** `Gate` — Erste adversariale Pruefrunde (C++-Review + Codex) ueber P0 — gefahren 22.08., Urteil PASS: beide Bruchauftraege (Gate 1, Gate 5) gescheitert, die P0-Kernflaeche traegt keinen Befund. Manifest docs/beweise/G0.md. Damit ist P0 vollstaendig. (abgenommen · 22.08.)
...
**■ abgenommen** — gebaut, gemessen UND von einem frischen Pruefer bestaetigt (T2/T3-Urteil PASS).
**▣ gebaut, Urteil offen** — es gibt ein Beweismanifest in `docs/beweise/`, aber kein PASS: der Pruefer steht aus oder hat NEEDS_WORK gesagt. Zaehlt nicht als fertig.
**▨ läuft** · **□ offen** — noch kein Manifest.
