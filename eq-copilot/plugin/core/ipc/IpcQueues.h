...
///
/// Warum eine Sequenz JE SLOT: bei "replace-oldest" darf der Erzeuger genau
/// den Slot ueberschreiben, aus dem der Verbraucher gerade kopiert. Der
/// Verbraucher liest deshalb die Sequenz vor UND nach dem Kopieren; hat sie
/// sich geaendert, war der Frame schon nicht mehr gueltig und wird verworfen —
/// ein zerrissener Frame darf nie den Envelope verlassen. Ohne diesen Riegel
/// waere "cap 2, replace-oldest" auf einer Seite eine Datenrace.
...
public:
    ...
    P2Schleuse() { for (auto& s : slots) { s.folge.store (0); s.laenge = 0; } }

    /// Erzeugerseite. `false` heisst nur: der Frame ist groesser als ein Slot —
    /// er wird gezaehlt verworfen, nie halb geschrieben.
    bool veroeffentlichen (const std::uint8_t* daten, std::size_t laenge) noexcept
    {
        ...
        // ungerade Sequenz = "wird beschrieben"
        s.folge.store (2 * p + 1, std::memory_order_release);
        std::memcpy (s.bytes, daten, laenge);
        s.laenge = laenge;
        s.folge.store (2 * p + 2, std::memory_order_release);
    ...
    std::size_t abholen (std::uint8_t* ziel, std::size_t zielGroesse) noexcept
    {
        for (;;)
        {
            ...
            const std::uint64_t vorher = s.folge.load (std::memory_order_acquire);
            if ((vorher & 1u) != 0)
            {
                // Wird gerade beschrieben: dieser Platz gehoert schon dem
            ...
            const std::uint64_t nachher = s.folge.load (std::memory_order_acquire);
            lese = c + 1;
            verbraucht.store (lese, std::memory_order_release);
            if (vorher != nachher)
                continue;            // waehrend des Kopierens ersetzt worden
            return laenge;
        }
    }
