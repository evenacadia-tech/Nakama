...
| Etappe | **Etappe 1 — Spezifikation vor Code, nach Matrix-Nacharbeit 2 (10.09.2026).** Dieses Manifest trägt Gate-Text, Ist-Stand, Verhaltensmatrix, Bauplan und Entscheide. **Kein Produkt-, Test-, Schema-, Fixture- oder Werkzeugcode in diesem Änderungssatz.** Zwei Prüfrunden: **Matrixprüfung 1** NEEDS_WORK mit zwölf Defekten (§7), eingearbeitet als **Matrix-Nacharbeit 1** (§7.1) — zwölf Zeilen korrigiert, drei neue (M-121 bis M-123), sechs Feinheiten neu gefasst, Abweichung 3 zurückgenommen. **Matrixprüfung 2** (§7.3) schloss zehn davon und fand **B-05** und **B-09** weiter offen; die **Matrix-Nacharbeit 2** (§7.4) hat sie geschlossen: vier Zeilen korrigiert (M-15, M-74, M-75, M-123), eine neue (**M-124**), drei Feinheiten neu gefasst. Etappen 2 bis 4 (Verträge, DSP-Kern, Integration) beginnen erst nach der Wiederprüfung und dem Fortsetzungsauftrag des Dirigenten. |
| Phase | **Offen.** Etappe 1 abgeschlossen; Matrixprüfung 1 **NEEDS_WORK** (zwölf Defekte, §7) → Matrix-Nacharbeit 1 (§7.1); Matrixprüfung 2 **NEEDS_WORK** (zehn geschlossen, B-05 und B-09 offen, §7.3) → Matrix-Nacharbeit 2 (§7.4). Befundzahl je Runde: 12 → 2. Die **Wiederprüfung 2** steht aus; vor ihr beginnt kein Bau. |
...
Vorschlag. Fünf Stellen weichen begründet von ihrem Wortlaut ab — eine davon
ist in der Matrix-Nacharbeit 1 zurückgenommen; sie stehen gesammelt in §5.17.
...
| M-44 | Kein Slot ist `free`, ein Programm soll gebaut werden | Der Befehl erhält **`busy_retry`** — er wird nicht gepuffert, nicht verworfen und nicht mit Gewalt durchgesetzt. Der Aufrufer wiederholt. | **BELEGT** · BAULÜCKE | **NEU** **B6**, Fall `busy_retry_wenn_kein_slot_frei`; **NEU** **B7**, derselbe Weg über die Transaktion | Ein fünfter Programmwunsch verdrängt eine aktive Bank; oder er wird still verworfen und die Transaktion meldet Erfolg | §44.2 wörtlich |
...
| ID | Zustand × Ereignis | Zusage samt Reihenfolge und Frist | Belegklasse | Test | Rotbeweis an der Zusage | Quelle |
|---|---|---|---|---|---|---|
| M-74 | Apply, Revert, Neutralisieren, Remove, Undo, Redo oder Preset-Laden | Jede dieser sieben Handlungen ist **eine atomare Transaktion mit genau einer neuen `state_revision`** — nicht null, nicht zwei. Die Reihenfolge ist fest; **die Wiederholungserkennung steht ganz vorn, alle fehlbaren und allozierenden Stufen liegen vor einem einzigen Commit-Punkt**: **Transaktions-ID im Wiederholungsfenster nachschlagen** (Treffer ⇒ gespeichertes Ergebnis, keine weitere Stufe) → validieren → Zonenriegel (bei Bedienänderungen, §5.6.3) → `base_revision` prüfen → Kandidaten-DTO vollständig bauen → Programm bauen (kann `busy_retry` liefern) → **`state_hash` über das Kandidaten-DTO bilden** → **Undo-Eintrag vorbereiten** (Schnappschuss des noch bestätigten Zustands) → **COMMIT-PUNKT** → Revision erhöhen, Committed tauschen, Hash und Undo-Eintrag übernehmen, Host-Dirty melden, `state_report` senden. **Jedes terminale Ergebnis** — angewandt **oder** abgewiesen — wird unter seiner Transaktions-ID im Fenster abgelegt. **MN2 (B-05):** der Nachschlag stand vorher hinter dem Zonenriegel; eine Wiederholung konnte damit „User-Schutz" bekommen statt des Ergebnisses, das M-123 zusagt. **MN1 (B-01):** Hash und Undo lagen einmal hinter dem Tausch — `stateHash` baut ein DTO und einen `MemoryBlock` und kann mit `grund` scheitern (`eq-copilot/plugin/state/NakamaParameter.cpp:163-183`). | **BELEGT** · **ENTSCHIEDEN (R11, B-01- und B-05-Regel)** · BAULÜCKE · **MN1** · **MN2** | **NEU** **B7**, sieben Fälle `<handlung>_erzeugt_genau_eine_revision`, dazu `nachschlag_steht_vor_jedem_zustandsabhaengigen_riegel` und `alle_fehlbaren_stufen_liegen_vor_dem_commit_punkt` | Neutralisieren zählt zwei Revisionen; oder Undo zählt gar keine; oder eine Stufe hinter dem Commit-Punkt kann fehlschlagen; oder eine bekannte Transaktions-ID läuft durch Validierung oder Zonenriegel, statt sofort ihr gespeichertes Ergebnis zu liefern | §44.3; §44.4 „`Neutralisieren` ist selbst eine versionierte Transaktion"; R11; Matrixprüfung 1 B-01, Matrixprüfung 2 B-05 |
| M-75 | Ein Fehler tritt in **irgendeiner** fehlbaren Stufe eines Apply auf | Der bestätigte Zustand bleibt **unverändert**, der `state_hash` bleibt **unverändert**, die Revision steigt **nicht**, und der Undo-Ring bekommt **keinen** Eintrag. Gemessen wird mit **je einem Einspritzpunkt pro fehlbarer Stufe**: Validierung, Zonenriegel, `base_revision`-Prüfung, Kandidaten-DTO, Programmbau (`busy_retry`), **Hashbildung** und **Undo-Vorbereitung** — sieben Punkte, alle vor dem Commit-Punkt. **Die Nachschlagestufe ist bewusst nicht darunter:** sie liest einen vorallokierten Ring fester Größe, allokiert nicht und kann nicht fehlschlagen — ein Treffer ist ein Ergebnis, ein Fehltreffer ist der Normalfall. Ein Einspritzpunkt dort wäre ein Fehler, den es nicht geben kann. **MN2 (B-05):** diese Eigenschaft ist ab hier ausdrücklich zugesagt, weil die Stufe seit MN2 **vor** allen Riegeln steht und ihr Scheitern sonst die ganze Kette umginge. **MN1 (B-01):** Hash und Undo hatten vorher keinen Einspritzpunkt, obwohl beide scheitern können. | **BELEGT** · **ENTSCHIEDEN (B-01- und B-05-Regel)** · BAULÜCKE · **MN1** · **MN2** | **NEU** **B7**, sieben Fälle `fehler_in_<stufe>_laesst_committed_und_hash_unveraendert` sowie `nachschlag_allokiert_nicht_und_schlaegt_nicht_fehl` (Allokationszähler über 10.000 Nachschläge) | Nach einem Fehler in der Hashbildung ist die Revision erhöht und der Committed-Zustand getauscht; oder ein Fehler in der Undo-Vorbereitung hinterlässt einen halben Ringeintrag; oder der Nachschlag allokiert und wird damit selbst zur fehlbaren Stufe vor allen anderen | R11; §44.3; Matrixprüfung 1 B-01, Matrixprüfung 2 B-05 |
| M-76 | 10.000 doppelte, vertauschte und veraltete Transaktionen, **fernsteuerungsfrei** gegen den lokalen Transaktionskern | Sie erzeugen **höchstens eine** gültige Revision und **niemals** einen Mischzustand. Der Kern führt dafür seine **eigene** Konflikt- und Wiederholungserkennung: jede Transaktion trägt eine Transaktions-ID und eine `base_revision`; `base_revision` ungleich der aktuellen `state_revision` ergibt Konflikt und wendet nichts an; eine bereits **beurteilte** Transaktions-ID — angewandt oder abgewiesen — liefert **dasselbe Ergebnis ohne zweite Revision**, und zwar schon in der ersten Stufe, vor jedem zustandsabhängigen Riegel (MN2, B-05). **MN1 (B-05):** das In-Flight-Register des `ControlClient` ist kein lokaler Riegel — es registriert ausgehende P0-Nachrichten, löscht den Eintrag beim terminalen ACK und erwartet die Idempotenz ausdrücklich vom Broker (`eq-copilot/plugin/core/ipc/controlclient/Nachrichten.cpp:157-184`, `:301-304`, `:332-336`). Es gehört dem Fernweg (S29–31) und kann diesen Beweis nicht tragen. | **BELEGT** · **ENTSCHIEDEN (B-05-Regel, §5.11.4)** · BAULÜCKE · **MN1** | **NEU** **B7**, Fälle `zehntausend_transaktionen_erzeugen_höchstens_eine_revision`, `falsche_base_revision_ist_konflikt_ohne_wirkung` und `dieselbe_transaktions_id_erzeugt_keine_zweite_revision` | Zwei Revisionen entstehen; oder ein vertauschtes Paar hinterlässt Werte aus beiden Transaktionen; oder der Beweis läuft über den Fernweg-ACK statt gegen den lokalen Kern und ist damit in P6 nicht fahrbar | §44.5 wörtlich; `CLAUDE.md` „Zustands- und Parameterhoheit bleibt in der Audio führenden Instanz"; Matrixprüfung 1 B-05 |
| M-123 | Eine abgeschlossene Transaktion wird **nach** ihrem Abschluss erneut angeboten | Der Eintrag der Transaktions-ID bleibt nach dem terminalen Ergebnis erhalten, und die Wiederholung wird **als erste Stufe** erkannt (M-74): sie liefert **dasselbe Ergebnis** — dieselbe Revision, denselben Hash — und erzeugt **keine** zweite Revision. Das Fenster ist ein vorallokierter Ring von **32** Einträgen; die 32 sind die Tiefe des Undo-Rings und damit die einzige Grenze, die der Vertrag setzt (`undo_tiefe`, `maximum: 32`), und genau die Spanne, über die ein alter Zustand per Undo noch erreichbar ist. **Jenseits des Fensters** ist die ID unbekannt; die Transaktion läuft die normale Kette, und ihre inzwischen veraltete `base_revision` erzeugt einen **Konflikt** — nicht einen stillen zweiten Erfolg. **MN2 (B-05):** der Pflicht-Rotbeweis ist der Ablauf aus der Matrixprüfung 2: Band bei 1000 Hz, Zone 900–1100 Hz nachträglich eingeschaltet (Band gilt als verletzt) → Transaktion T verschiebt es zulässig auf 1050 Hz → eine weitere Transaktion auf 500 Hz → **T wird wiederholt**. Erwartet ist T's ursprüngliches Ergebnis, **kein „User-Schutz"** und keine zweite Revision. | **ENTSCHIEDEN (B-05-Regel, §5.11.4)** · BAULÜCKE · **MN1** · **MN2** | **NEU** **B7**, Fälle `wiederholung_nach_abschluss_liefert_dasselbe_ergebnis`, `wiederholung_unter_einer_zone_bekommt_nicht_user_schutz` (der Ablauf 1000 → 1050 → 500 → T) und `wiederholung_jenseits_des_fensters_ist_konflikt` | Die Wiederholung erzeugt eine zweite Revision; oder der Eintrag wird beim Commit gelöscht und die Wiederholung läuft als neue Transaktion durch; oder **die Wiederholung läuft in den Zonenriegel und bekommt „User-Schutz" statt T's Ergebnis**; oder jenseits des Fensters wird stillschweigend Erfolg gemeldet | `eq-ipc-v3.schema.json:2321`; §44.5; Matrixprüfung 1 B-05, Matrixprüfung 2 B-05 |
| M-124 | Eine **abgewiesene** Transaktion wird innerhalb des Fensters erneut angeboten | Sie bekommt **dasselbe Urteil wie beim ersten Mal** — denselben Fehlergrund (Konflikt, User-Schutz, `busy_retry`, Bereichsverletzung), dieselbe unveränderte Revision, denselben unveränderten Hash. Im Fenster liegt deshalb **jedes** terminale Ergebnis, nicht nur das erfolgreiche. Der Eintrag ist rein buchhalterisch: er ist **transient** (er lebt im Prozessor und stirbt mit ihm), steht **nicht** im Kind `Dsp`, **nicht** im DTO und **nicht** im `state_hash` — eine Wiederholung kann nur von einem Aufrufer kommen, der die Transaktion noch für offen hält, und der stirbt mit derselben Sitzung. **MN2 (B-05):** ohne diese Zusage bekäme dieselbe ID beim zweiten Mal ein anderes Urteil, sobald sich der Zustand zwischenzeitlich bewegt hat — genau der Fall, den der Zonenablauf in M-123 zeigt. | **ENTSCHIEDEN (B-05-Regel)** · BAULÜCKE · **MN2** | **NEU** **B7**, Fälle `abgewiesene_transaktion_wird_gleich_abgewiesen_wiederholt` (je einmal für Konflikt und User-Schutz) und `wiederholungsfenster_ist_nicht_teil_des_zustands` (Save/Load lässt es unberührt, der Hash ändert sich nicht) | Die Wiederholung einer abgewiesenen Transaktion läuft durch und wird angewandt; oder sie bekommt einen anderen Fehlergrund als das Original; oder das Fenster taucht im gespeicherten Zustand auf und verändert den Hash | B-05-Regel („Das gespeicherte Ergebnis umfasst deshalb auch abgewiesene Transaktionen"); §53.8 „state_hash umfasst ausschließlich das validierte DSP-DTO"; Matrixprüfung 2 B-05 |
...
Gezählt nach der Matrix-Nacharbeit 2 (10.09.2026), aus den Zeilen selbst, nicht abgeschrieben:

| Klasse | Zeilen | Anteil |
|---|---:|---:|
| **BELEGT** (aus verbindlicher Quelle, ganz oder teilweise) | 76 | 61 % |
| **ENTSCHIEDEN** (R1 bis R15 und die Regeln B-01 bis B-12, samt Feinheiten in §5) | 79 | 64 % |
| davon **allein** durch einen Entscheid getragen | 48 | 39 % |
| **BAULÜCKE** | 124 | 100 % |
| **OFFEN** (Produktfrage) | 0 | 0 % |
| davon mit der Marke **MN1** (Nacharbeit 1) | 15 | 12 % |
| davon mit der Marke **MN2** (Nacharbeit 2) | 5 | 4 % |

Vier Zeilen tragen **beide** Marken (M-15, M-74, M-75, M-123); M-124 ist neu in der Nacharbeit 2. Der Verlauf: **120** Zeilen nach Etappe 1 (BELEGT 73, ENTSCHIEDEN 72, nur entschieden 47) → **123** nach der Nacharbeit 1 (76 / 78 / 47) → **124** heute.
...
Fünf Stellen weichen vom Wortlaut einer Regel ab — **eine davon ist in der
Matrix-Nacharbeit 1 zurückgenommen**. Sie stehen jeweils unter ihrer Regel
...
2. **Die feste Reihenfolge innerhalb einer Transaktion — Wiederholungserkennung ganz vorn, alle fehlbaren Stufen vor einem einzigen Commit-Punkt (MN1, B-01; MN2, B-05):** **Transaktions-ID im Wiederholungsfenster nachschlagen** → validieren → Zonenriegel (nur bei Bedienänderungen, §5.6.3) → `base_revision` prüfen (§5.11.4) → Kandidaten-DTO vollständig bauen → Programm bauen (kann `busy_retry` liefern) → **`state_hash` über das Kandidaten-DTO bilden** → **Undo-Eintrag vorbereiten** (Schnappschuss des noch bestätigten Zustands) → **COMMIT-PUNKT** → Revision erhöhen, Committed tauschen, Hash und Undo-Eintrag übernehmen, Host-Dirty melden, `state_report` senden. Hinter dem Commit-Punkt steht nur noch, was **nicht fehlschlagen und nicht allozieren** kann. **Jedes terminale Ergebnis** — angewandt oder abgewiesen — wird zum Schluss unter seiner Transaktions-ID im Fenster abgelegt.
   ...
   **Warum Hash und Undo vor dem Commit-Punkt gehören (MN1, B-01).** `nakama::parameter::stateHash` ruft `dtoKanon`, baut einen `juce::MemoryBlock` und gibt bei nicht kanonisierbarem Inhalt `false` mit `grund` zurück (`eq-copilot/plugin/state/NakamaParameter.cpp:163-183`). Er ist also eine **fehlbare, allozierende** Stufe und kein zugesicherter Nachlauf. Dasselbe gilt für die Undo-Ablage: sie baut einen vollständigen Schnappschuss. Lagen beide hinter dem Tausch, war „Scheitert eine Stufe, wird nichts getauscht" nicht mehr erfüllbar — es hätte eine Revision ohne Hash oder ohne Rückweg geben können. Beide Schnappschüsse sind vor dem Tausch bildbar: der Hash gehört zum **Kandidaten**-DTO, der Undo-Eintrag zum **noch bestätigten** Zustand.
   M-74 trägt dieselbe Stufenliste wörtlich, und M-75 hat **je einen Einspritzpunkt pro fehlbarer Stufe** — sieben, einschließlich Hashbildung und Undo-Vorbereitung, aber ausdrücklich **nicht** den Nachschlag — mit dem Rotbeweis an „Hash unverändert".
...
4. **Idempotenz und Konflikt liegen im lokalen Transaktionskern (MN1, B-05; Reihenfolge MN2, B-05; Dirigent-Entscheide 10.09.2026).** Jede Transaktion trägt eine **Transaktions-ID** und eine **`base_revision`**. Der Kern schlägt die ID **als erste Stufe** nach, vor jedem zustandsabhängigen Riegel: ein Treffer liefert sofort das **gespeicherte Ergebnis** und durchläuft keine weitere Stufe — weder Validierung noch Zonenriegel noch die `base_revision`-Prüfung. Erst eine unbekannte ID geht in die Kette aus §5.11.2. `base_revision` ungleich der aktuellen `state_revision` ⇒ Konflikt, nichts angewandt.
   **Gespeichert wird jedes terminale Urteil, nicht nur der Erfolg (MN2, B-05).** Der Eintrag trägt Transaktions-ID, Ausgang (angewandt oder abgewiesen samt Grund), Revision und Hash. Eine Wiederholung bekommt damit nie ein anderes Urteil als das Original — auch dann nicht, wenn sich der Zustand dazwischen bewegt hat (M-124). Das Fenster ist ein **vorallokierter Ring von 32 Einträgen**; die 32 sind die Tiefe des Undo-Rings und die einzige Grenze, die der Vertrag setzt (`undo_tiefe`, `maximum: 32`) — kein erfundener Deckel. Es ist **transient**: es steht nicht im Kind `Dsp`, nicht im DTO und nicht im `state_hash`, denn eine Wiederholung kann nur von einem Aufrufer kommen, der die Transaktion noch für offen hält, und der stirbt mit derselben Sitzung. Jenseits des Fensters ist die ID unbekannt; die Transaktion läuft die normale Kette, und ihre veraltete `base_revision` erzeugt einen Konflikt (M-123).
...
Fünf Stellen bauen etwas anderes, als der Wortlaut einer Regel sagt; **Nummer 3
ist seit der Matrix-Nacharbeit 1 zurückgenommen** und bleibt als Verlauf stehen,
damit die Wiederprüfung die Kehre sieht. Heute wirksam sind also **vier**. Jede
steht mit ihrer Begründung auch unter ihrer Regel; hier stehen sie zusammen,
damit die Prüfung sie an einer Stelle findet.
