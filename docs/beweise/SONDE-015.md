...
| Etappe | **Etappe 1 — Spezifikation vor Code, nach Matrix-Nacharbeit 1 (10.09.2026).** Dieses Manifest trägt Gate-Text, Ist-Stand, Verhaltensmatrix, Bauplan und Entscheide. **Kein Produkt-, Test-, Schema-, Fixture- oder Werkzeugcode in diesem Änderungssatz.** Die Matrixprüfung 1 endete mit **NEEDS_WORK, zwölf Defekten** (§7); die **Matrix-Nacharbeit 1** hat sie als B-01 bis B-12 eingearbeitet (§7.1): zwölf Matrixzeilen korrigiert, drei neue (M-121 bis M-123), sechs Feinheiten in §5 neu gefasst, Abweichung 3 zurückgenommen, §4.2 und §4.5 berichtigt. Etappen 2 bis 4 (Verträge, DSP-Kern, Integration) beginnen erst nach der Wiederprüfung und dem Fortsetzungsauftrag des Dirigenten. |
| Phase | **Offen.** Etappe 1 abgeschlossen, Matrixprüfung 1 **NEEDS_WORK** (zwölf Defekte, §7), Matrix-Nacharbeit 1 eingearbeitet (§7.1); die **Wiederprüfung** steht aus. Vor ihr beginnt kein Bau. |
...
**Die Matrix-IDs `M-01` bis `M-123` dieses Manifests sind eigen.** Sie sind
...
| M-15 | Zwei Slots sind belegt und eingeschaltet | Die Bänder wirken **als Kaskade in Slotreihenfolge 0 bis 7**, nicht als Parallelsumme. Gemessen wird der **Gesamtfrequenzgang des Audiopfads gegen das PRODUKT der analytischen Einzelantworten** (bei Parallelsumme fällt der Vergleich: zwei +6-dB-Bells ergeben kaskadiert +12 dB, parallel rund +9,5 dB im Scheitel). Die **Reihenfolge** selbst wird an einer **nicht kommutierenden** Kombination belegt — ein dynamisches Band vor einem statischen auf derselben Frequenz, und zwei Bänder mit verschiedenem `channel_mode` (left und mid) —, deren Vertauschung ein messbar anderes Ergebnis liefert. Sie hängt nie davon ab, in welcher Reihenfolge die Slots belegt wurden. **MN1 (B-09):** der vorige Rotbeweis verglich zwei **identische** Bänder bei vertauschter Belegungsreihenfolge — der fällt auch bei einem falsch parallel gebauten Pfad nie. | **ENTSCHIEDEN (R-Feinheit, §5.2)** · BAULÜCKE · **MN1** | **NEU** **B6**, Fälle `kaskade_gegen_produkt_der_einzelantworten`, `dynamisch_vor_statisch_kommutiert_nicht` und `zwei_channel_modes_kommutieren_nicht` | Der Gesamtgang deckt sich mit der Parallelsumme statt mit dem Produkt; oder die nicht kommutierende Kombination liefert in beiden Reihenfolgen dasselbe Ergebnis | §53.8 „acht feste Band-Slots mit stabilen Parameter-IDs"; Matrixprüfung 1 B-09 |
...
| ID | Zustand × Ereignis | Zusage samt Reihenfolge und Frist | Belegklasse | Test | Rotbeweis an der Zusage | Quelle |
|---|---|---|---|---|---|---|
| M-74 | Apply, Revert, Neutralisieren, Remove, Undo, Redo oder Preset-Laden | Jede dieser sieben Handlungen ist **eine atomare Transaktion mit genau einer neuen `state_revision`** — nicht null, nicht zwei. Die Reihenfolge ist fest, und **alle fehlbaren und allozierenden Stufen liegen VOR einem einzigen Commit-Punkt**: validieren → Zonenriegel (bei Bedienänderungen, §5.6.3) → Transaktions-ID und `base_revision` prüfen → Kandidaten-DTO vollständig bauen → Programm bauen (kann `busy_retry` liefern) → **`state_hash` über das Kandidaten-DTO bilden** → **Undo-Eintrag vorbereiten** (Schnappschuss des noch bestätigten Zustands) → **COMMIT-PUNKT** → Revision erhöhen, Committed tauschen, Hash und Undo-Eintrag übernehmen, Host-Dirty melden, Bericht senden. Die Stufen hinter dem Commit-Punkt können nicht fehlschlagen und allozieren nicht. **MN1 (B-01):** vorher lagen Hashbildung und Undo-Ablage **hinter** dem Committed-Tausch — `stateHash` baut ein DTO und einen `MemoryBlock` und kann mit `grund` scheitern (`eq-copilot/plugin/state/NakamaParameter.cpp:163-183`), also war „Scheitert eine Stufe, wird nichts getauscht" dort nicht mehr erfüllbar. | **BELEGT** · **ENTSCHIEDEN (R11, B-01-Regel)** · BAULÜCKE · **MN1** | **NEU** **B7**, sieben Fälle `<handlung>_erzeugt_genau_eine_revision` und ein Fall `alle_fehlbaren_stufen_liegen_vor_dem_commit_punkt` | Neutralisieren zählt zwei Revisionen, weil es intern zweimal schreibt; oder Undo zählt gar keine; oder eine Stufe hinter dem Commit-Punkt kann fehlschlagen und hinterlässt eine Revision ohne Hash | §44.3; §44.4 „`Neutralisieren` ist selbst eine versionierte Transaktion"; R11; Matrixprüfung 1 B-01 |
| M-75 | Ein Fehler tritt in **irgendeiner** fehlbaren Stufe eines Apply auf | Der bestätigte Zustand bleibt **unverändert**, der `state_hash` bleibt **unverändert**, die Revision steigt **nicht**, und der Undo-Ring bekommt **keinen** Eintrag. Gemessen wird mit **je einem Einspritzpunkt pro fehlbarer Stufe**: Validierung, Zonenriegel, Transaktions-/Revisionsriegel, Kandidaten-DTO, Programmbau (`busy_retry`), **Hashbildung** und **Undo-Vorbereitung** — sieben Punkte, alle vor dem Commit-Punkt. **MN1 (B-01):** Hash und Undo hatten vorher keinen Einspritzpunkt, obwohl beide scheitern können. | **BELEGT** · **ENTSCHIEDEN (B-01-Regel)** · BAULÜCKE · **MN1** | **NEU** **B7**, sieben Fälle `fehler_in_<stufe>_lässt_committed_und_hash_unverändert` | Nach einem Fehler in der Hashbildung ist die Revision erhöht und der Committed-Zustand getauscht; oder ein Fehler in der Undo-Vorbereitung hinterlässt einen halben Ringeintrag | R11; §44.3; Matrixprüfung 1 B-01 |
| M-76 | 10.000 doppelte, vertauschte und veraltete Transaktionen, **fernsteuerungsfrei** gegen den lokalen Transaktionskern | Sie erzeugen **höchstens eine** gültige Revision und **niemals** einen Mischzustand. Der Kern führt dafür seine **eigene** Konflikt- und Wiederholungserkennung: jede Transaktion trägt eine Transaktions-ID und eine `base_revision`; `base_revision` ungleich der aktuellen `state_revision` ergibt Konflikt und wendet nichts an; eine bereits angewandte Transaktions-ID liefert **dasselbe Ergebnis ohne zweite Revision**. **MN1 (B-05):** das In-Flight-Register des `ControlClient` ist kein lokaler Riegel — es registriert ausgehende P0-Nachrichten, löscht den Eintrag beim terminalen ACK und erwartet die Idempotenz ausdrücklich vom Broker (`eq-copilot/plugin/core/ipc/controlclient/Nachrichten.cpp:157-184`, `:301-304`, `:332-336`). Es gehört dem Fernweg (S29–31) und kann diesen Beweis nicht tragen. | **BELEGT** · **ENTSCHIEDEN (B-05-Regel, §5.11.4)** · BAULÜCKE · **MN1** | **NEU** **B7**, Fälle `zehntausend_transaktionen_erzeugen_höchstens_eine_revision`, `falsche_base_revision_ist_konflikt_ohne_wirkung` und `dieselbe_transaktions_id_erzeugt_keine_zweite_revision` | Zwei Revisionen entstehen; oder ein vertauschtes Paar hinterlässt Werte aus beiden Transaktionen; oder der Beweis läuft über den Fernweg-ACK statt gegen den lokalen Kern und ist damit in P6 nicht fahrbar | §44.5 wörtlich; `CLAUDE.md` „Zustands- und Parameterhoheit bleibt in der Audio führenden Instanz"; Matrixprüfung 1 B-05 |
| M-123 | Eine abgeschlossene Transaktion wird **nach** ihrem Abschluss erneut angeboten | Der Eintrag der Transaktions-ID bleibt nach dem Commit so lange erhalten, dass die Wiederholung noch als solche erkannt wird: die Wiederholung liefert **dasselbe Ergebnis** — dieselbe Revision, denselben Hash — und erzeugt **keine** zweite Revision. Das Fenster ist die **Tiefe des Undo-Rings, 32**; das ist die einzige Grenze, die der Vertrag überhaupt setzt (`undo_tiefe`, `maximum: 32`), und genau die Spanne, über die ein alter Zustand per Undo noch erreichbar ist. Jenseits davon ist eine wiederholte Transaktions-ID ein Aufruferfehler und wird als **Konflikt** beantwortet, nicht als idempotenter Erfolg. **MN1 (B-05):** kein erfundener Deckel — die 32 kommen aus dem Vertrag, nicht aus einer Schätzung. | **ENTSCHIEDEN (B-05-Regel, §5.11.4)** · BAULÜCKE · **MN1** | **NEU** **B7**, Fälle `wiederholung_nach_abschluss_liefert_dasselbe_ergebnis` und `wiederholung_jenseits_des_fensters_ist_konflikt` | Die Wiederholung erzeugt eine zweite Revision; oder der Eintrag wird beim Commit gelöscht und die Wiederholung läuft als neue Transaktion durch; oder jenseits des Fensters wird stillschweigend Erfolg gemeldet | `eq-ipc-v3.schema.json:2321`; §44.5; Matrixprüfung 1 B-05 |
...
Gezählt nach der Matrix-Nacharbeit 1 (10.09.2026), aus den Zeilen selbst, nicht abgeschrieben:

| Klasse | Zeilen | Anteil |
|---|---:|---:|
| **BELEGT** (aus verbindlicher Quelle, ganz oder teilweise) | 76 | 62 % |
| **ENTSCHIEDEN** (R1 bis R15 und die Regeln B-01 bis B-12, samt Feinheiten in §5) | 78 | 63 % |
| davon **allein** durch einen Entscheid getragen | 47 | 38 % |
| **BAULÜCKE** | 123 | 100 % |
| **OFFEN** (Produktfrage) | 0 | 0 % |
| davon in der Matrix-Nacharbeit 1 **geändert oder neu** (Marke **MN1**) | 15 | 12 % |

Vor der Nacharbeit waren es 120 Zeilen (BELEGT 73, ENTSCHIEDEN 72, nur entschieden 47).
...
2. **Die feste Reihenfolge innerhalb einer Transaktion — alle fehlbaren Stufen vor einem einzigen Commit-Punkt (MN1, B-01):** validieren → Zonenriegel (nur bei Bedienänderungen, §5.6.3) → Transaktions-ID und `base_revision` prüfen (§5.11.4) → Kandidaten-DTO vollständig bauen → Programm bauen (kann `busy_retry` liefern) → **`state_hash` über das Kandidaten-DTO bilden** → **Undo-Eintrag vorbereiten** (Schnappschuss des noch bestätigten Zustands) → **COMMIT-PUNKT** → Revision erhöhen, Committed tauschen, Hash und Undo-Eintrag übernehmen, Host-Dirty melden, `state_report` senden. Hinter dem Commit-Punkt steht nur noch, was **nicht fehlschlagen und nicht allozieren** kann.
   **Warum Hash und Undo davor gehören.** `nakama::parameter::stateHash` ruft `dtoKanon`, baut einen `juce::MemoryBlock` und gibt bei nicht kanonisierbarem Inhalt `false` mit `grund` zurück (`eq-copilot/plugin/state/NakamaParameter.cpp:163-183`). Er ist also eine **fehlbare, allozierende** Stufe und kein zugesicherter Nachlauf. Dasselbe gilt für die Undo-Ablage: sie baut einen vollständigen Schnappschuss. Lagen beide hinter dem Tausch, war „Scheitert eine Stufe, wird nichts getauscht" nicht mehr erfüllbar — es hätte eine Revision ohne Hash oder ohne Rückweg geben können. Beide Schnappschüsse sind vor dem Tausch bildbar: der Hash gehört zum **Kandidaten**-DTO, der Undo-Eintrag zum **noch bestätigten** Zustand.
   M-74 trägt dieselbe Stufenliste wörtlich, und M-75 hat **je einen Einspritzpunkt pro fehlbarer Stufe** — sieben, einschließlich Hashbildung und Undo-Vorbereitung — mit dem Rotbeweis an „Hash unverändert".
...
4. **Idempotenz und Konflikt liegen im lokalen Transaktionskern (MN1, B-05, Dirigent-Entscheid 10.09.2026).** Jede Transaktion trägt eine **Transaktions-ID** und eine **`base_revision`**. `base_revision` ungleich der aktuellen `state_revision` ⇒ Konflikt, nichts angewandt. Eine bereits angewandte Transaktions-ID liefert **dasselbe Ergebnis** — dieselbe Revision, denselben Hash — **ohne** zweite Revision; der Eintrag bleibt nach dem Commit erhalten, solange der Zustand per Undo noch erreichbar ist, also über **32** Revisionen (M-123). Die 32 sind die Tiefe des Undo-Rings und damit die einzige Grenze, die der Vertrag setzt (`undo_tiefe`, `maximum: 32`) — kein erfundener Deckel.
