...
|---|---|---|---|---|
| K-S1 | Broker stirbt **mitten im Frameverkehr** — mindestens ein P2-Liveframe ist unterwegs oder halb geschrieben | Ein halber Frame resynchronisiert nicht, sondern beendet den Strom (bestehende Envelope-Regel). Nach dem Neustart ist der erste Zustand am Main `disconnected`, danach wieder vollständig innerhalb der Frist aus §3.1. Kein halber Frame wird je als Messwert sichtbar. | **Wahrscheinlich (beobachtet).** Frameverkehr ist bei N ≥ 4 Sonden × 10 Hz zwar dicht, aber ein Kill auf fester Wanduhr trifft keinen laufenden Schreibvorgang garantiert. Berichtsfeld `k_s1.offener_handle_fehler`: **getroffen** genau dann, wenn im Killfenster mindestens ein Client einen Fehler auf einem **bereits offenen Handle** meldet — `letzterFehler` gesetzt **und** `brokerPipeFehlt == false` (`core/ipc/ControlClient.h:165-169`). Ein reiner `CreateFileW`-`ERROR_FILE_NOT_FOUND` ist ausdrücklich **kein** Treffer: er heisst „die Pipe war schon weg", nicht „mitten im Transfer". | **NEU** `A24:kill_im_frameverkehr`; bestehende Envelope-Hälfte `broker/tests/transport_fuzz.rs` (ein kaputter Frame beendet den Strom) |
...
kill:           je Killmatrixzeile { urteil: getroffen | nicht_getroffen |
                ...
                k_s1.offener_handle_fehler, k_s2.snapshot_vor_kill,
