...
| Etappe | **Etappe 4a gebaut auf `dc6c148a`; Erstprüfung 4a NEEDS_WORK mit sechs Defekten (§10.8), Nacharbeit VERTAGT hinter den Audit (User-Wort 10.09.2026, Register NAK-246).** Reihenfolge: SONDE-015-Audit-Nacharbeit D8/D10/N-12/N-9 (`docs/beweise/roh/SONDE-015-audit-nacharbeit-auftrag.txt`) → NAK-246 → `/code-review ultra` (Skill §3.7) → Nacharbeit 1 der Etappe 4a → Etappe 4b (Fernweg-Anteile, voller Kanon). Etappe 1 (Matrix) nach sieben Matrixprüfungen, Konvergenzentscheid und NAK-245-Runde (§7); Etappe 2 (Verträge in drei Sprachen) nach Erstprüfung und zwei Nacharbeiten (§8); Etappe 3 (DSP-Kern als Bibliothek, B6 scharf) nach Erstprüfung und drei Nacharbeiten (§9). |
