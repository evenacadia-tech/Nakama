...
$kanon = @(
    ...
    [pscustomobject]@{ Kuerzel='B31'; Name='EqCopSondeLebenslaufTest'; Art='plugin'; Argumente=@(); AbPhase='jetzt'; Behauptung='Sonde-Lebenslaufbein (NAK-312 R-312-7): der Testkonstruktor der Sonde uebernimmt einen Probe-Pipenamen unveraendert und einen Namen ausserhalb des Probe-Namensraums fail-closed als leeren Pipenamen; ohne die Starthaken macht im Testbau kein v3-Client einen Verbindungsversuch, mit ihnen meldet der ControlClient binnen 8 s verbunden ueber die Probe-Pipe eines Testservers, und die Telemetrie koppelt nach dessen welcome; die Sonde wird mit beiden laufenden Clients auf dem Heap abgebaut. Jeder Name, mit dem ein Client startet, liegt vor dem Start im Probe-Namensraum.' }
