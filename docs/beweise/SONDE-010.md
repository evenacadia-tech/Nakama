...
`7457684`, Selbstaudit-Nachlauf `c98bdf9`; Kanon auf BEIDEN committeten
Ständen **GRÜN 32/32** (`docs/beweise/roh/SONDE-010-7457684.md` und
`docs/beweise/roh/SONDE-010-c98bdf9.md`). Referenz für jede Zeile unten ist
...
**Zahlen dieser Runde.** B10 `EqCopIpcTest` 161 → **192 Prüfungen, 0 Fehler**;
...
* **`p0_ueberlauf_trennt_die_verbindung` musste getaktet werden.** Seit
  ...
  Pfad.
* **Laufnummer je Clientlauf (Selbstaudit).** Beim adversarialen Lesen des
  fertigen Diffs fiel auf, dass ein nach `kStopFristMs` ABGELÖSTER Thread bei
  einem erneuten `start()` `laeuft == true` wiedersähe und auf DERSELBEN
  `Laufzeit` weiterliefe — zwei Threads auf einer Pipe. Beide Clients tragen
  deshalb `Laufzeit::lebenslauf`; jeder Lauf endet, sobald die Nummer nicht mehr
  die aktuelle ist, und nur der aktuelle Lauf setzt `fertig`. Probe: B10 G17
  Fall (2b), einmal gebrochen (`roh/SONDE-010-bruchproben-phase2.md`). Die
  Matrix sagt zum Neustart nach dem Ablösen nichts; das ist die kleinste
  tragfähige Festlegung, die `B-CC-12` nicht in ein Datenrennen führt.
...
Rohausgabe: docs\beweise\roh\SONDE-010-c98bdf9.md
```

Derselbe Lauf auf `7457684` (vor dem Selbstaudit-Nachlauf) steht in
`docs/beweise/roh/SONDE-010-7457684.md`; beide sind GRÜN 32/32.
...
| **E** Behauptung ≤ Messung | A21 auf das Gemessene begrenzt, Testname mit (`D-A21-01`); alle Zahlen hier sind gemessen (192, 146/9/9, 32/32, 10 016/10 016) und nicht abgeschrieben; Matrixzeilen werden als Kennung zitiert, nicht als Zeilennummer; der lebende Kopf ist nachgezogen, dieser Abschnitt hängt unten an; jede neue Prüfung wurde einmal absichtlich gebrochen, die roten Rohausgaben liegen in `roh/SONDE-010-bruchproben-phase2.md`. |
...
## Kanon-Lauf - SONDE-010 Ursachenrunde Phase 2 (Nachlauf Laufnummer)

**Lauf:** 2026-08-30 16:19 | **Runner:** `tools/beweise.ps1` | **Urteil:** GRUEN - 32/32 Kanon-Laeufe bestanden | 2 geplante Pruefung(en) noch nicht gebaut | 1 stillgelegte(s) Bein(e), siehe Uebersicht | **Exitcode:** 0 | **Rohausgabe:** [roh/SONDE-010-c98bdf9.md](roh/SONDE-010-c98bdf9.md)

### Kopf - woran gemessen wurde

| Feld | Wert |
|---|---|
| Zeitpunkt | 2026-08-30 16:19:13 +02:00 |
| Rechner | SCHUBBINATOR200 \| Windows 10.0.26200.0 |
| Zweig | master |
| Commit | c98bdf9 SONDE-010 Ursachenrunde: Laufnummer je Client-Lauf - ein abgeloester Thread faehrt nach start() nicht daneben weiter |
| Commit (voll) | c98bdf9c957cb4dd4621a33d369cd315d9c53542 |
| Arbeitsbaum | sauber |
| JUCE gepinnt | 8.0.9 |
| JUCE auf Platte | 8.0.9-dirty |
| FL Studio | FL Studio 2025 25.2.5.5319 \| FL Studio 2026 26.1.4.5589 |
...
| # | Behauptung | Befehl | Ergebnis | Dauer | Rohausgabe |
|---|---|---|---|---|---|
| A1 | Passthrough ist bitgleich; 0 Samples Latenz, 0 Tail; NaN/Inf werden gezaehlt, aber nicht veraendert. | `eq-copilot\build\plugin\EqCopNullTest_artefacts\Release\EqCopNullTest.exe` | [OK] Exit 0 | 0,10 s | [A1](roh/SONDE-010-c98bdf9.md#a1) |
| A2 | AnalyseEngine deckt sich mit der eingefrorenen Offline-Referenz (Fixture-SHA-256 als Determinismus-Riegel). | `eq-copilot\build\plugin\EqCopGoldenTest_artefacts\Release\EqCopGoldenTest.exe eq-copilot\fixtures` | [OK] Exit 0 | 10,18 s | [A2](roh/SONDE-010-c98bdf9.md#a2) |
| A3 | Hoer-Markierung bleibt verriegelt: Render/Freilauf bitgleich, Analyse-Abgriff sitzt vor der Faerbung, und der U10-Term ist gedeckt - OHNE Playhead faerbt bei sonst voller Erlaubnis kein Sample (T11, mit Gegenprobe bei laufendem Transport). | `eq-copilot\build\plugin\EqCopMarkierungTest_artefacts\Release\EqCopMarkierungTest.exe` | [OK] Exit 0 | 6,43 s | [A3](roh/SONDE-010-c98bdf9.md#a3) |
| A4 | Broker-Vertragstests gruen (Framing, Protokoll, Bindung, Aggregat, Server). | `cargo test --manifest-path broker/Cargo.toml --color never` | [OK] Exit 0 | 3,22 s | [A4](roh/SONDE-010-c98bdf9.md#a4) |
| A4b | C++-PipeClient: stop/reconnect und ganze Frames sind zeitlich begrenzt, SQOS ist Identification, Peerbytes sind UTF-8/NUL-sauber, ACK-Sequenzen streng und u64-Zaehler wire-sicher. | `eq-copilot\build\plugin\EqCopPipeClientTest_artefacts\Release\EqCopPipeClientTest.exe` | [OK] Exit 0 | 1,08 s | [A4b](roh/SONDE-010-c98bdf9.md#a4b) |
| A5 | Referenzbein (jsonschema, draft 2020-12): Schema haelt die Engine-Teilmenge ein, Textriegel deckt jede gemessene Kante, jedes Fixture wird wie im Manifest klassifiziert, jede Definition hat ein Negativfixture. | `py -3.13 tools\eq-copilot\pruefe_v3_vertrag.py --abdeckung` | [OK] Exit 0 | 1,74 s | [A5](roh/SONDE-010-c98bdf9.md#a5) |
| A6 | Beide Bandgitter sind bytegleich zur Neuerzeugung; 221 Baender, 64 Gruppen als exakte Partition. | `py -3.13 tools\eq-copilot\erzeuge_bandgitter.py --pruefen` | [OK] Exit 0 | 0,17 s | [A6](roh/SONDE-010-c98bdf9.md#a6) |
| A7 | Quantisierungsvertrag bytegleich zur Neuerzeugung; Rundung, Saettigung und Nichtendliches als Testvektoren. | `py -3.13 tools\eq-copilot\erzeuge_quantisierung.py --pruefen` | [OK] Exit 0 | 0,17 s | [A7](roh/SONDE-010-c98bdf9.md#a7) |
| A8 | Fixture-Korpus und MANIFEST bytegleich zur Neuerzeugung; keine verwaiste Datei. | `py -3.13 tools\eq-copilot\erzeuge_v3_fixtures.py --pruefen` | [OK] Exit 0 | 0,21 s | [A8](roh/SONDE-010-c98bdf9.md#a8) |
| A9 | Codegen-Drift ist 0: die Neugenerierung aus dem .fbs ist bytegleich zum committeten C++- und Rust-Code; flatc, C++-Header und Rust-Crate tragen dieselbe gepinnte Version; jedes Tabellenfeld traegt eine explizite Feld-ID. | `py -3.13 tools\eq-copilot\pruefe_flatc_drift.py` | [OK] Exit 0 | 0,31 s | [A9](roh/SONDE-010-c98bdf9.md#a9) |
| A10 | Binaerer Fixture-Korpus und sein MANIFEST bytegleich zur Neuerzeugung; keine verwaiste Datei. | `py -3.13 tools\eq-copilot\erzeuge_fb_fixtures.py --pruefen` | [OK] Exit 0 | 1,32 s | [A10](roh/SONDE-010-c98bdf9.md#a10) |
| A11 | Die fuenf v2-Vertraege (ipc v2, measurement v1, report v1, snapshot v3, aggregat v1) sind gueltiges JSON und gueltige JSON-Schemas; ihre $id-Familie ist eingefroren. | `py -3.13 tools\eq-copilot\pruefe_v2_schemas.py` | [OK] Exit 0 | 0,34 s | [A11](roh/SONDE-010-c98bdf9.md#a11) |
| A12 | Parameterbestand (109 IDs, §53.8) haelt den Vertrag; RFC-8785-Zahlenvektoren tragen den RFC-Text und werden von rfc8785 bestaetigt; State-Fixture-Korpus und MANIFEST bytegleich zur Neuerzeugung. | `py -3.13 tools\eq-copilot\erzeuge_state_fixtures.py --pruefen` | [OK] Exit 0 | 0,28 s | [A12](roh/SONDE-010-c98bdf9.md#a12) |
| A13 | Capabilityreport FL: die zehn Bits aus §53.6 entsprechen der v3-Vertragsform und stehen so, wie die Rohdaten der Termine A und B sie tragen; jedes supported hat einen Termin, jedes unsupported seinen festen Fallback. | `py -3.13 tools\eq-copilot\pruefe_host_capabilities.py` | [OK] Exit 0 | 0,23 s | [A13](roh/SONDE-010-c98bdf9.md#a13) |
| A14 | Der gemeinsame Kern traegt keine Bundle-Identitaet (Entwurf §53.4): NakamaKern.lib wird vor der Messung geloescht und von MSBuild neu erzeugt (jede Kernquelle uebersetzt, Objekte und Lib juenger als der Zeitanker, Objektzahl = Quellzahl), enthaelt danach keinen eingefrorenen Identitaetswert aus plugin-identities-v1.json (Text in ASCII/UTF-16LE, Viercodes als Integer beider Byteordnungen, CIDs roh und COM-vertauscht) und genau die erwarteten Kernobjekte ohne JUCE-Modulobjekt; die Gegenprobe findet die Pflichtnadeln im gebauten Gen-Bundle, sonst waere das Schweigen ueber den Kern wertlos. Fehlende Voraussetzung (kein Neubau, Artefakt nicht auffindbar, --nur-messen) endet mit Exit 3, mit registriertem Befund mit 2, nie mit 0. Seit NAK-100 (30.08.2026) ohne K1b-Textscan, Ortsriegel, JUCE-Baum-Riegel und Frischewachen (Haertung NAK-103). | `py -3.13 tools\eq-copilot\pruefe_kern_identitaetsfrei.py` | [OK] Exit 0 | 5,87 s | [A14](roh/SONDE-010-c98bdf9.md#a14) |
| A17 | Installer-Manifest: keine zweite Identitaet (kein Viercode, keine Class-ID, kein Produkt- oder Bundlename ausser im Pfad), jeder Quellpfad ist der aus Ziel + Identitaetsdatei nachgerechnete Bundle-Ordner, `art` ist eine geschlossene Menge, jedes aktive Ziel genau einmal und jedes stillgelegte benannt mit lesbarer Marke, Broker aus dem Crate-Namen, Zielverzeichnisse geschuetzt, Signaturzeile ehrlich, Rueckweg samt NAK-41-Riegel und hash_art vollstaendig; jede der 15 Regeln faellt an einem verdorbenen Manifest, die konkreten Umgehungen (Zielkollision, state_schema-Drift, Stilllegung umgangen, Marke unlesbar, ungueltige ziel_id-Typen, Pfadtraversal) fallen einzeln. [4] vergleicht im Kanon weich (Abweichung nach Relink = Hinweis, fehlendes Artefakt oder nicht bildbarer Ordner-Hash = Fehler), hart nur mit --release; [4b] berichtet den installierten Stand ohne Urteil, ok nur bei Journalstatus OK; [5] Ordner-Hash v1 bytegleich in Python und PowerShell, Nicht-ASCII bricht beide ab. Jede gelesene JSON-Datei wird vor dem Zugriff strukturell geprueft, jede unerwartete Ausnahme endet als Klartext mit Exit 2 statt als Traceback. | `py -3.13 tools\eq-copilot\pruefe_installer_manifest.py` | [OK] Exit 0 | 1,01 s | [A17](roh/SONDE-010-c98bdf9.md#a17) |
| A18 | Gegenpfad installieren<->Rueckweg gefahren (Sandbox, nichts installiert): Erstinstallation traegt moduleinfo.json, ein Tausch ersetzt statt zu mischen, -Pruefen sieht den ganzen Ordner, der Rueckweg stellt den Vorzustand bytegleich her und laesst KEIN leeres .vst3-Gehaeuse stehen, selbst angelegte Ordner verschwinden nur leer, der NAK-41-Riegel greift und zerstoert seine eigene Wiederholungsquelle nicht, eine dritte Artefaktsorte bricht ab. | `py -3.13 tools\eq-copilot\pruefe_installer_gegenpfad.py` | [OK] Exit 0 | 41,36 s | [A18](roh/SONDE-010-c98bdf9.md#a18) |
| A19 | BandGridZahlen.h ist bytegleich aus den zwei eingefrorenen Gitterfixturen erzeugt; die 64 Live-Gruppen partitionieren die 221 Feinbaender lueckenlos und ueberschneidungsfrei, und die groben Kanten sind bitgleiche Kopien feiner Kanten (kein zweites Filterbank-Gitter). | `py -3.13 tools\eq-copilot\erzeuge_bandgitter_header.py --pruefen` | [OK] Exit 0 | 0,16 s | [A19](roh/SONDE-010-c98bdf9.md#a19) |
| A20 | Envelope-Fixture-Korpus und MANIFEST bytegleich zur Neuerzeugung; keine verwaiste Datei; jede der 14 Envelope-Regeln hat mindestens ein Negativfixture. | `py -3.13 tools\eq-copilot\erzeuge_envelope_fixtures.py --pruefen` | [OK] Exit 0 | 0,34 s | [A20](roh/SONDE-010-c98bdf9.md#a20) |
| A21 | v3-Envelope unter Zufall: 20 000 Zufallspuffer bringen den Pruefer nie aus dem Tritt und JEDER angenommene Frame erfuellt jede Kopfregel; 3000 gekippte P2-Payloadbits fallen einzeln an der CRC; feindliche Laengen (0, 15, ueber der Grenze, 0xFFFFFFFF) und die u32-Grenze von 16+payload_len enden in der erwarteten Fehlerklasse, ohne Absturz und ohne Eingabeallokation in Groesse der behaupteten Laenge; 300 Runden Fragmentierung (byteweise und in Zufallshaeppchen) liefern exakt dieselben Frames; ein kaputter Frame beendet den Strom statt zu resynchronisieren; die Ratengrenze haelt unter Flut. | `cargo test --manifest-path broker/Cargo.toml --test transport_fuzz --color never` | [OK] Exit 0 | 0,42 s | [A21](roh/SONDE-010-c98bdf9.md#a21) |
| A22 | Ende-zu-Ende ueber die PROBE-Pipe, zwei Sprachen ein Draht: 32 echte C++-Sondenpaare koppeln sich am echten Rust-Listener (32 Control + 32 Telemetry, jede Telemetrieverbindung ueber link_id + challenge + gleiche runtime_nonce), fluten P2 bis die Schleuse mit Cap 2 nachweislich ersetzt, und WAEHRENDDESSEN geht kein einziger P0-Frame verloren; die P0-Antwortlatenz bleibt unter der Schranke. Keine Verbindung wird wegen Envelope, Rate oder P0-Ueberlauf geschlossen. | `py -3.13 tools\eq-copilot\pruefe_ipc_last.py` | [OK] Exit 0 | 10,50 s | [A22](roh/SONDE-010-c98bdf9.md#a22) |
| A15 | STILLGELEGT - mass bis 28.08.2026 Nakama Suna (NkPr): Passthrough bitgleich ueber drei Samplerates und fuenf Blockgroessen, 0 Samples Latenz, kein Tail, kein Hostparameter; Bundlevertrag laesst nur passive_probe zu; speichern-laden-speichern bytegleich. | `(nicht gefahren)` | [STILLGELEGT] seit 2026-08-28 (S9b/SONDE-007c): das Ziel NakamaSuna ist stillgelegt - Suna ist in Nakama Probeeq aufgegangen (design/abnahmen/2026-08-28-suna-stilllegung-vorgezogen.md). Weder gebaut noch gefahren; die Zeile bleibt sichtbar, damit die Kanonzahl nicht still sinkt. | - | - |
| A16 | Nakama Probeeq (NkAc): heute ebenfalls Passthrough bitgleich (die EQ-DSP kommt in P6), 0 Samples Latenz, kein Tail, kein Hostparameter; Bundlevertrag laesst nur active_probe zu; speichern-laden-speichern bytegleich. Seit der T3-Nacharbeit 29.08.2026 (G1 §4.2) faehrt dasselbe Bein die Gate-7-Kette AM GEBAUTEN BUNDLE Nakama Probeeq - Klasse active_probe, denn die Sondenschale traegt genau EINE Klasse je Uebersetzung: ein sonst gueltiger Stand mit measurement_position=post_fader_contribution kommt ueber setStateInformation read-only zurueck, die Instanz bleibt neutral, die Originalbytes gehen unveraendert an den Host zurueck und ein erneutes Laden waescht nichts; auch eine bereits klassifizierte Instanz faellt beim Nachreichen desselben Standes auf neutral zurueck; Gegenprobe mit insert laedt normal. Zusaetzlich misst dasselbe Bein direkt an der oeffentlichen positionErlaubt, dass post_fader_contribution fuer alle vier Klassen abgelehnt wird - das gemeinsame Ergebnis von Capability-Vorpruefung und Klassenmatrix, ohne die beiden Haelften zu trennen. Die vollstaendige 16er-Matrix samt Bundlevertraegen misst B2. | `eq-copilot\build\plugin\EqCopProbeeqNullTest_artefacts\Release\EqCopProbeeqNullTest.exe` | [OK] Exit 0 | 0,10 s | [A16](roh/SONDE-010-c98bdf9.md#a16) |
| B1 | Bundle-Identitaet (CIDs, JUCE_VST3_CAN_REPLACE_VST2=0) eingefroren. | `eq-copilot\build\plugin\EqCopIdentityTest_artefacts\Release\EqCopIdentityTest.exe` | [OK] Exit 0 | 0,12 s | [B1](roh/SONDE-010-c98bdf9.md#b1) |
| B2 | State-Schema 2: Roundtrip bytegleich, Schema-1-Migration rein und golden, unbekanntes Major read-only mit Originalbytes, Duplicate erkennbar (gleiche instance_id, verschiedene runtime_nonce) und aufloesbar, Host-Dirty; Parametertabelle deckungsgleich mit dem Vertrag; RFC-8785-state_hash bytegleich zu Python und Rust. | `eq-copilot\build\plugin\EqCopStateMigrationTest_artefacts\Release\EqCopStateMigrationTest.exe` | [OK] Exit 0 | 0,40 s | [B2](roh/SONDE-010-c98bdf9.md#b2) |
| B3 | Hostkontext (Anwesenheit, Parameterpunkte, Buslatenz) wird gemessen, nicht geraten; Quellhash-Gate des JUCE-Patches gruen. | `eq-copilot\build\plugin\EqCopHostContextTest_artefacts\Release\EqCopHostContextTest.exe` | [OK] Exit 0 | 0,05 s | [B3](roh/SONDE-010-c98bdf9.md#b3) |
| B3b | Termin-B-Messgeraet: Passthrough bitgleich, Sprung-/Automations-/Latenzmessung inkl. Fehlalarm-Riegel, Bericht-Rueckweg, 0 Allokationen. | `eq-copilot\build\plugin\EqCopHostProbeTest_artefacts\Release\EqCopHostProbeTest.exe` | [OK] Exit 0 | 0,08 s | [B3b](roh/SONDE-010-c98bdf9.md#b3b) |
| B3c | v3-Vertrag: C++ klassifiziert den Fixture-Korpus wie das Manifest (Urteil UND Verletzungsmenge), Bandgitter und Quantisierung bitgleich. | `eq-copilot\build\plugin\EqCopSchemaTest_artefacts\Release\EqCopSchemaTest.exe` | [OK] Exit 0 | 0,23 s | [B3c](roh/SONDE-010-c98bdf9.md#b3c) |
| B4 | StampedAudioQueue und Ein-Block-Quarantaene: Ganzblockaufnahme bitgleich ueber jeden Ringumlauf, Ueberlauf BEIDER Ringe verwirft den ganzen Block und nie eine Teilmenge, Oversize ueber der Slotkapazitaet faellt fuer die Analyse und laesst Audio unberuehrt, Flush (numSamples 0) ist kein Verlust, Mono dupliziert L, Mehrfach-Tap-Layout traegt; Quarantaene versiegelt erst mit bewiesener Fortsetzung, Seek und Transportkante verwerfen genau EINEN Block, stehende Projektzeit (FL-Teilpuffer) ist kein Bruch, Projektzeit-Ueberlauf und negative Zeit sind behandelt; Worker-Publikation folgt monotonen 50-/250-ms-Deadlines statt Batchzahl, holt nach Pausen nicht auf und laesst wartende Reset-/Frame-Aufrufer vor; verdrahtet: Passthrough bitgleich ueber 18 Blockgroessen von 1 bis 16384, 0 Samples Latenz, kein Tail, 0 Allokationen im Audiothread ueber 4000 Bloecke wechselnder Groesse mit Transportkanten, und die Engine bekommt den Strom bis auf den Block in Quarantaene. | `eq-copilot\build\plugin\EqCopQueueStressTest_artefacts\Release\EqCopQueueStressTest.exe` | [OK] Exit 0 | 3,37 s | [B4](roh/SONDE-010-c98bdf9.md#b4) |
| B9 | Fixed-memory Loudness (§48.1): der LoudnessAccumulator deckt sich mit der ausgebauten Vektorrechnung innerhalb ±0,1 LU (Entwurf §49) ueber konstante, rampende, zufaellige und einstuendige Korpora sowie ueber Stille unter dem absoluten Gate; Kurz-LUFS ist BITGLEICH; ein adversarialer Sweep legt 1000 Bloecke in den Grenzbin des relativen Gates und die selbstgemeldete Schranke unsicherheitLu() deckt jeden Lauf; eine Million Zellen laufen mit 0 Allokationen durch, waehrend die Gegenprobe (alte Rechnung) allozert; NaN/Inf-Zellen sind gezaehlt statt still als 0 verbucht und l_j == -70,0 exakt bleibt wertgleich; ueber dem Feingitter traegt ein OBERBAND aus Bins von 1 LU bis ueber lautheit(DBL_MAX) - mit ZWEI Pegeln darin (Korpus des T2-Pruefers), einem adversarialen Sweep im Oberband-Grenzbin, beiden Richtungen der Naht zwischen den Aufloesungen und der Gegenprobe, dass kein Block durch das Raster faellt. | `eq-copilot\build\plugin\EqCopLoudnessGoldenTest_artefacts\Release\EqCopLoudnessGoldenTest.exe` | [OK] Exit 0 | 0,15 s | [B9](roh/SONDE-010-c98bdf9.md#b9) |
| B5 | FeatureEngine v2 haelt Zeit-, Validity-, Event- und Bandvertraege: Bandgitter und alle 61 Quantisierungsvektoren bitgleich zum v3-Vertrag, Bitmap LSB-first mit Fuellbits 0, FFT gegen Parseval und einen Sinus auf der Binmitte, K-Gewichtung ueber 20 Hz..20 kHz unter 0,1 dB an der BS.1770-Referenzkette; Drop/Seek(laufend UND gestoppt)/Loop-Wrap/moeglicher Straddle/Transportkante/Sampleratewechsel/Neuanlauf/Beweislagewechsel trennen JEDES offene Fenster - auch den K-Filterzustand, bitgleich gemessen - waehrend FL-Teilstuecke mit stehender Projektzeit lokal weiterlaufen, aber kein unbewiesenes Projektintervall oder FFT-Event erzeugen; Frame-Stempel umfassen den echten ueberlappenden FFT-Support und bleiben an int64/u32-Grenzen darstellbar; Drop zaehlt als Segment, alles andere als Epoche; alle sieben Erzeuger-Stempelregeln fallen einzeln und mit eigener Nummer; kein spektraler Fluss ueberbrueckt eine Grenze, der Ereignisring ist fest gedeckelt; LUFS-S trifft die analytisch gerechnete Erwartung unter 0,1 LU; die Nyquist-Kappe greift bei 22,05 kHz wirklich; verdrahtet: alle sieben Gueltigkeitsbits kommen ueber die Hostbruecke durch, der Playhead-Rueckfallweg nachweislich nur zwei. | `eq-copilot\build\plugin\EqCopAnalysisGoldenTest_artefacts\Release\EqCopAnalysisGoldenTest.exe` | [OK] Exit 0 | 2,38 s | [B5](roh/SONDE-010-c98bdf9.md#b5) |
| B6 | Aktiver DSP-Kern liefert die eingefrorene Referenzantwort. | `eq-copilot\build\plugin\EqCopDspGoldenTest_artefacts\Release\EqCopDspGoldenTest.exe` | [GEPLANT] geplant (ab P6) | - | - |
| B7 | Apply/Revert ist transaktional - kein halber Zustand ueberlebt. | `eq-copilot\build\plugin\EqCopTransactionTest_artefacts\Release\EqCopTransactionTest.exe` | [GEPLANT] geplant (ab P6) | - | - |
| B10 | v3-Envelope in C++ klassifiziert den Envelope-Korpus wie das Manifest (Urteil UND Verstossmenge, alle 14 Regeln mit Negativfixture); CRC32C trifft die RFC-3720-Vektoren, P0/P1 tragen CRC exakt 0, P2 die Pflichtsumme ueber genau die Payloadbytes; 40 000 Zufallspuffer bringen den Pruefer nie aus dem Tritt und 7671 angenommene EINBIT-Mutanten gueltiger Frames halten jede Kopfregel (reiner Zufall wird praktisch immer abgewiesen - die Invariante braucht deshalb die Mutanten, sonst spraeche sie ueber eine leere Menge), 3000 gekippte P2-Bits fallen einzeln, byteweise Zustellung liefert dieselben 40 Frames und ein kaputter Frame beendet den Strom; Pipetoken trifft das Golden aus §48.3 samt SHA-256- und RFC-4648-Vektoren; P0 verwirft nichts und meldet den 65. Eintrag, P1 koalesziert an der Position und haelt Ereignisse fuer den Reconnect vor, die P2-Schleuse ersetzt den aeltesten ungesendeten Frame, uebergibt 100 000 Frames mit 0 Allokationen (mit Gegenprobe am selben Zaehler) und liefert unter Flut keinen zerrissenen Frame; verdrahtet: Control koppelt Telemetry ueber link_id + challenge, ein ungekoppelter Telemetry-Connect wird geschlossen, der Client verbindet nach Serverneustart von selbst wieder, ein kaputter Envelope vom Server schliesst die Verbindung, und ein P0-Ueberlauf WAEHREND einer stehenden Verbindung schliesst sie ebenfalls statt still zu kuerzen. | `eq-copilot\build\plugin\EqCopIpcTest_artefacts\Release\EqCopIpcTest.exe` | [OK] Exit 0 | 38,59 s | [B10](roh/SONDE-010-c98bdf9.md#b10) |
| B8 | Lifecycle-Klassifikation §53.5: unclassified beim Laden und audio-neutral; Schema-1 sensor\|pre\|post -> legacy (immer passiv), hub bzw. bestaetigter Schema-2-Main-State -> main; ein Scannerlauf klassifiziert nicht; read-only nimmt die Klassifikation zurueck; Brokerstart nur fuer main mit offenem Editor; die Sondenbundles bleiben bis gueltigem State neutral und werden nie main. | `eq-copilot\build\plugin\EqCopLebenslaufTest_artefacts\Release\EqCopLebenslaufTest.exe` | [OK] Exit 0 | 0,13 s | [B8](roh/SONDE-010-c98bdf9.md#b8) |


---

## Dirigentenstand — 2026-08-30 (Sitzung 3e24ab41): Ursachenrunde geprüft — NEEDS_WORK, vier Restdefekte als NAK-104 ausgegliedert

**Stand dieses Abschnitts:** `c72d51e` (Basis der Ursachenrunde `05235cf`; Ticketbasis `a7b0740`). **Marke unverändert** `T2 NEEDS_WORK 2026-08-29 nachgearbeitet`.

**Ursachenrunde (§3.4, Wegwechsel):** Phase 1 Verhaltensmatrix `d31ae1c` (Opus/max `nakama-s1415-05235cf-bau`, 7bf24d7d); Matrixprüfung Codex high Thread `01a052bf-3248-7cc3-a18e-28db63762563` — elf Punkte, je mit Regel entschieden; Phase 1b `7deff2e` (9a7e132c); Wiederprüfung der Matrix Thread `01a052cf-fc76-7d83-9966-c571295f84cf` — drei Restpunkte entschieden, in Phase 2 als datierter Nachtrag in `C-LS-06`, `B-TC-07`, `C-LS-07`; Phase 2 Implementierung nach Matrix in C++ und Rust (6cc62362, Aufsicht ENG): `7457684`, `f821f9d`, `c98bdf9`, `c72d51e`. Kanon GRÜN 32/32 auf `7457684` und `c98bdf9`; B10 161 → 192 Prüfungen; Rust 146 + 9 + 9; A22 10 016/10 016; Bruchprobe je Matrixzeile (`roh/SONDE-010-bruchproben-phase2.md`).

**Wiederprüfung (Vorlage B, Fixdiff `05235cf...c72d51e` über die Ticketpfade, Matrix als Referenz):** Codex `gpt-5.6-sol` xhigh, frischer Thread `01a05313-2d88-7ce1-becc-3ca4cd50fc42`, HEAD vor/nach `c72d51e`, 829 s — Kopfzeile wörtlich:

> URTEIL: NEEDS_WORK — Befunde 4, 6 und 8 bleiben teilweise offen; zusätzlich bricht der neue TrennMelder-Pfad `C-LS-06` im Telemetrie-zuerst-Fall. B10 war mit 192/192 grün, die vorhandenen Rust-Transporttests mit 56/56 und Fuzz mit 9/9, diskriminieren diese vier Szenarien jedoch nicht.

**Vom Dirigenten an der Quelle bestätigt, alle vier DEFEKT** (je eine Matrixzeile bricht): `IpcQueues.h` — `for (versuch < kSlots * 2)` ist begrenzt, ein wandernder Verbraucher kann jeden Kandidaten vorher beanspruchen (`A-P2-04` „IMMER einen Platz"); `IpcVerbindung::schreibenGenau` ruft im Zeitlimit `ioAbbrechen()` (Zeile 169), danach liefert `empfangenes (0)` in `ControlClient.cpp` sofort `LeseAusgang::fehler` (`B-CC-07`); die `Laufzeit` samt `verbindung` wird nach dem Ablösen von einem neuen `start()` weiterbenutzt, `lebenslauf` schützt nur die äußere Schleife (`B-CC-12`/`B-TC-07`); `server_v3.rs` wartet nur `if ist_control && hatte_telemetrie` — endet Telemetry zuerst und räumt ihren Eintrag vor `telemetrie_getrennt`, ist `hatte_telemetrie` falsch (`C-LS-06`).

**Entscheid nach §3.4:** Die eine Ursachenrunde ist gefahren; die vier verbleibenden Defekte werden mit der Matrix als **NAK-104** ausgegliedert. Das Ticket bleibt `gebaut`. NAK-104 ist technisch, klein umrissen und ohne Haltgrund — es ist die nächste Arbeit (Basis `c72d51e`, frischer Worker, Matrix + die vier Befunde wörtlich, je Befund ein Test, der das Szenario diskriminiert; danach Wiederprüfung nach Vorlage B über den Fixdiff; PASS ⇒ Marke `T2 PASS` und Abschluss dieses Tickets). Danach steht G1 (Phasengate, Haltgrund §4 — Frage an den User).

**Rundenbilanz** (`tools/dirigent/rundenbilanz.py`, S14–15-eigene Runden): Bau `a7b0740..97c956d` Produkt +5593/−7, Tests +1867/−0, Prüfwerkzeug +671/−0; R1 `97c956d..6fc3224` Produkt +1534/−163, Tests +354/−41; R2 `6fc3224..a0053e4` Produkt +662/−93, Tests +576/−25; R3 `a0053e4..444e125` Produkt +581/−75, Tests +83/−19; Ursachenrunde `05235cf..c72d51e` Produkt +1341/−329, Tests +391/−2, Prüfwerkzeug +1/−1, Doku +6650/−0. Keine Runde ohne Produktfortschritt; Befunde je Prüfrunde 10 → 7 → 6 → 7 → 4.

---

## Kanon-Lauf - SONDE-010 NAK-104

**Lauf:** 2026-08-30 17:33 | **Runner:** `tools/beweise.ps1` | **Urteil:** GRUEN - 32/32 Kanon-Laeufe bestanden | 2 geplante Pruefung(en) noch nicht gebaut | 1 stillgelegte(s) Bein(e), siehe Uebersicht | **Exitcode:** 0 | **Rohausgabe:** [roh/SONDE-010-b4fe522.md](roh/SONDE-010-b4fe522.md)

### Kopf - woran gemessen wurde

| Feld | Wert |
|---|---|
| Zeitpunkt | 2026-08-30 17:33:06 +02:00 |
| Rechner | SCHUBBINATOR200 \| Windows 10.0.26200.0 |
| Zweig | master |
| Commit | b4fe522 SONDE-010 NAK-104: vier Restdefekte der Ursachenrunde behoben |
| Commit (voll) | b4fe522aba41c62f87011ce5b9a5a64291c4589e |
| Arbeitsbaum | sauber |
| JUCE gepinnt | 8.0.9 |
| JUCE auf Platte | 8.0.9-dirty |
| FL Studio | FL Studio 2025 25.2.5.5319 \| FL Studio 2026 26.1.4.5589 |
...
| # | Behauptung | Befehl | Ergebnis | Dauer | Rohausgabe |
|---|---|---|---|---|---|
| A1 | Passthrough ist bitgleich; 0 Samples Latenz, 0 Tail; NaN/Inf werden gezaehlt, aber nicht veraendert. | `eq-copilot\build\plugin\EqCopNullTest_artefacts\Release\EqCopNullTest.exe` | [OK] Exit 0 | 0,11 s | [A1](roh/SONDE-010-b4fe522.md#a1) |
| A2 | AnalyseEngine deckt sich mit der eingefrorenen Offline-Referenz (Fixture-SHA-256 als Determinismus-Riegel). | `eq-copilot\build\plugin\EqCopGoldenTest_artefacts\Release\EqCopGoldenTest.exe eq-copilot\fixtures` | [OK] Exit 0 | 11,37 s | [A2](roh/SONDE-010-b4fe522.md#a2) |
| A3 | Hoer-Markierung bleibt verriegelt: Render/Freilauf bitgleich, Analyse-Abgriff sitzt vor der Faerbung, und der U10-Term ist gedeckt - OHNE Playhead faerbt bei sonst voller Erlaubnis kein Sample (T11, mit Gegenprobe bei laufendem Transport). | `eq-copilot\build\plugin\EqCopMarkierungTest_artefacts\Release\EqCopMarkierungTest.exe` | [OK] Exit 0 | 6,43 s | [A3](roh/SONDE-010-b4fe522.md#a3) |
| A4 | Broker-Vertragstests gruen (Framing, Protokoll, Bindung, Aggregat, Server). | `cargo test --manifest-path broker/Cargo.toml --color never` | [OK] Exit 0 | 3,30 s | [A4](roh/SONDE-010-b4fe522.md#a4) |
| A4b | C++-PipeClient: stop/reconnect und ganze Frames sind zeitlich begrenzt, SQOS ist Identification, Peerbytes sind UTF-8/NUL-sauber, ACK-Sequenzen streng und u64-Zaehler wire-sicher. | `eq-copilot\build\plugin\EqCopPipeClientTest_artefacts\Release\EqCopPipeClientTest.exe` | [OK] Exit 0 | 1,08 s | [A4b](roh/SONDE-010-b4fe522.md#a4b) |
| A5 | Referenzbein (jsonschema, draft 2020-12): Schema haelt die Engine-Teilmenge ein, Textriegel deckt jede gemessene Kante, jedes Fixture wird wie im Manifest klassifiziert, jede Definition hat ein Negativfixture. | `py -3.13 tools\eq-copilot\pruefe_v3_vertrag.py --abdeckung` | [OK] Exit 0 | 1,73 s | [A5](roh/SONDE-010-b4fe522.md#a5) |
| A6 | Beide Bandgitter sind bytegleich zur Neuerzeugung; 221 Baender, 64 Gruppen als exakte Partition. | `py -3.13 tools\eq-copilot\erzeuge_bandgitter.py --pruefen` | [OK] Exit 0 | 0,17 s | [A6](roh/SONDE-010-b4fe522.md#a6) |
| A7 | Quantisierungsvertrag bytegleich zur Neuerzeugung; Rundung, Saettigung und Nichtendliches als Testvektoren. | `py -3.13 tools\eq-copilot\erzeuge_quantisierung.py --pruefen` | [OK] Exit 0 | 0,16 s | [A7](roh/SONDE-010-b4fe522.md#a7) |
| A8 | Fixture-Korpus und MANIFEST bytegleich zur Neuerzeugung; keine verwaiste Datei. | `py -3.13 tools\eq-copilot\erzeuge_v3_fixtures.py --pruefen` | [OK] Exit 0 | 0,21 s | [A8](roh/SONDE-010-b4fe522.md#a8) |
| A9 | Codegen-Drift ist 0: die Neugenerierung aus dem .fbs ist bytegleich zum committeten C++- und Rust-Code; flatc, C++-Header und Rust-Crate tragen dieselbe gepinnte Version; jedes Tabellenfeld traegt eine explizite Feld-ID. | `py -3.13 tools\eq-copilot\pruefe_flatc_drift.py` | [OK] Exit 0 | 0,35 s | [A9](roh/SONDE-010-b4fe522.md#a9) |
| A10 | Binaerer Fixture-Korpus und sein MANIFEST bytegleich zur Neuerzeugung; keine verwaiste Datei. | `py -3.13 tools\eq-copilot\erzeuge_fb_fixtures.py --pruefen` | [OK] Exit 0 | 1,66 s | [A10](roh/SONDE-010-b4fe522.md#a10) |
| A11 | Die fuenf v2-Vertraege (ipc v2, measurement v1, report v1, snapshot v3, aggregat v1) sind gueltiges JSON und gueltige JSON-Schemas; ihre $id-Familie ist eingefroren. | `py -3.13 tools\eq-copilot\pruefe_v2_schemas.py` | [OK] Exit 0 | 0,32 s | [A11](roh/SONDE-010-b4fe522.md#a11) |
| A12 | Parameterbestand (109 IDs, §53.8) haelt den Vertrag; RFC-8785-Zahlenvektoren tragen den RFC-Text und werden von rfc8785 bestaetigt; State-Fixture-Korpus und MANIFEST bytegleich zur Neuerzeugung. | `py -3.13 tools\eq-copilot\erzeuge_state_fixtures.py --pruefen` | [OK] Exit 0 | 0,29 s | [A12](roh/SONDE-010-b4fe522.md#a12) |
| A13 | Capabilityreport FL: die zehn Bits aus §53.6 entsprechen der v3-Vertragsform und stehen so, wie die Rohdaten der Termine A und B sie tragen; jedes supported hat einen Termin, jedes unsupported seinen festen Fallback. | `py -3.13 tools\eq-copilot\pruefe_host_capabilities.py` | [OK] Exit 0 | 0,26 s | [A13](roh/SONDE-010-b4fe522.md#a13) |
| A14 | Der gemeinsame Kern traegt keine Bundle-Identitaet (Entwurf §53.4): NakamaKern.lib wird vor der Messung geloescht und von MSBuild neu erzeugt (jede Kernquelle uebersetzt, Objekte und Lib juenger als der Zeitanker, Objektzahl = Quellzahl), enthaelt danach keinen eingefrorenen Identitaetswert aus plugin-identities-v1.json (Text in ASCII/UTF-16LE, Viercodes als Integer beider Byteordnungen, CIDs roh und COM-vertauscht) und genau die erwarteten Kernobjekte ohne JUCE-Modulobjekt; die Gegenprobe findet die Pflichtnadeln im gebauten Gen-Bundle, sonst waere das Schweigen ueber den Kern wertlos. Fehlende Voraussetzung (kein Neubau, Artefakt nicht auffindbar, --nur-messen) endet mit Exit 3, mit registriertem Befund mit 2, nie mit 0. Seit NAK-100 (30.08.2026) ohne K1b-Textscan, Ortsriegel, JUCE-Baum-Riegel und Frischewachen (Haertung NAK-103). | `py -3.13 tools\eq-copilot\pruefe_kern_identitaetsfrei.py` | [OK] Exit 0 | 6,00 s | [A14](roh/SONDE-010-b4fe522.md#a14) |
| A17 | Installer-Manifest: keine zweite Identitaet (kein Viercode, keine Class-ID, kein Produkt- oder Bundlename ausser im Pfad), jeder Quellpfad ist der aus Ziel + Identitaetsdatei nachgerechnete Bundle-Ordner, `art` ist eine geschlossene Menge, jedes aktive Ziel genau einmal und jedes stillgelegte benannt mit lesbarer Marke, Broker aus dem Crate-Namen, Zielverzeichnisse geschuetzt, Signaturzeile ehrlich, Rueckweg samt NAK-41-Riegel und hash_art vollstaendig; jede der 15 Regeln faellt an einem verdorbenen Manifest, die konkreten Umgehungen (Zielkollision, state_schema-Drift, Stilllegung umgangen, Marke unlesbar, ungueltige ziel_id-Typen, Pfadtraversal) fallen einzeln. [4] vergleicht im Kanon weich (Abweichung nach Relink = Hinweis, fehlendes Artefakt oder nicht bildbarer Ordner-Hash = Fehler), hart nur mit --release; [4b] berichtet den installierten Stand ohne Urteil, ok nur bei Journalstatus OK; [5] Ordner-Hash v1 bytegleich in Python und PowerShell, Nicht-ASCII bricht beide ab. Jede gelesene JSON-Datei wird vor dem Zugriff strukturell geprueft, jede unerwartete Ausnahme endet als Klartext mit Exit 2 statt als Traceback. | `py -3.13 tools\eq-copilot\pruefe_installer_manifest.py` | [OK] Exit 0 | 1,10 s | [A17](roh/SONDE-010-b4fe522.md#a17) |
| A18 | Gegenpfad installieren<->Rueckweg gefahren (Sandbox, nichts installiert): Erstinstallation traegt moduleinfo.json, ein Tausch ersetzt statt zu mischen, -Pruefen sieht den ganzen Ordner, der Rueckweg stellt den Vorzustand bytegleich her und laesst KEIN leeres .vst3-Gehaeuse stehen, selbst angelegte Ordner verschwinden nur leer, der NAK-41-Riegel greift und zerstoert seine eigene Wiederholungsquelle nicht, eine dritte Artefaktsorte bricht ab. | `py -3.13 tools\eq-copilot\pruefe_installer_gegenpfad.py` | [OK] Exit 0 | 44,29 s | [A18](roh/SONDE-010-b4fe522.md#a18) |
| A19 | BandGridZahlen.h ist bytegleich aus den zwei eingefrorenen Gitterfixturen erzeugt; die 64 Live-Gruppen partitionieren die 221 Feinbaender lueckenlos und ueberschneidungsfrei, und die groben Kanten sind bitgleiche Kopien feiner Kanten (kein zweites Filterbank-Gitter). | `py -3.13 tools\eq-copilot\erzeuge_bandgitter_header.py --pruefen` | [OK] Exit 0 | 0,16 s | [A19](roh/SONDE-010-b4fe522.md#a19) |
| A20 | Envelope-Fixture-Korpus und MANIFEST bytegleich zur Neuerzeugung; keine verwaiste Datei; jede der 14 Envelope-Regeln hat mindestens ein Negativfixture. | `py -3.13 tools\eq-copilot\erzeuge_envelope_fixtures.py --pruefen` | [OK] Exit 0 | 0,35 s | [A20](roh/SONDE-010-b4fe522.md#a20) |
| A21 | v3-Envelope unter Zufall: 20 000 Zufallspuffer bringen den Pruefer nie aus dem Tritt und JEDER angenommene Frame erfuellt jede Kopfregel; 3000 gekippte P2-Payloadbits fallen einzeln an der CRC; feindliche Laengen (0, 15, ueber der Grenze, 0xFFFFFFFF) und die u32-Grenze von 16+payload_len enden in der erwarteten Fehlerklasse, ohne Absturz und ohne Eingabeallokation in Groesse der behaupteten Laenge; 300 Runden Fragmentierung (byteweise und in Zufallshaeppchen) liefern exakt dieselben Frames; ein kaputter Frame beendet den Strom statt zu resynchronisieren; die Ratengrenze haelt unter Flut. | `cargo test --manifest-path broker/Cargo.toml --test transport_fuzz --color never` | [OK] Exit 0 | 0,39 s | [A21](roh/SONDE-010-b4fe522.md#a21) |
| A22 | Ende-zu-Ende ueber die PROBE-Pipe, zwei Sprachen ein Draht: 32 echte C++-Sondenpaare koppeln sich am echten Rust-Listener (32 Control + 32 Telemetry, jede Telemetrieverbindung ueber link_id + challenge + gleiche runtime_nonce), fluten P2 bis die Schleuse mit Cap 2 nachweislich ersetzt, und WAEHRENDDESSEN geht kein einziger P0-Frame verloren; die P0-Antwortlatenz bleibt unter der Schranke. Keine Verbindung wird wegen Envelope, Rate oder P0-Ueberlauf geschlossen. | `py -3.13 tools\eq-copilot\pruefe_ipc_last.py` | [OK] Exit 0 | 10,57 s | [A22](roh/SONDE-010-b4fe522.md#a22) |
| A15 | STILLGELEGT - mass bis 28.08.2026 Nakama Suna (NkPr): Passthrough bitgleich ueber drei Samplerates und fuenf Blockgroessen, 0 Samples Latenz, kein Tail, kein Hostparameter; Bundlevertrag laesst nur passive_probe zu; speichern-laden-speichern bytegleich. | `(nicht gefahren)` | [STILLGELEGT] seit 2026-08-28 (S9b/SONDE-007c): das Ziel NakamaSuna ist stillgelegt - Suna ist in Nakama Probeeq aufgegangen (design/abnahmen/2026-08-28-suna-stilllegung-vorgezogen.md). Weder gebaut noch gefahren; die Zeile bleibt sichtbar, damit die Kanonzahl nicht still sinkt. | - | - |
| A16 | Nakama Probeeq (NkAc): heute ebenfalls Passthrough bitgleich (die EQ-DSP kommt in P6), 0 Samples Latenz, kein Tail, kein Hostparameter; Bundlevertrag laesst nur active_probe zu; speichern-laden-speichern bytegleich. Seit der T3-Nacharbeit 29.08.2026 (G1 §4.2) faehrt dasselbe Bein die Gate-7-Kette AM GEBAUTEN BUNDLE Nakama Probeeq - Klasse active_probe, denn die Sondenschale traegt genau EINE Klasse je Uebersetzung: ein sonst gueltiger Stand mit measurement_position=post_fader_contribution kommt ueber setStateInformation read-only zurueck, die Instanz bleibt neutral, die Originalbytes gehen unveraendert an den Host zurueck und ein erneutes Laden waescht nichts; auch eine bereits klassifizierte Instanz faellt beim Nachreichen desselben Standes auf neutral zurueck; Gegenprobe mit insert laedt normal. Zusaetzlich misst dasselbe Bein direkt an der oeffentlichen positionErlaubt, dass post_fader_contribution fuer alle vier Klassen abgelehnt wird - das gemeinsame Ergebnis von Capability-Vorpruefung und Klassenmatrix, ohne die beiden Haelften zu trennen. Die vollstaendige 16er-Matrix samt Bundlevertraegen misst B2. | `eq-copilot\build\plugin\EqCopProbeeqNullTest_artefacts\Release\EqCopProbeeqNullTest.exe` | [OK] Exit 0 | 0,11 s | [A16](roh/SONDE-010-b4fe522.md#a16) |
| B1 | Bundle-Identitaet (CIDs, JUCE_VST3_CAN_REPLACE_VST2=0) eingefroren. | `eq-copilot\build\plugin\EqCopIdentityTest_artefacts\Release\EqCopIdentityTest.exe` | [OK] Exit 0 | 0,12 s | [B1](roh/SONDE-010-b4fe522.md#b1) |
| B2 | State-Schema 2: Roundtrip bytegleich, Schema-1-Migration rein und golden, unbekanntes Major read-only mit Originalbytes, Duplicate erkennbar (gleiche instance_id, verschiedene runtime_nonce) und aufloesbar, Host-Dirty; Parametertabelle deckungsgleich mit dem Vertrag; RFC-8785-state_hash bytegleich zu Python und Rust. | `eq-copilot\build\plugin\EqCopStateMigrationTest_artefacts\Release\EqCopStateMigrationTest.exe` | [OK] Exit 0 | 0,46 s | [B2](roh/SONDE-010-b4fe522.md#b2) |
| B3 | Hostkontext (Anwesenheit, Parameterpunkte, Buslatenz) wird gemessen, nicht geraten; Quellhash-Gate des JUCE-Patches gruen. | `eq-copilot\build\plugin\EqCopHostContextTest_artefacts\Release\EqCopHostContextTest.exe` | [OK] Exit 0 | 0,06 s | [B3](roh/SONDE-010-b4fe522.md#b3) |
| B3b | Termin-B-Messgeraet: Passthrough bitgleich, Sprung-/Automations-/Latenzmessung inkl. Fehlalarm-Riegel, Bericht-Rueckweg, 0 Allokationen. | `eq-copilot\build\plugin\EqCopHostProbeTest_artefacts\Release\EqCopHostProbeTest.exe` | [OK] Exit 0 | 0,09 s | [B3b](roh/SONDE-010-b4fe522.md#b3b) |
| B3c | v3-Vertrag: C++ klassifiziert den Fixture-Korpus wie das Manifest (Urteil UND Verletzungsmenge), Bandgitter und Quantisierung bitgleich. | `eq-copilot\build\plugin\EqCopSchemaTest_artefacts\Release\EqCopSchemaTest.exe` | [OK] Exit 0 | 0,27 s | [B3c](roh/SONDE-010-b4fe522.md#b3c) |
| B4 | StampedAudioQueue und Ein-Block-Quarantaene: Ganzblockaufnahme bitgleich ueber jeden Ringumlauf, Ueberlauf BEIDER Ringe verwirft den ganzen Block und nie eine Teilmenge, Oversize ueber der Slotkapazitaet faellt fuer die Analyse und laesst Audio unberuehrt, Flush (numSamples 0) ist kein Verlust, Mono dupliziert L, Mehrfach-Tap-Layout traegt; Quarantaene versiegelt erst mit bewiesener Fortsetzung, Seek und Transportkante verwerfen genau EINEN Block, stehende Projektzeit (FL-Teilpuffer) ist kein Bruch, Projektzeit-Ueberlauf und negative Zeit sind behandelt; Worker-Publikation folgt monotonen 50-/250-ms-Deadlines statt Batchzahl, holt nach Pausen nicht auf und laesst wartende Reset-/Frame-Aufrufer vor; verdrahtet: Passthrough bitgleich ueber 18 Blockgroessen von 1 bis 16384, 0 Samples Latenz, kein Tail, 0 Allokationen im Audiothread ueber 4000 Bloecke wechselnder Groesse mit Transportkanten, und die Engine bekommt den Strom bis auf den Block in Quarantaene. | `eq-copilot\build\plugin\EqCopQueueStressTest_artefacts\Release\EqCopQueueStressTest.exe` | [OK] Exit 0 | 3,38 s | [B4](roh/SONDE-010-b4fe522.md#b4) |
| B9 | Fixed-memory Loudness (§48.1): der LoudnessAccumulator deckt sich mit der ausgebauten Vektorrechnung innerhalb ±0,1 LU (Entwurf §49) ueber konstante, rampende, zufaellige und einstuendige Korpora sowie ueber Stille unter dem absoluten Gate; Kurz-LUFS ist BITGLEICH; ein adversarialer Sweep legt 1000 Bloecke in den Grenzbin des relativen Gates und die selbstgemeldete Schranke unsicherheitLu() deckt jeden Lauf; eine Million Zellen laufen mit 0 Allokationen durch, waehrend die Gegenprobe (alte Rechnung) allozert; NaN/Inf-Zellen sind gezaehlt statt still als 0 verbucht und l_j == -70,0 exakt bleibt wertgleich; ueber dem Feingitter traegt ein OBERBAND aus Bins von 1 LU bis ueber lautheit(DBL_MAX) - mit ZWEI Pegeln darin (Korpus des T2-Pruefers), einem adversarialen Sweep im Oberband-Grenzbin, beiden Richtungen der Naht zwischen den Aufloesungen und der Gegenprobe, dass kein Block durch das Raster faellt. | `eq-copilot\build\plugin\EqCopLoudnessGoldenTest_artefacts\Release\EqCopLoudnessGoldenTest.exe` | [OK] Exit 0 | 0,16 s | [B9](roh/SONDE-010-b4fe522.md#b9) |
| B5 | FeatureEngine v2 haelt Zeit-, Validity-, Event- und Bandvertraege: Bandgitter und alle 61 Quantisierungsvektoren bitgleich zum v3-Vertrag, Bitmap LSB-first mit Fuellbits 0, FFT gegen Parseval und einen Sinus auf der Binmitte, K-Gewichtung ueber 20 Hz..20 kHz unter 0,1 dB an der BS.1770-Referenzkette; Drop/Seek(laufend UND gestoppt)/Loop-Wrap/moeglicher Straddle/Transportkante/Sampleratewechsel/Neuanlauf/Beweislagewechsel trennen JEDES offene Fenster - auch den K-Filterzustand, bitgleich gemessen - waehrend FL-Teilstuecke mit stehender Projektzeit lokal weiterlaufen, aber kein unbewiesenes Projektintervall oder FFT-Event erzeugen; Frame-Stempel umfassen den echten ueberlappenden FFT-Support und bleiben an int64/u32-Grenzen darstellbar; Drop zaehlt als Segment, alles andere als Epoche; alle sieben Erzeuger-Stempelregeln fallen einzeln und mit eigener Nummer; kein spektraler Fluss ueberbrueckt eine Grenze, der Ereignisring ist fest gedeckelt; LUFS-S trifft die analytisch gerechnete Erwartung unter 0,1 LU; die Nyquist-Kappe greift bei 22,05 kHz wirklich; verdrahtet: alle sieben Gueltigkeitsbits kommen ueber die Hostbruecke durch, der Playhead-Rueckfallweg nachweislich nur zwei. | `eq-copilot\build\plugin\EqCopAnalysisGoldenTest_artefacts\Release\EqCopAnalysisGoldenTest.exe` | [OK] Exit 0 | 2,48 s | [B5](roh/SONDE-010-b4fe522.md#b5) |
| B6 | Aktiver DSP-Kern liefert die eingefrorene Referenzantwort. | `eq-copilot\build\plugin\EqCopDspGoldenTest_artefacts\Release\EqCopDspGoldenTest.exe` | [GEPLANT] geplant (ab P6) | - | - |
| B7 | Apply/Revert ist transaktional - kein halber Zustand ueberlebt. | `eq-copilot\build\plugin\EqCopTransactionTest_artefacts\Release\EqCopTransactionTest.exe` | [GEPLANT] geplant (ab P6) | - | - |
| B10 | v3-Envelope in C++ klassifiziert den Envelope-Korpus wie das Manifest (Urteil UND Verstossmenge, alle 14 Regeln mit Negativfixture); CRC32C trifft die RFC-3720-Vektoren, P0/P1 tragen CRC exakt 0, P2 die Pflichtsumme ueber genau die Payloadbytes; 40 000 Zufallspuffer bringen den Pruefer nie aus dem Tritt und 7671 angenommene EINBIT-Mutanten gueltiger Frames halten jede Kopfregel (reiner Zufall wird praktisch immer abgewiesen - die Invariante braucht deshalb die Mutanten, sonst spraeche sie ueber eine leere Menge), 3000 gekippte P2-Bits fallen einzeln, byteweise Zustellung liefert dieselben 40 Frames und ein kaputter Frame beendet den Strom; Pipetoken trifft das Golden aus §48.3 samt SHA-256- und RFC-4648-Vektoren; P0 verwirft nichts und meldet den 65. Eintrag, P1 koalesziert an der Position und haelt Ereignisse fuer den Reconnect vor, die P2-Schleuse ersetzt den aeltesten ungesendeten Frame, uebergibt 100 000 Frames mit 0 Allokationen (mit Gegenprobe am selben Zaehler) und liefert unter Flut keinen zerrissenen Frame; verdrahtet: Control koppelt Telemetry ueber link_id + challenge, ein ungekoppelter Telemetry-Connect wird geschlossen, der Client verbindet nach Serverneustart von selbst wieder, ein kaputter Envelope vom Server schliesst die Verbindung, und ein P0-Ueberlauf WAEHREND einer stehenden Verbindung schliesst sie ebenfalls statt still zu kuerzen. | `eq-copilot\build\plugin\EqCopIpcTest_artefacts\Release\EqCopIpcTest.exe` | [OK] Exit 0 | 49,55 s | [B10](roh/SONDE-010-b4fe522.md#b10) |
| B8 | Lifecycle-Klassifikation §53.5: unclassified beim Laden und audio-neutral; Schema-1 sensor\|pre\|post -> legacy (immer passiv), hub bzw. bestaetigter Schema-2-Main-State -> main; ein Scannerlauf klassifiziert nicht; read-only nimmt die Klassifikation zurueck; Brokerstart nur fuer main mit offenem Editor; die Sondenbundles bleiben bis gueltigem State neutral und werden nie main. | `eq-copilot\build\plugin\EqCopLebenslaufTest_artefacts\Release\EqCopLebenslaufTest.exe` | [OK] Exit 0 | 0,14 s | [B8](roh/SONDE-010-b4fe522.md#b8) |


---

## NAK-104 — 2026-08-30 (Worker, hat nicht geurteilt)

**Stand:** die vier Restdefekte, die der Prüfer (Codex xhigh, Thread
`01a05313`, Stand `c72d51e`) gegen die Verhaltensmatrix gefunden hat und die
der Dirigent an der Quelle bestätigt hat, sind behoben. Quelle und Test je
Defekt, je einmal absichtlich gebrochen. **Ein Urteil spricht dieser Abschnitt
nicht** — er nennt, was gemessen wurde.

**Basis:** `7f29dc5` · **Fix-Commit:** `b4fe522` · Ticketgrenze eingehalten
(`eq-copilot/plugin/core/ipc/**`, `eq-copilot/plugin/tests/IpcTestMain.cpp`,
`broker/src/transport/**`, dieser Rundenabschnitt, `docs/beweise/roh/`); die
Matrix im lebenden Kopf ist unverändert, Änderungswünsche stehen unten als
Vorschlag.

### Die vier Defekte — Quelle, Test, Bruchprobe

| # | Matrixzeile | Quelle (Datei · Symbol) | Test (neu) | Bruchprobe rot | grün |
|---|---|---|---|---|---|
| 1 | `A-P2-03`, `A-P2-04` | `IpcQueues.h` · `P2Schleuse::veroeffentlichen` — statt `kSlots * 2` fester Versuche eine **bewiesen endende** Schleife (`for (;; ++p)`) plus Stillstandswache `kOhneFortschrittMax`; Testhaken als Templateparameter (`KeinP2Haken`, `haken.vorAnspruch`) | B10 E2 · „Kollisionsloecher lassen den Verbraucher wirklich zurueckfallen", „der Verbraucher ist zwischen JEDEM Versuch weitergerueckt", „und der Erzeuger bekommt TROTZDEM einen Platz — garantiert, nicht begrenzt versucht", „der neueste Frame liegt danach wirklich da" | `1 verworfen, 6 Loecher` + neuester Frame fehlt ([B1](roh/SONDE-010-bruchproben-nak104.md#b1--a-p2-03a-p2-04--die-alte-feste-schranke-kslots--2)) | `0 verworfen, 6 Loecher`, Frame da |
| 2 | `B-CC-07` | `IpcVerbindung.cpp` · `IpcVerbindung::schreibenGenau` — das Zeitlimit bricht mit `CancelIoEx (h, &ov)` nur SEINE Operation ab, nicht mehr per `ioAbbrechen()` die ganze Verbindung | B10 G16b · „ein vor dem Verbindungsende empfangener ACK geht nicht verloren — auch nach einem Write-Zeitlimit" | `0 ACKs, empfangen=0` ([B2](roh/SONDE-010-bruchproben-nak104.md#b2--b-cc-07--das-write-zeitlimit-sperrt-den-lesepfad)) | `1 ACKs, empfangen=1` |
| 3 | `B-CC-12`, `B-TC-07` | `ControlClient.cpp` / `TelemetryClient.cpp` · `Laufzeit::neueVerbindung`, `threadLauf`, `eineVerbindung` — eine eigene `IpcVerbindung` je `start()`, dem Thread als `shared_ptr` in die Hand gegeben; dazu `abgeloest (meinLauf)` (ein abgelöster Lauf schreibt keinen gemeinsamen Zustand mehr) und der Riegel nach dem `helloProvider` (ein abgelöster Lauf öffnet keine neue Pipe) | B10 G17 (2c) · „der zurueckkehrende alte Lauf trennt die neue Verbindung NICHT" + „und der neue Lauf beantwortet danach weiter P0"; G17 (2d) · „dieselbe Zusage gilt fuer den TelemetryClient" | `3 Versuche (vorher 2), 3 Serververbindungen (vorher 2)` — beide Clients ([B3](roh/SONDE-010-bruchproben-nak104.md#b3--b-cc-12b-tc-07--eine-verbindung-für-alle-läufe)) | `2 / 2`, Status verbunden |
| 4 | `C-LS-06` | `server_v3.rs` · `TrennStand::erwartet`, `trennmelder_telemetrie_erwartet`, `auf_telemetrie_getrennt_warten`, `kopplung_loesen` (ohne Rückgabewert) — der Wartepunkt hängt am Trennstand der Kopplung, nicht mehr am Kopplungsregister | Rust `server_v3::telemetrie_endet_zuerst_control_getrennt_folgt_trotzdem` | `["control_verbunden", "telemetrie_gekoppelt", "control_getrennt"]` ([B4](roh/SONDE-010-bruchproben-nak104.md#b4--c-ls-06--wartepunkt-am-kopplungsregister-statt-am-trennstand)) | Reihenfolge `telemetrie_getrennt` → `control_getrennt`, `lebenszyklus_reihenfolge_verletzt` 0 |

**Alle vier Bruchproben laufen gegen denselben committeten Stand `b4fe522`**,
je Probe genau ein zurückgenommener Fix. Deshalb trägt jede rote Ausgabe
dieselbe Grundzahl (207 Prüfungen), und an jeder ist ablesbar, dass die drei
anderen Proben grün bleiben — die Tests treffen je ihren eigenen Defekt und
nicht einen gemeinsamen Nebeneffekt. Rohausgabe:
[`roh/SONDE-010-bruchproben-nak104.md`](roh/SONDE-010-bruchproben-nak104.md).

### Warum keine Zahl mehr, sondern ein Beweis (Defekt 1)

Der Prüfer hat nicht „sechs ist zu wenig" gesagt, sondern „begrenztes Versuchen
ist keine Garantie". Eine höhere Zahl wäre derselbe Fehler gewesen. Die
Schleife endet jetzt aus einem Argument, das im Quellkommentar steht:

* Der Verbraucher hält je Zeitpunkt höchstens EINEN Platz und beansprucht nur
  Positionen unterhalb von `schreib`; `schreib` bewegt allein der Erzeuger, und
  der steht während des Aufrufs.
* Scheitert Versuch `p+k`, hält der Verbraucher eine Position `c ≡ p+k (mod
  kSlots)`; scheitert auch `p+k+1`, ist die dann gehaltene Position eine andere
  und — weil er nur vorwärts räumt — eine höhere.
* Die gehaltenen Positionen wachsen also streng monoton unter einer festen
  Schranke. Der Vorrat ist endlich, der nächste Versuch gelingt.

Bricht die SPSC-Zusage (zwei Verbraucher, oder einer, der nie freigibt), trägt
das Argument nicht mehr. Dann dreht die Schleife nicht ewig: bleibt der
gemeldete Verbraucherstand über `kSlots * 4` Fehlversuche stehen, fällt der
Frame gezählt (`beanspruchtVerworfen`) statt den erzeugenden Worker
festzuhalten. Der Zähler bleibt damit die Wache, die er war — er ist jetzt nur
an einen gemessenen Stillstand geknüpft statt an eine geratene Versuchszahl.

Der Testhaken ist ein **Templateparameter**, kein Laufzeitzeiger:
`P2Schleuse<SlotBytes, KeinP2Haken>` ist die Produktionsfassung, ihr
`vorAnspruch` hat keinen Rumpf. Der Erzeugerpfad behält damit exakt seine
Kosten — keine Prüfung, kein Zweig, keine Allokation (CLAUDE.md: `veroeffentlichen`
bleibt allokations-, lock- und wartefrei). Der Verbraucherstand wird erst beim
ERSTEN Fehlversuch gelesen; der häufige Weg kostet kein Atomic mehr als vorher.

### Kanon auf dem committeten Stand

`GRUEN - 32/32 Kanon-Laeufe bestanden | 2 geplante Pruefung(en) noch nicht
gebaut | 1 stillgelegte(s) Bein(e)` · Exitcode 0 · Commit `b4fe522`,
Arbeitsbaum sauber · Rohausgabe
[`roh/SONDE-010-b4fe522.md`](roh/SONDE-010-b4fe522.md) (Abschnitt „Kanon-Lauf -
SONDE-010 NAK-104" oben).

* **B10 `EqCopIpcTest`:** 207 Prüfungen, 0 Fehler (vorher 192 auf `c72d51e`).
* **Rust:** 147 + 9 + 9 (vorher 146 + 9 + 9).
* **A22 Lastbein:** 10 016 / 10 016 P0 beantwortet, max 22 ms, p99 21 ms, bei
  39 001 ersetzten P2-Frames; `p2_neueste_verworfen: 0`,
  `p2_kollisionsloecher: 0`, `p0_ueberlaeufe: 0`, Ingress-Höchststand 3;
  Brokerbericht `control_getrennt: 32`, `telemetrie_getrennt: 32`.

### Prüfliste (`tools/dirigent/pruefliste.md`) — A, B, E

* **A · Rückstau und Prioritätsklassen.** Die P2-Politik „ältesten ungesendeten
  Frame ersetzen" gilt jetzt ohne Ausnahme: der neueste Frame fällt in keiner
  Lage mehr, auch nicht bei einem wandernden Verbraucher (Defekt 1). Der
  Lesepfad hängt nicht mehr hinter „erst alles senden" — ein Write-Zeitlimit
  sperrt ihn nicht länger (Defekt 2). Der Zähler `beanspruchtVerworfen`, der
  strukturell 0 sein muss, hat weiterhin seinen Test und wird vom Lastbein A22
  ausgewertet (`p2_neueste_verworfen: 0`).
* **B · Lebenszyklus.** Beim Trennen gilt weiter „erst entkoppeln, dann Fristen
  und Joins" — der Wartepunkt hängt jetzt aber am Trennstand statt am Register,
  sonst wäre er genau in dem Fall stumm, für den er gebaut wurde (Defekt 4).
  Jeder Join behält seine Frist; `stop()` bleibt reentrant und fristbegrenzt;
  neu ist, dass ein abgelöster Lauf die Verbindung eines späteren nicht mehr
  berührt und nach einem langen Provideraufruf keine neue Pipe mehr öffnet
  (Defekt 3). Verbinden↔trennen und starten↔stoppen liegen in einem Commit.
* **E · Behauptung ≤ Messung.** Jede der vier neuen Prüfungen wurde einmal
  gebrochen, die Rohausgabe liegt bei, und jede misst ihre Vorbedingung, statt
  sie zu hoffen (Rückstand von sechs Positionen und sechs Sprünge bei Defekt 1;
  „der Peer liest kein einziges Byte" bei Defekt 2; „der neue Lauf steht
  NACHGEWIESEN" — Status `verbunden` gibt es nur nach vollständig geprüftem
  Welcome — bei Defekt 3; `betreten_anzahl("telemetrie_getrennt") == 1` bei
  Defekt 4). Die Reihenfolgeprüfung in Rust vergleicht ausgepackte Positionen,
  weil `Option::cmp` ein fehlendes `telemetrie_getrennt` sonst wie die richtige
  Reihenfolge aussehen ließe. Alle Zahlen oben stammen aus dem Kanonlauf auf
  `b4fe522`, keine ist abgeschrieben.
* **Geänderte Zusage — drei Stellen.** Die Zusagen selbst sind unverändert
  (`A-P2-04` sagte schon „der Erzeuger bekommt IMMER einen Platz"); geändert
  hat sich nur, wie sie eingelöst werden. `git grep -n "kSlots \* 2"` und
  `git grep -ln "beanspruchtVerworfen"` zeigen als Fundstellen außerhalb des
  Codes nur Verlaufstexte (`docs/beweise/SONDE-010.md` Rundentabelle,
  `SONDE-010-verlauf.md`, `NAK-96.md`, `docs/offene-punkte.md`) — die sind
  append-only und werden nicht umgeschrieben. Die B10-Behauptung in
  `tools/beweise.ps1` nennt die P2-Politik unverändert richtig und blieb
  deshalb unangetastet. Eine Stelle außerhalb der Ticketgrenze ist jetzt
  falsch, siehe Vorschlag 6.

### Vorschläge an den Dirigenten (nicht ausgeführt)

Die Matrix ist Spezifikation und wurde nicht angefasst. Fünf Testspalten nennen
die neuen Prüfungen noch nicht, und eine Stelle außerhalb der Ticketgrenze
trägt jetzt eine überholte Begründung:

1. `A-P2-04`, Spalte Test: die deterministische Probe mit **wanderndem**
   Verbraucher aufnehmen (B10 E2 · „der Erzeuger bekommt TROTZDEM einen
   Platz — garantiert, nicht begrenzt versucht", samt der zwei
   Vorbedingungszeilen). Die dort genannte Probe hält zwei Plätze gleichzeitig
   und lässt den dritten statisch frei; sie bildet das Wandern nicht ab.
2. `A-P2-04`, Spalte Zusage: aufnehmen, dass „bekommt IMMER einen Platz" unter
   der SPSC-Zusage gilt und `beanspruchtVerworfen` nur noch bei GEMESSENEM
   Stillstand des Verbrauchers wächst (`kSlots * 4` Fehlversuche ohne
   Fortschritt). Heute liest sich die Zeile so, als sei der Zähler
   unerreichbar; erreichbar ist er genau dann, wenn die Zusage selbst gebrochen
   ist — das ist seine Aufgabe.
3. `B-CC-07`, Spalte Test: **G16b** ergänzen. Das bisher genannte G16 liefert
   den ACK nach einem erfolgreichen Write und fährt den Fehlerpfad nicht.
4. `B-CC-12` und `B-TC-07`, Spalte Test: **G17 (2c)** und **G17 (2d)**
   ergänzen — Ablösen, `start()`, nachgewiesene neue Verbindung, DANN den alten
   Callback freigeben.
5. `C-LS-06`, Spalte Test: `server_v3::telemetrie_endet_zuerst_control_getrennt_folgt_trotzdem`
   ergänzen. Die dort genannten Proben beenden alle Control zuerst.
6. **Außerhalb der Ticketgrenze:** `docs/plugin-wissen.md` (Abschnitt
   P2-Schleuse) begründet noch mit „Der zweite Platz ist immer frei, weil es
   genau einen Verbraucher gibt." Genau diese Begründung hat der Prüfer
   widerlegt — sie gilt nur, wenn der Verbraucher zwischen den Versuchen
