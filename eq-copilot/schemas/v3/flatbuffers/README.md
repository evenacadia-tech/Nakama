...
| Rust-Crate | `broker/Cargo.toml` | `pruefe_flatc_drift.py` |
...
Puffer, `required`-Felder sind da, Vektorlängen passen zum Puffer. Das ist der
eine Punkt, an dem das Format wirklich hart ist, und beide Leser benutzen ihn,
...
| `bandwert_bereich` | Ein i16-Bandwert außerhalb des Bereichs seiner Kodierung (`quantisierung-v1.json`). |
