...
bytegleich) einen Eintrag, fährt clang-tidy aus den Build Tools 2022
(`VC\Tools\Llvm\x64\bin`) parallel darüber und zählt eindeutige Fundstellen
(Datei, Zeile, Spalte, Check) gegen die Ratsche in der Datei (Grenze =
Iststand der Erstmessung, Ziel 0). Regelsatz `eq-copilot/plugin/.clang-tidy`
(bugprone, clang-analyzer, concurrency, performance, Auswahl aus cert,
cppcoreguidelines, misc; Ausnahmen dort begründet). Wie A32 nicht blockierend
