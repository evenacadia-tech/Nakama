...
| NAK-16 | 08-19 | **ST-Map-Live-Refraktion** (User-Freigabe 19.08., ersetzt den NAK-15-Ansatz nach dessen Verwurf): Cycles backt pro Drehframe eine Refraktionskarte (UV-Gradient-Plate, ein IOR, kein Denoise, 16 Bit als Hi/Lo-Split in RGBA-PNG); WebGL im Blatt sampelt damit den LEBENDEN Nexus-Canvas — das Glas bricht den echten Hintergrund. Glanz aus `--nur-glas`-Pass. Erst EIN Probe-Still beweisen (Maßstab: Glasinneres ändert sich mit dem Hintergrund — Ähnlichkeit zum alten Frame ist als Metrik VERBOTEN), dann 72er-Sequenz. Plan im Detail: `docs/NEXT-SESSION.md`. |
