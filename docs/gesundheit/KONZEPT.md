...
| C++-Statikanalyse (seit NAK-288, 12.09.2026) | `tools/plan/tidy.py` (Bein A33, Exit 4 bei Riss; Regelsatz `eq-copilot/plugin/.clang-tidy`, eigener Ninja-Baum `eq-copilot/build-tidy`) | clang-tidy über alle Übersetzungseinheiten des Plugins ohne Tests und ohne den flatc-Codegen: Lebensdauer, Aliasing, nicht initialisierte Werte, unbeabsichtigte Kopien, thread-unsichere Bibliotheksaufrufe, Objektschneiden — eindeutige Fundstellen als Ratsche, Ziel 0 |
...
| `tidy.py` | GRENZE | wie heute: clang-tidy-Ratsche über das Plugin, bleibt Bein A33 (Minuten, weil der Ninja-Baum konfiguriert und jede Einheit geparst wird — im Prüfgang vor einem Ticket deshalb nur mit `--nur` auf die Ticketpfade) | NAK-288 |
...
|---|---|---|
| 19.09.2026 | §4.3 `/tiefenaudit` bekommt eine Agentenfassung als Entwurf (`docs/gesundheit/tiefenaudit/LIES-MICH.md`, Karte `karte.json`, Werkzeug `tools/pruefung/tiefenaudit_karte.py`; Skill aus der Vorlage angelegt, `88502c62`; noch kein Lauf). §9 Zeile 2 bleibt in der Sache stehen und wird präzisiert: feste Arbeitsbereiche je Agent kommen zurück, aber als Daten mit Vorprüfung; jeder Auftrag wird gerechnet, keiner von Hand gepflegt. Register NAK-333 | User-Auftrag 19.09.2026 (Wortlaut in `docs/gesundheit/tiefenaudit/LIES-MICH.md`); Ausgangsmaterial Tiefenaudit 3 |
