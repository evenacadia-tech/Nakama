---
title: "Claude UX/UI-Arbeitskern - Nakama-Design"
...
## 3. Projekt-Overrides (Nakama-Design)

Vor Designarbeit gelten `CLAUDE.md` dieses Projekts und die dort genannten
Quellen. Besonders wichtig:

- Vision und Referenzen kommen vom User; Claude hilft beim Praezisieren
  und baut erst nach Freigabe. Fehlende Vorstellung wird per `/interview`
  erfragt, nie durch eigene Stilentscheidung ersetzt.
- Die Assettruhe (`assets/`) ist vor jeder Gestaltungsentscheidung die
  erste Quelle; Abnahmen (`abnahmen/`) sind bindend.
- Was die Oberflaechen zeigen MUESSEN, steht abschliessend in
  `oberflaechen-spezifikation.md`; keine Inhalte oder Zustaende erfinden.
- Lesbarkeit ist Produktmassstab: Ein Profi muss den relevanten Befund
  sehr schnell ablesen koennen.
- Glow, Bloom, Weichzeichner und generische Effekt-Baukastenoptik sind
  kein Ersatz fuer Struktur oder Handschrift.
- Im Prototyp sind Demo-Daten erlaubt: realistisch UND sichtbar als Demo
  gekennzeichnet (die Regel "jeder Wert braucht eine reale Quelle" gilt
  dem spaeteren Produkt).
- Dieses Projekt gestaltet die konventionelle Plugin-UI. Die daneben
  existierende eigenstaendige Begleit-App ist nicht Thema und kein
  Referenzmaterial.
- Pixelkorrektheit, Bedienbarkeit, Markenpassung und Schoenheit sind
  getrennte Beweise.
