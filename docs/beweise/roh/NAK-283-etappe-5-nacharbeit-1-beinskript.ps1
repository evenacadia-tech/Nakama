# NAK-283 Etappe 5 Nacharbeit 1 — die Beine, einzeln gefahren (Bauer, 13.09.2026)
#
# Kein voller Kanon und kein Laufzeit-Arm (beides am Ticketende, §6.8).
# Gefahren werden die Beine des Auftrags und JEDES weitere Kanon-Bein, dessen
# Pruefbinary eine geaenderte Quelle dieser Nacharbeit uebersetzt oder linkt.
# Bauform wie docs/beweise/roh/NAK-283-etappe-5-beinskript.ps1: die Auswahl
# rechnet MSBuild selbst - CL.read.*.tlog nennt jede Datei, die cl.exe fuer
# ein Ziel geoeffnet hat, link.read.*.tlog jede Bibliothek, gegen die es
# gelinkt wurde. Die Kanonzeilen kommen aus tools/beweise.ps1
# (PowerShell-AST), jedes Bein faehrt mit genau den Argumenten des Kanons.
#
#   Auftrag:  B18 (Ziel) B16 B17 B19 B20 B22 B23 B24 B25 B9 B5 A10 B3c A26,
#             dazu A16 und A1 als Wache fuer den Audiopfad.
#   Python mit Artefakt aus eq-copilot/build/plugin (Zuordnung wie Etappe 5):
#             gefahren, wenn ihr Artefakt eine geaenderte Quelle liest oder linkt.
...
#   A33 (tools/plan/tidy.py) laeuft getrennt.
#
# Vor dem Bau: kein fremder Bau-, Test- oder Kanonprozess (NAK-290), sonst Abbruch.
#
# Aufruf aus dem Workspace-Root:
#   pwsh -File docs/beweise/roh/NAK-283-etappe-5-nacharbeit-1-beinskript.ps1
# Rohausgabe: docs/beweise/roh/NAK-283-etappe-5-nacharbeit-1-beine.txt

[CmdletBinding()]
param(
    [string] $Wurzel = (Resolve-Path (Join-Path $PSScriptRoot '..\..\..')).Path,
    [string] $Ziel   = (Join-Path $PSScriptRoot 'NAK-283-etappe-5-nacharbeit-1-beine.txt')
)
...
$fremd = @(Get-Process -Name cl, link, MSBuild, cargo, rustc, cmake, clang-tidy -ErrorAction SilentlyContinue) +
         @(Get-Process | Where-Object { $_.Name -like 'EqCop*' })
if ($fremd.Count -gt 0) { throw "Fremder Bau- oder Testprozess laeuft (NAK-290): $(($fremd | ForEach-Object { "$($_.Name)#$($_.Id)" }) -join ', ')" }
...
$basis = 'd8c3f74c656bb88a08999edbf6705440a4f2e71a'

# Die geaenderten Quellen dieser Nacharbeit.
$quellen = @(
...
$auftrag = [ordered]@{
    ...
    'B18' = 'Auftrag (Ziel)'; 'B16' = 'Auftrag'; 'B17' = 'Auftrag'; 'B19' = 'Auftrag'; 'B20' = 'Auftrag'
...
$zeilen = @(
    'NAK-283 Etappe 5 Nacharbeit 1 - Beine, einzeln gefahren',
    "Zeitpunkt: $(Get-Date -Format o)",
    "HEAD: $(git -C $Wurzel rev-parse HEAD)",
    "Basis: $basis",
    'Fremde Bau-, Test- oder Kanonprozesse vor dem Bau: keine (NAK-290).',
...
# wenn sie auf einer Zeile steht, die diese Nacharbeit gegenueber dem Basis-SHA
...
$zeilen += @(
    ...
    "warning C (eindeutige Zeilen): $($warnungen.Count) an $($warnungsStellen.Count) Stellen; davon auf einer Zeile, die diese Nacharbeit hinzugefuegt oder geaendert hat: $($warnungenAusGeaendert.Count)")
...
foreach ($k in $kanonPlugin) {
    ...
    $zeilen += ('{0,-5} {1,-38} {2,-60} {3}' -f $k.Kuerzel, $k.Name, (Kurz $a.Quellen), $status)
}
...
$zeilen += ''
$zeilen += ("URTEIL: {0} von {1} Beinen Exit 0; warning C auf von dieser Nacharbeit geaenderten Zeilen: {2}" -f ($bilanz.Count - $rot.Count), $bilanz.Count, $warnungenAusGeaendert.Count)
