...
- Messwerte an andere Programme gibt Nakama nur in den FL-Sitzungen heraus, die die Automatik
  für ihre Messläufe selbst startet; in deinen eigenen Sitzungen ist dieser Weg zu (U59; Bau in
  NAK-310).
- Drückst du „Festhalten“ zweimal in derselben Sekunde, oder speichern zwei gleichnamige
  Messpunkte gleichzeitig, bleiben beide Messstände erhalten: die zweite Datei bekommt einen kleinen
  Zusatz im Namen, und die Meldung nennt genau die Datei, die entstanden ist; nichts wird ersetzt
  oder gelöscht (U65; Bau in NAK-313, bis dahin wird der zweite Export abgelehnt und die erste
  Datei bleibt).
- Kommt die Verbindung zwischen Gen und dem Hintergrunddienst nicht zustande, unterscheidet Gen
  ehrlich zwischen „Dienst nicht erreichbar“ (mit dem Handgriff, der hilft) und „Fassungen passen
  nicht zusammen“ (kein Handgriff); welche Ablehnung in welche Meldung fällt, wird mit NAK-313
  Etappe 6 festgezogen (NAK-392).
...
- **Mono-Bass (U61):** den Bass sauber in die Mitte legen mit einer kleinen Verzögerung, oder
  verzögerungsfrei mit leichtem Übersprechen wie heute? Bis zur Abnahme der Klangregelung bleibt
  das gebaute Verhalten.
- **Dynamisches Band (U62):** worauf hört es (nur auf seinen eigenen Bereich oder auf einen
  wählbaren), und wie greift es? Bis zur Abnahme der Klangregelung bleibt das gebaute Verhalten.
- **Güte bei Cuts und Shelves (U63):** Steilheit wählbar wie bei Pro-Q, oder Resonanz wie heute?
  Bis zur Abnahme der Klangregelung bleibt das gebaute Verhalten.
- **Mix-Regler (U64):** paralleles Mischen wie Dry/Wet (mit Löchern an steilen Cut-Kanten), oder
  halbe EQ-Kurve? Bis zur Abnahme der Klangregelung bleibt das gebaute Verhalten.
- **Geparkt, bis die Technik fertig ist (Designteil, 12.09.2026):** das Aussehen beider
  ...
  Befunde in den Fokus (U37), Ort des Hinweises „unter X Hz nicht gesucht", wenn die
  Befundliste nur Zonenkarten zeigt (U66).
