...
| Etappe | **Etappe 1 — Spezifikation vor Code.** Dieses Manifest trägt Gate-Text, Ist-Stand, Verhaltensmatrix, Bauplan und Entscheide. **Kein Produkt-, Test-, Schema-, Fixture- oder Werkzeugcode in diesem Änderungssatz.** Etappen 2 bis 4 (Verträge, DSP-Kern, Integration) beginnen erst nach der lesenden Codex-Matrixprüfung und dem Fortsetzungsauftrag des Dirigenten. |
| Phase | **Offen.** Etappe 1 abgeschlossen; Matrixprüfung steht aus. |
...
**Die Matrix-IDs `M-01` bis `M-120` dieses Manifests sind eigen.** Sie sind
...
Vorschlag. Fünf Stellen weichen begründet von ihrem Wortlaut ab; sie stehen
gesammelt in §5.17.
...
| `state_hash` | Inhalt von `state_report.dsp` | Der Hash deckt das **volle** DTO; die Wire-Nutzlast ist eine geprüfte **Teilmenge** davon (ohne `v2.global.eq_enabled`, §5.13). Ein Empfänger, der den Hash aus der Nutzlast nachrechnet, bekommt zwangsläufig einen anderen Wert. |
...
| M-02 | `eq_enabled` false, alle 120 Werte auf Default | Auch **ohne** Hard-Bypass ist der neutrale Zustand bitidentisch: 0 dB Trim läuft **nicht** als Multiplikation durch die Samples, sondern über einen Unity-Kurzschluss — einschließlich der Rampe, die auf dem Weg zu 1,0 durch `0.99999994f` laufen kann. | **BELEGT** (NAK-81 b) · BAULÜCKE | **A16**, Fall `bypass_false_alles_neutral_ist_bitidentisch`; **NEU** **B6**, Fall `unity_kurzschluss_statt_multiplikation` | Der Ausgang weicht um ein LSB ab, weil `x * powf(10, 0/20)` gerechnet wurde; oder die Rampe endet auf `0.99999994f` statt exakt `1.0f` | NAK-81 (b) wörtlich (§1.15); `nakama-parameter-v1.json:15` |
...
| M-15 | Zwei Slots liegen auf derselben Frequenz | Die Bänder wirken **hintereinander in Slotreihenfolge 0 bis 7**, nicht parallel summiert. Die Reihenfolge ist Teil des Goldens; sie ändert sich nie und hängt nicht davon ab, in welcher Reihenfolge die Slots belegt wurden. | **ENTSCHIEDEN (R-Feinheit, §5.2)** · BAULÜCKE | **NEU** **B6**, Fall `slotreihenfolge_ist_die_kettenreihenfolge` | Zwei identische Bänder auf verschiedenen Slots liefern verschiedene Ergebnisse je nach Belegungsreihenfolge | §53.8 „acht feste Band-Slots mit stabilen Parameter-IDs" |
...
| M-37 | `auto_gain` = true, ein High-Shelf mit +6 dB überstreicht das ganze Gitter | Der abgeleitete Wert liegt bei **−6 dB ±0,1 dB**. Gerechnet wird der pink-gewichtete Energiemittelwert der statischen Betragsantwort über ein logarithmisches Gitter von 20 Hz bis 20 kHz in 1/12-Oktav-Schritten (121 Stellen, gleiche Gewichtung je Oktave), Formel und Kanalfaltung in §5.4. | **ENTSCHIEDEN (R4)** · BAULÜCKE | **NEU** **B6**, Fall `shelf_plus_sechs_ergibt_minus_sechs`; Referenzkurvenkorpus | Der Wert weicht um mehr als 0,1 dB ab; oder er kippt bei einem schmalen tiefen Notch ins Absurde, weil in dB statt in Energie gemittelt wurde | R4 |
...
| M-61 | Direkt nach dem Remove | Der Undo-Ring trägt **ein** Objekt mit Slot-ID, Typ, `channel_mode`, Dynamic-Zustand und **sämtlichen** Werten. Undo stellt es als **ein** Objekt in **einer** Transaktion wieder her — Slot-ID und Band-ID sind dieselben wie vorher. | **BELEGT** · BAULÜCKE | **NEU** **B7**, Fall `undo_stellt_das_entfernte_band_als_ein_objekt_her` | Undo stellt die Werte her, aber nicht `channel_mode`; oder es braucht zwei Schritte; oder das Band landet auf einem anderen Slot | Fünferblock 02 `:136-150`; R5 |
...
| M-67 | Eine Transaktion setzt ein belegtes, eingeschaltetes Band **neu** in eine eingeschaltete Zone | Die Transaktion wird **abgewiesen**, mit dem eigenen Grund „User-Schutz" und **ohne halbe Anwendung**: der bestätigte Zustand, die Revision und der Hash bleiben unverändert. Getroffen ist genau der Fall, in dem sich der Slot ändert (Frequenz, `enabled` oder `occupied`) und das Prädikat „belegt und eingeschaltet und Frequenz in einer eingeschalteten Zone" dadurch von falsch auf wahr kippt. | **ENTSCHIEDEN (R6)** · BAULÜCKE | **NEU** **B7**, Fälle `band_in_zone_ziehen_wird_abgewiesen`, `einschalten_in_der_zone_wird_abgewiesen` und `abweisung_laesst_hash_und_revision_unveraendert` | Das Band landet in der Zone; oder die Frequenz wird gesetzt und `enabled` nicht, sodass ein halber Zustand entsteht | R6; Interview `:128-135` „Harte Sperre für alle" |
...
| ID | Zustand × Ereignis | Zusage samt Reihenfolge und Frist | Belegklasse | Test | Rotbeweis an der Zusage | Quelle |
|---|---|---|---|---|---|---|
| M-74 | Apply, Revert, Neutralisieren, Remove, Undo, Redo oder Preset-Laden | Jede dieser sieben Handlungen ist **eine atomare Transaktion mit genau einer neuen `state_revision`** — nicht null, nicht zwei. Die Reihenfolge ist fest: validieren → Zonenriegel → Programm bauen → Revision erhöhen → Committed tauschen → Hash bilden → Host-Dirty melden → Bericht senden. | **BELEGT** · **ENTSCHIEDEN (R11)** · BAULÜCKE | **NEU** **B7**, sieben Fälle `<handlung>_erzeugt_genau_eine_revision` | Neutralisieren zählt zwei Revisionen, weil es intern zweimal schreibt; oder Undo zählt gar keine und der Hash läuft aus dem Tritt | §44.3; §44.4 „`Neutralisieren` ist selbst eine versionierte Transaktion"; R11 |
| M-75 | Ein Fehler tritt **mitten** in einem Apply auf | Der bestätigte Zustand bleibt **unverändert**, der `state_hash` bleibt **unverändert**, und die Revision steigt **nicht**. Das ist durch die Reihenfolge garantiert: das vollständige Kandidaten-DTO und sein Programm entstehen **vor** dem Tausch; scheitert eine Stufe, wird nichts getauscht. | **BELEGT** · BAULÜCKE | **NEU** **B7**, Fall `fehler_mitten_im_apply_laesst_committed_unveraendert` — mit **je einem** Einspritzpunkt pro Stufe (Validierung, Zonenriegel, Programmbau, Banktausch) | Nach einem Fehler in der dritten Stufe trägt der Committed-Zustand die ersten zwei Änderungen | R11; §44.3 |
| M-76 | 10.000 doppelte, vertauschte und veraltete Befehle | Sie erzeugen **höchstens eine** gültige Revision und **niemals** einen Mischzustand. Ein Befehl mit falscher `base_revision` wird als Konflikt abgewiesen; eine wiederholte `command_id` gilt als idempotent und erzeugt keinen zweiten Zustand. | **BELEGT** · BAULÜCKE | **NEU** **B7**, Fall `zehntausend_befehle_erzeugen_hoechstens_eine_revision` | Zwei Revisionen entstehen; oder ein vertauschtes Paar hinterlässt Werte aus beiden Befehlen | §44.5 wörtlich |
...
| ID | Zustand × Ereignis | Zusage samt Reihenfolge und Frist | Belegklasse | Test | Rotbeweis an der Zusage | Quelle |
|---|---|---|---|---|---|---|
| M-95 | Ein Preset wird geschrieben | Eigenes Schema mit `preset_schema_version` = 1, **nie** mit `dsp_schema_version` vermischt. Es trägt zusätzlich die `dsp_schema_version`, mit der es geschrieben wurde — damit ein späteres Layout weiß, was es liest. | **BELEGT** · **ENTSCHIEDEN (R12)** · BAULÜCKE | **NEU** **B7**, Fall `preset_traegt_zwei_getrennte_versionen`; **A11** für die Schemagültigkeit | Das Preset trägt nur eine Version und ein v3-Layout kann seinen Inhalt nicht mehr einordnen | R12; U18 „eigene Kennungen und eine eigene Version, die nie mit der Zustandsversion vermischt wird" |
...
| M-103 | Die Nutzlast | Das **validierte DSP-DTO** (`dsp_schema_version` 2, global v1 und v2, acht Bänder mit `occupied`, Zonen) plus die abgeleiteten Angaben `auto_gain_db`, Klemmliste und verletzte Bänder. Alles darin ist **abgeleitet oder bestätigt**, nichts ist eine Vorschau. | **BELEGT** · **ENTSCHIEDEN (R13)** · BAULÜCKE | **A5**, **A8** (Fixtures), **B3c**, **A4**; **NEU** **B7**, Sendepfad | Eine Vorschau reist im Bericht; oder eine abgeleitete Angabe fehlt und Gen kann den Auto-Gain-Wert nicht mitwandern lassen | R13 |
| M-104 | `v2.global.eq_enabled` und die Nutzlast | Der Schlüssel wird aus der Wire-Nutzlast **ausgenommen**: `dsp.parameters` trägt ihn nicht, und der Leser **weist einen Bericht ab, der ihn enthält**. Grund: `state_report.eq_enabled` ist für S29–31 reserviert, und dieselbe Angabe unter einem anderen Namen zu liefern wäre genau der Vorgriff, den die Feldregel verbietet. Der Schlüssel bleibt im **DTO** und im **`state_hash`** (§5.13). | **ENTSCHIEDEN (R2, R13, Abweichung 3 in §5.17)** · BAULÜCKE | **A5**, Fall `dsp_nutzlast_ohne_eq_enabled`; **B3c** und **A4** je ein Ablehnungsfall; **NEU** **B7**, Sendepfad | Der Sender packt den vollen Parametersatz ein und liefert den EQ-Betriebszustand vor S29–31; oder der Leser nimmt ihn an | R2 „hier reist nichts davon"; R13; `reservierte-nachrichten-v1.json:242` |
| M-105 | Ein Empfänger will den Hash prüfen | Er **rechnet den `state_hash` nicht aus der Nutzlast nach**. Die Nutzlast ist eine geprüfte Teilmenge (M-104); ein nachgerechneter Hash wäre zwangsläufig verschieden. Der Bericht trägt den Hash weiterhin als eigenes Feld. | **ENTSCHIEDEN (§5.13)** · BAULÜCKE | **A4** (Rust), Fall `broker_rechnet_den_hash_nicht_nach`; **A5** als Vertragssatz | Der Broker verwirft jeden Bericht als inkonsistent, weil er den Hash nachrechnet | §5.13; §2.12 |
...
|---|---:|---:|
| **BELEGT** (aus verbindlicher Quelle, ganz oder teilweise) | 73 | 61 % |
| **ENTSCHIEDEN** (R1 bis R15 samt Feinheiten in §5) | 72 | 60 % |
| davon **allein** durch einen Dirigentenentscheid getragen | 47 | 39 % |
| **BAULÜCKE** | 120 | 100 % |
...
**Matrixzeilen, die hier fallen müssen:** M-14, M-24 (Vertragshälfte), M-28
(Enumgrenze), M-65, M-66, M-72 bis M-73 (DTO-Hälfte), M-77, M-78
(Vertragsgrenze), M-86 bis M-94, M-95 bis M-99, M-101 bis M-105, M-108 bis
M-112.
...
Revisionen (M-74 bis M-85), Belegung und Remove/Undo (M-58 bis M-65), Zonen
(M-66 bis M-73), Preset (M-95 bis M-100).

**Bein A16 `EqCopProbeeqNullTest`, umgeschrieben.** Es misst danach: Default-Stand
bitidentisch über 1000 Blöcke bei vier Sampleraten (M-01), 0 Latenz und kein Tail
im Passthrough (M-51), Save/Load/Save bytegleich in v2 (M-93), `eq_enabled` true
mit `bypass` true bitidentisch (M-05), nach `eq_enabled` false wieder bitidentisch
nach dem Fade (M-04), Mix 0 bitidentisch (M-33), Passthrough sanitisiert nicht
(M-50). Die alte Behauptung „kein Hostparameter" fällt (M-116).
...
Fünf Stellen weichen vom Wortlaut einer Regel ab. Sie stehen jeweils unter
ihrer Regel **und** gesammelt in §5.17.
...
5. **`eq_enabled` ist im DTO und im Hash.** Es ist ein persistenter Host-Parameter wie `bypass` und gehört damit in den bestätigten Zustand. Es reist aber **nicht** über den Draht — siehe §5.13 und Abweichung 3 in §5.17.
...
3. **Kanalfaltung.** `E(f) = ½ · (\|H_L(f)\|² + \|H_R(f)\|²)`, wobei `H_L` das Produkt aller Bänder mit `channel_mode` aus {stereo, left, mid, side} ist und `H_R` das Produkt aller mit {stereo, right, mid, side}. Ein Mid-Band wirkt auf L und R betragsgleich für ein korreliertes Signal, ein Side-Band betragsgleich für ein dekorreliertes; eine exakte M/S-Faltung bräuchte eine Annahme über die Korrelation des Materials — und die wäre genau der materialabhängige Anteil, den R4 ausschließt.
...
3. **Die Kollisionsregel prüft eine Kante, keinen Zustand.** Abgewiesen wird eine Transaktion genau dann, wenn für mindestens einen Slot das Prädikat P = (belegt ∧ eingeschaltet ∧ `freq_hz` in einer eingeschalteten Zone) **von falsch auf wahr kippt** UND die Transaktion mindestens eines der Felder `occupied`, `enabled` oder `freq_hz` dieses Slots ändert. Damit gilt: hineinziehen wird abgewiesen (M-67), eine Zone über einem liegenden Band einschalten ist erlaubt und meldet Verletzung (M-68), und ein bereits verletztes Band bleibt bedienbar (M-69). Ein reiner Zustandsvergleich „P ist wahr ⇒ ablehnen" würde M-68 und M-69 beide brechen.
...
2. **Die feste Reihenfolge innerhalb einer Transaktion:** validieren → Zonenriegel → Kandidaten-DTO vollständig bauen → Programm bauen (kann `busy_retry` liefern) → Revision erhöhen → Committed tauschen → Hash bilden → Undo-Eintrag ablegen → Host-Dirty melden → `state_report` senden. Scheitert eine Stufe, wird nichts getauscht (M-75). Die Reihenfolge ist die Zusage; sie steht so im Bauplan, damit der Einspritzpunkt je Stufe im Rotbeweis benannt werden kann.
...
4. **Idempotenz über `command_id`, Konflikt über `base_revision`.** Beides existiert bereits im In-Flight-Register des `ControlClient` (SONDE-014); dieses Ticket benutzt es, es baut kein zweites.
...
1. **Abweichung 3 (§5.17): die Wire-Nutzlast lässt `v2.global.eq_enabled` aus.** R13 sagt „das validierte DSP-DTO", R2 sagt „`state_report.eq_enabled` bleibt reserviert; hier reist nichts davon". Beide gleichzeitig sind nur erfüllbar, wenn `dsp.parameters` diesen einen Schlüssel **nicht** trägt. Gebaut wird genau das: der Schlüssel bleibt im DTO und im `state_hash` (er ist persistent und automatisierbar), aber die Wire-Nutzlast ist eine **geprüfte Teilmenge** — der Leser weist einen Bericht ab, der ihn enthält. Die Alternative, `eq_enabled` ganz aus dem DTO zu nehmen, wurde verworfen: dann wäre der einzige Schalter, der über bitidentisch oder nicht entscheidet, nicht im Hash, und ein Projekt-Reload könnte denselben Hash für zwei verschiedene Klangzustände melden.
2. **Der Empfänger rechnet den Hash nicht nach.** Folgt aus Punkt 1 und steht als eigene Zeile (M-105), weil ein Empfänger, der es doch tut, jeden Bericht als inkonsistent verwirft — ein Fehler, der erst im Feld auffällt.
...
Fünf Stellen bauen etwas anderes, als der Wortlaut einer Regel sagt. Jede steht
mit ihrer Begründung auch unter ihrer Regel; hier stehen sie zusammen, damit
die Matrixprüfung sie an einer Stelle findet.
...
| 3 | **R13:** die Nutzlast ist „das validierte DSP-DTO" · **R2:** „`state_report.eq_enabled` bleibt reserviert; hier reist nichts davon" | `dsp.parameters` lässt `v2.global.eq_enabled` **aus**; der Leser weist einen Bericht ab, der ihn enthält. Der Schlüssel bleibt im DTO und im `state_hash`. | Beide Regeln sind sonst nicht gleichzeitig erfüllbar. Dieselbe Angabe unter einem anderen Namen zu liefern wäre der Vorgriff, den die Feldregel verbietet (`reservierte-nachrichten-v1.json:242`). Die Alternative — `eq_enabled` ganz aus dem DTO — wurde verworfen, weil dann der Schalter, der über bitidentisch entscheidet, nicht im Hash läge und zwei verschiedene Klangzustände denselben Hash melden könnten. |
