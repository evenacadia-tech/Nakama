...
| Rundenbilanz Etappe 2 | **Nacharbeit 3:** `py -3.13 tools/dirigent/rundenbilanz.py --runden 8c02520 899df16 71194a7 a3654f5 562c4cc 14e0d5c`: W1+W2 Produkt 13 Dateien +2895/−1924, Tests 5 Dateien +771/−28; W3 Produkt 8 Dateien +594/−210, Tests 1 Datei +356/−2; W4 Produkt 5 Dateien +258/−8, Tests 1 Datei +286/−0; W5 Produkt 2 Dateien +96/−1, Tests 1 Datei +108/−0; W6 Produkt 9 Dateien +278/−93, Tests 2 Dateien +301/−11. Über die ganze Runde `8c02520..14e0d5c`: **Produkt 25 Dateien +4120/−2235 · Tests 5 Dateien +1822/−41 · Prüfwerkzeug 1 Datei +26/−0 · Doku 22 Dateien +3834/−0**. **Kein Konvergenz-Signal** — jede der fünf Klassen trägt echten Produktdiff. Was diese Runde von der vorigen unterscheidet, ist die Konstellation der Messung, nicht ihre Menge. Davor: `py -3.13 tools/dirigent/rundenbilanz.py --runden 9876822 725ab66 6631c36 da00cc7 fb92928 163b865 14dea8c`: V4 Produkt 12 Dateien +299/−18, Tests 4 Dateien +532/−14; V7 Produkt 3 Dateien +97/−16, Tests 1 Datei +132/−0; V6 Produkt 6 Dateien +185/−16, Tests 2 Dateien +244/−12; V1+V2 Produkt 23 Dateien +2391/−251, Tests 5 Dateien +817/−11; V5 Produkt 10 Dateien +758/−94, Tests 4 Dateien +626/−47; V3 Produkt 26 Dateien +1552/−510, Tests 2 Dateien +489/−0. Über die ganze Runde `9876822..14dea8c`: **Produkt 42 Dateien +4774/−397 · Tests 9 Dateien +2836/−80 · Prüfwerkzeug 2 Dateien +173/−8 · Doku 33 Dateien +3306/−0**. **Kein Konvergenz-Signal** — jede der sechs Klassen trägt echten Produktdiff. Die Zahl allein trägt hier keine Aussage: auch Runde 1 hatte Produktdiff, und trotzdem war der Produktpfad nicht geschlossen. Was diese Runde unterscheidet, ist der Ort der Messung, nicht ihre Menge. |
...
| Kanon nachher | **Nacharbeit 3: GRUEN 53/53 auf `043e98e`, Arbeitsbaum sauber** — die Beglaubigung des Endstands dieser Runde, Rohausgabe `roh/SONDE-013-043e98e.md`. Der Weg dahin gehört zum Beleg: der erste Abschlusslauf dieser Runde wurde **abgebrochen**, weil das Selbstaudit während seines Laufs zwei Funde im eigenen Diff ergab (negativer Blockanfang, fehlende Paarneubildung im Befehlspfad — `9af2f9c`). Ein Lauf auf einem Baum, der sich unter ihm ändert, beglaubigt nichts; er ist deshalb ohne Rohausgabe geblieben, und der Lauf auf `043e98e` ist der einzige dieser Runde. Davor: **Nacharbeit 2: GRUEN 53/53 auf `b3eccf7`, Arbeitsbaum sauber** — die Beglaubigung des Endstands dieser Runde, Rohausgabe `roh/SONDE-013-b3eccf7.md`. Der Weg dahin gehört zum Beleg: der erste Abschlusslauf auf `14dea8c` kam **ROT 52/53** zurück (`roh/SONDE-013-14dea8c.md`). Gefallen ist **B5** `EqCopAnalysisGoldenTest` mit Exit `0xC00000FD` (STACK_OVERFLOW) in `zwillingsprobe()`: die Funktion legt ZWEI `FeatureEngine`-Objekte in ihren Rahmen, jedes rund ein halbes Megabyte, und sie wird aus `main` gerufen, dessen Rahmen schon mehrere davon trägt — das neue Feld `Passagenfenster::epoche` (R03) kippte den 1-MB-Vorgaberahmen von MSVC. Beide Engines liegen seither auf dem Heap (`b3eccf7`); gemessen wird dadurch nichts anderes. **Die rote Rohausgabe bleibt stehen** — ein Kanon, der einmal rot war, ist eine Tatsache über diese Runde und keine Lücke darin. Davor: **Nacharbeit 1: GRUEN 53/53 auf `41a707b`, Arbeitsbaum sauber** — die Beglaubigung des Endstands dieser Runde, Rohausgabe `roh/SONDE-013-41a707b.md`. Der Weg dahin steht in §11.9 und ist Teil des Belegs: der erste Abschlusslauf auf `5fdf36b` kam **ROT** (52/53, A5 `--abdeckung`: `passage_id` ohne Negativfixture, `roh/SONDE-013-5fdf36b-dirty.md`), der zweite auf `8777410` ebenfalls **ROT** (52/53, B10 `kopplungswarte_faelscht_den_backoff_nicht`, eine Zeitannahme im Test unter Kanonlast, `roh/SONDE-013-8777410.md`). Beide Rohausgaben bleiben stehen — ein Kanon, der einmal rot war, ist eine Tatsache ueber diese Runde und keine Luecke darin. Davor: **GRUEN 53/53 auf `f71d741`, Arbeitsbaum sauber** — die Beglaubigung des Etappenendes. Rohausgabe `docs/beweise/roh/SONDE-013-f71d741.md`. Davor derselbe Stand als `9f54129-dirty` (im Baum lagen sechs Dateien mit LEEREM inhaltlichem Diff — reine Zeilenenden aus dem Rotbeweiswerkzeug, §10.10) und **GRUEN 53/53 auf `3203a02`** vor den Nacharbeiten aus dem Selbstaudit. Rohausgaben `docs/beweise/roh/SONDE-013-3203a02.md` und `docs/beweise/roh/SONDE-013-9f54129-dirty.md` (der zweite trägt den `-dirty`-Stempel: im Baum lagen sechs Dateien, deren Diff LEER war — reine Zeilenenden aus dem Rotbeweiswerkzeug, siehe §10.10. Der Wiederholungslauf auf sauberem Baum steht darunter). Der Weg dahin: **GRUEN 53/53 auf `3203a02`** vor den Nacharbeiten, derselbe Lauf auf `9f54129` mit `-dirty`-Stempel, und der saubere Lauf auf `f71d741`. Davor: **GRUEN 51/51 auf `e8c179b`** für C bis G. Rohausgabe `docs/beweise/roh/SONDE-013-e8c179b.md`. Davor: **GRUEN 49/49 auf `e480fe3`** für C bis F. Rohausgabe `docs/beweise/roh/SONDE-013-e480fe3.md`. Davor: **GRUEN 48/48 auf `e3e2299`** für C bis E. Rohausgabe `docs/beweise/roh/SONDE-013-e3e2299.md`. Davor: **GRUEN 46/46 auf `c8b310b`** — die Beglaubigung der Etappen C und D. Rohausgabe `docs/beweise/roh/SONDE-013-c8b310b.md`. Der Weg dahin steht in §10.3 bis §10.5: der Lauf auf `ab0251d` war GRUEN 45/45, aber `-dirty` gestempelt (die zwei Dateien der beginnenden Etappe D lagen schon im Baum); der Lauf auf `e2ea2ec` war der erste auf sauberem Baum und kam **ROT 45/46** zurück — gefallen war **B8** an genau der Produktwirkung, die M-33 und M-34 herstellen, und dass kein anderes Bein fiel, ist die eigentliche Aussage jenes Laufs. Die Beinzahl wächst von 41 auf **53** und sinkt um kein Bein; 2 geplante noch nicht gebaut (B6, B7 ab P6), 1 stillgelegtes (A15). |
| Testanzahl | **Nacharbeit 3 (2026-09-04):** A5 508 Prüfungen über 317 Fixtures · B5 242 · B23 **127** (davor 75) · B16 92 · B3c 75 · B19 57 · B26 **52** (davor 50) · B18 46 · B24 30 · B25 27 · A6 26 · B20 24 · B22 24 · B21 22 · A4-SI 22 · B17 23 · Broker **536** Fälle über alle Beine (davon **A4-verdrahtung 55**, in dieser Runde von 42 auf 55 gewachsen) · JSON-Fixturekorpus **317** (87 gültig, 230 ungültig). Alle Zahlen aus dem Abschlusslauf dieser Runde (`roh/SONDE-013-043e98e.md`), nicht abgeschrieben. Davor: **Nacharbeit 2 (2026-09-04):** A5 504 Prüfungen über 313 Fixtures · B10 313 · B5 242 (davor 237) · B16 92 · B3c 75 · B23 75 · B19 57 · B26 50 · B13 46 (neu: die vier Fälle des Rückwegs) · B18 46 · B24 30 · B25 27 · A6 26 · B20 24 · B22 24 · B17 23 · B21 22 · A4-SI 22 · B15 25 · B14 10 · Broker **501** Fälle über alle Beine (davon **A4-verdrahtung 42**, in dieser Runde von 15 auf 42 gewachsen) · JSON-Fixturekorpus 313 (85 gültig, 228 ungültig). Alle Zahlen aus dem Abschlusslauf dieser Runde, nicht abgeschrieben. Davor: **Nacharbeit 1 (2026-09-04):** A5 489 Prüfungen über 299 Fixtures · B10 313 · B16 82 · B26 50 · B25 27 · Broker 473 Fälle (davon A4-verdrahtung 14 neu, A4-SI zwei neue Produktwegfälle) · JSON-Fixturekorpus 299 (82 gültig, 217 ungültig). Alle Zahlen aus den Läufen dieser Runde, nicht abgeschrieben. Davor: A5 475 Prüfungen · B2 176 · B23 54 (neu) · B24 29 (neu) · A4-prepost 13 (neu) · A4-experiment 18 (neu) · B25 20 (neu) · B26 36 (neu) · A4-taint 14 (neu) · A26 22 Korpusfälle (neu) · A3 61 · B3c 75 · B8 77 · B10 313 · B16 52 · B17 23 (neu) · B18 44 (neu) · B19 57 (neu) · B20 24 (neu) · B21 22 (neu) · B22 24 (neu) · B5 237 · Broker 203 Lib-Tests plus alle Integrationsbeine · JSON-Fixturekorpus 285 (75 gültig, 210 ungültig) · Binärkorpus 104 (20 gültig, 84 ungültig) · Envelope-Korpus 37. Alle Zahlen aus dem Lauf dieser Sitzung, nicht abgeschrieben. |
...
| ID | Ausgangszustand × Ereignis | Zusage samt Reihenfolge und Frist | Test | Quelle / Belegstatus |
|---|---|---|---|---|
| M-13 | Zwei Sonden melden `measurement_position` `pre` beziehungsweise `post` mit derselben `pair_id` | Ein Paar entsteht nur aus **genau einer** PRE- und **genau einer** POST-Hälfte derselben `pair_id` in derselben Session. Reihenfolge: Descriptor validieren → Paarkandidat bilden → Vollständigkeit prüfen → erst dann eine Paaraussage erzeugen. Eine fehlende Hälfte ergibt einen benannten unvollständigen Zustand, nie eine halbe Aussage. Zwei PRE oder zwei POST derselben `pair_id` sind ein Konflikt, kein Zufallssieger. | **gemessen** (2026-09-04, Etappe F): **A4** `sonde013_prepost.rs`::`pairing_requires_exactly_one_pre_and_one_post` und ::`duplicate_half_is_conflict_not_first_wins` — eine fehlende Hälfte ergibt `HaelfteFehlt` ohne jede Zahl, zwei PRE oder zwei POST ergeben `Paarkonflikt` statt eines Zufallssiegers. Rotbeweis `docs/beweise/roh/SONDE-013-rot-M-13.txt`. **Nacharbeit 1** (2026-09-04, B25): `bilde_paare` wird aus dem Evidenzempfaenger gerufen, und der Empfaenger behaelt die Kurven — gemessen: **A4**-verdrahtung::`prepost_join_laeuft_im_produktpfad`, Rotbeweis `roh/SONDE-013-rot-B25-r1.txt`. **Nacharbeit 2** (2026-09-04, R31/R32): der Join gruppiert je SITZUNG, dann je `pair_id`, und das Urteil erreicht Gen ueber `session_snapshot.paare`. Gemessen: **A4**-verdrahtung::`paare_bleiben_in_ihrer_sitzung` und ::`paarurteil_erreicht_gen_ueber_outbox_und_push`, **B13**::`R14_R32_experiment_and_pair_verdict_arrive_in_the_model`; Rotbeweise `roh/SONDE-013-rot-R31-r2.txt` und `-R32-r2.txt`. | `eq-ipc-v3.schema.json`:573-576,613,642; Entwurf §3.4:699-711; §10:930-935. **BELEGT, gemessen im v3-Pfad** (v2-Vorläufer `broker/src/lib.rs`:656) |
| M-14 | Vollständiges Paar; Aussage wird gebildet | Es gibt **drei getrennte Ergebnisse**, nie einen einzigen Differenzwert: rohe Messdifferenz derselben Projektfenster, ausgerichtetes pegelbezogenes Delta nach sicherer Restlag- und Gainschätzung, und interpretierte Wirkung. Die Trennung verhindert, dass mehr Pegel als mehr Höhen oder eine Laufzeitänderung als EQ-Effekt erscheint. | **gemessen** (2026-09-04, Etappe F): **A4**::`three_results_are_never_collapsed` — eine Kette mit +6 dB zeigt roh +6,02 dB, ausgerichtet 0 dB und die Wirkung „breitbandig gleichmäßig"; ohne sicheres Alignment gibt es die rohe Differenz, aber KEIN ausgerichtetes Delta und keine Wirkung. Rotbeweis `docs/beweise/roh/SONDE-013-rot-M-14.txt`. **Nacharbeit 1** (2026-09-04, B25/B28): das Dreifachergebnis entsteht im Produktpfad und traegt Praesenzbits statt NaN — gemessen: **A4**-verdrahtung::`prepost_join_laeuft_im_produktpfad`, **A4**-prepost::`leere_baender_erzeugen_kein_nan`. **Nacharbeit 2** (2026-09-04, R30): EINE Konvention — `haelfte_aus_historie` uebergibt lineare Amplituden, `relation_db` rechnet dB; dazu `capture_s = min(aktiv_s)` aus der Projektspanne, ohne das der Suchraum 0 Frames war und PRE/POST nie einen Lag fand. Gemessen: **A4**-verdrahtung::`prepost_filtert_ausschluss_und_rechnet_in_einer_konvention`, Rotbeweis `roh/SONDE-013-rot-R30-r2.txt`. | Entwurf §38.1:2470-2477. **BELEGT, gemessen** |
...
| M-22 | Eine Hälfte ist getrennt, stale, sammelt noch oder lief ohne Transport | Harte Ausschlüsse führen zu `unclear` mit benanntem Grund, nicht zu einer schwachen Zahl: fehlende Hälfte, getrennte Hälfte, stale Hälfte, Zustand nicht messbereit, fehlende Projektzeit, Fensterlänge ≤ 0. Der v2-Vorläufer führt genau diese Fälle bereits; der v3-Pfad erbt sie samt Wortlautklasse. | **gemessen** (2026-09-04, Etappe F): **A4**::`hard_exclusions_each_name_their_reason` — sieben Ausschlüsse, jeder von BEIDEN Seiten gefahren, jeder mit benanntem Grund und ohne Zahl und ohne Lag. Rotbeweis `docs/beweise/roh/SONDE-013-rot-M-22.txt`. **Nacharbeit 2** (2026-09-04, R33): die Eviction raeumt die Paarurteile der Sitzung UND bildet neu, damit die verbleibende Haelfte mit Grund gefuehrt wird. Gemessen: **A4**-verdrahtung::`eviction_raeumt_das_paarurteil_und_bildet_neu`, Rotbeweis `roh/SONDE-013-rot-R33-r2.txt`. **Nacharbeit 3 (B18/B19, 2026-09-04):** das Paarurteil wird bei JEDER Evidenzaenderung neu gebildet — Append, Invalidierung, Kapazitaets- und Tombstone-Eviction — und beim Neustart aus der persistierten Evidenz gerechnet; gemessen: `sonde013_verdrahtung::invalidierung_bildet_das_paarurteil_neu` und `::paarurteil_entsteht_beim_neustart_aus_der_evidenz`, Rotbeweise `roh/SONDE-013-rot-B18-r3.txt`, `roh/SONDE-013-rot-B19-r3.txt`. | `broker/src/lib.rs`:700-806; Entwurf §58 („ehrliche Herabstufung"). **BELEGT, gemessen im v3-Pfad** |
...
| Rückgabewerte und Zähler werden ausgewertet | **M-63** (`evidence_gesperrt`), **M-75** (`oversize_drop`), **M-05** (Verlustzähler des Ereignisrings). **Nacharbeit 1**: `interventionsRingUeberlauf` wird jetzt WIRKLICH gelesen (B04, M-39), `evidence_beeinflusst` ist ein eigener Zähler (B14), und `p1Uebergeben()` wertet das P1-Ergebnis aus, statt den Ring bedingungslos zu leeren (B06). **Nacharbeit 2**: `nichtEndlicheSamples()` hat einen Leser im Produktpfad (R06), der `ausschlussgrund` wird in Paarbildung UND Resultatmessung wirklich gefiltert (R29), und der Store-Append entscheidet über die Annahme, statt nur gezählt zu werden (R07). Ein gesetzter Zähler ohne Leser ist dasselbe wie kein Zähler. |
...
| Nach dem Schließen wird nichts mehr geliefert | **M-59**: beide Terminalereignisse schließen **alle** Intervalle; **M-48** schließt auch die ohne Terminal. **Nacharbeit 2** (R10): auch die VERDRÄNGUNG bekommt ihr terminales `verdraengt` und schliesst ihre Taintintervalle — vorher schloss der Bestandsdeckel nur intern. |
| Stopp-Fenster ist ein Test, kein Zufall | **M-50**: Neustart mit offenem Versuch, **keine** stillschweigende Fortsetzung. **Nacharbeit 2** (R12): der Test erzeugt einen ZWEITEN Coordinator auf demselben Store. Der Restart-Test der Runde 1 tat das nicht — er konnte deshalb nicht fallen. |
...
| Exakte Feldmenge, `additionalProperties:false` in beiden Sprachen | **M-65** ausdrücklich: `evidence_snapshot` ist heute `additionalProperties: false` — das ist der Grund, warum das Belegen kein additiver Schritt ist. **Nacharbeit 2** (R26): seit dieser Runde läuft **jeder vom Broker erzeugte v3-Payload** durch die Vertragsengine. Vorher schrieb der Broker `von_sample`/`bis_sample` und ein Top-Level `ausgeschlossen` — jeder erzeugte Payload war vertragswidrig, und kein Test sah es. |
| Discriminator vor dem Inhalt geprüft, beide Richtungen | **M-57** (`invalidate_umfang`), **M-67** (fünf neue Diskriminatormengen), **M-13** (`measurement_position`). **Nacharbeit 2** (R28): der Discriminator wird jetzt auch auf der EMPFANGENDEN Seite geprüft — `ganze_sitzung` mit Bereich und ein verdrehter Bereich fallen beide im Plugin-Leser (**B13**). |
...
| Was der Kanon nicht baut, darf er nicht als frisch bezeugen | **M-80**: das Erzeugerskript ist selbst ein Kanonbein mit `--pruefen`. **Nacharbeit 2**: A8 war seit dem V4-Commit rot — die dort von Hand angelegten Fixtures hatten keinen Erzeuger und galten dem Generator als verwaist. Sie haben ihn jetzt. **Ein von Hand geschriebenes Fixture ohne Erzeuger ist ein rotes Bein**, auch wenn beide Vertragsengines es annehmen. |

### E. Behauptung ≤ Messung

| Zeile | Wo in diesem Manifest gemessen |
|---|---|
| Behauptung sagt nicht mehr, als der Test misst | Jede der 86 Matrixzeilen nennt ihren Test; **NEU** markiert, was noch nicht existiert. **M-74** verlangt für „keine Allokation" einen Allokationszähler. **Nacharbeit 2**: das ist die Lehre dieser Runde. Ein Test, der das Modul direkt ruft, misst die Funktion und nicht den Weg — er **kann nicht fallen**. Jeder Fall, der eine Wirkung behauptet, fährt seit dieser Runde gegen einen echten Store (`HarnischMitStore`) und liest sein Ergebnis dort. |
...
| Writer-Fixtures statt Handschrift | **M-80**: der Korpus wird vom Erzeugerskript gebaut und eingefroren; **M-68**: eine Migration-2-Fixture käme vom Writer. **Nacharbeit 2**: die vierzehn neuen Fixtures dieser Runde kommen aus `erzeuge_v3_fixtures.py`, und A8 prüft den Korpus wieder bytegleich zur Neuerzeugung. |

### F. Änderungssatz

| Zeile | Wo in diesem Manifest gemessen |
|---|---|
| speichern↔laden, starten↔stoppen, aktivieren↔abklingen im selben Commit | **M-69** (Save und Load gemeinsam), **M-40** mit **M-47** (begin und abort), **M-37** mit **M-38** (begin und end), **M-34** mit **M-35** (aktivieren und abklingen), **M-51** (anlegen und löschen), **M-68** (Migration mit Vor- und Rückweg). **Nacharbeit 2** (R08): **speichern↔wirken** ist dieselbe Beziehung — Befehlsguard und Domänenereignis liegen in EINEM `store.append`. Vorher committete der Guard vor der Wirkung, und ein Crash dazwischen liess den Retry idempotent quittieren und die Wirkung dauerhaft überspringen. |
...
| Jede Zusage hat einen Rotbeweis, der an einem Testfall fällt | **§11**: 33 Rotbeweise plus einer für den Selbstauditfund, je zwei Läufe in der Rohdatei. Der Fall B19 wurde wiederholt, weil die erste Mutation an einem Übersetzungsfehler fiel statt an einem Test — ein Beweis, der nur zeigt, dass unbalancierte Klammern nicht bauen, ist keiner. |
...
### Z3. Nacharbeit 3 — die Zeilen, die diese Runde neu misst

| Zeile | Wo in diesem Manifest gemessen |
|---|---|
| Dieselbe Regel in ALLEN Sprachen, und der Test dafür existiert beidseitig | **§13.3**, C1: `eq-copilot/fixtures/v3/handschlag-v1.json` ist von Hand geschrieben und wird von **B23** und von **A4**-verdrahtung gelesen. Beide Seiten waren für sich grün — nur ihr Verhältnis war falsch, und ein Bein einer Seite konnte daran nicht fallen. |
| Ein Puffer, der „später wiederholt", hat einen Abflussweg | **§13.6**, B17: `outbox_lesen` bekommt seinen Produktaufrufer; der Re-Subscribe spielt jeden `object_key` in Reihenfolge nach und kompaktiert erst NACH der Zustellung. Eine Zustellschuld ohne Leser ist ein Defekt, kein Zustand. |
| Gekoppelte Lebenszyklen gehören in denselben Änderungssatz | **§13.4**, B14/B16: Befehlsriegel und Rücknahme in EINEM Append; scheitert er, bleibt kein lokaler Ausschluss stehen. **§13.2**, C8: `urteileVersuch` leert den Blindvergleich wie `brichVersuchAb`. |
| Behauptung ≤ Messung — auch im NAMEN einer Zahl | **§13.7**, B11: „Loudness" war das Mittel spektraler P50-Werte und „Peak" das höchste gemittelte Band-P95. Beide Zahlen waren plausibel und beide etwas anderes als ihr Name. Sie sind jetzt `nicht gemessen` mit Grund (§7.1 E-11). |
| Ein Test misst mit der Konstellation, in der die Zusage fallen kann | **§13**: zwei Passagen, zwei Sessions, Seek zwischen Markierung und Worker, genau EIN Resultatsnapshot, injizierter Append-Fehler, Neustart mit OFFENEM Versuch, abgelehnter Push mit Reconnect. Die Runde 2 fuhr überall den störungsfreien Erstaufruf. |
| Jede neue Prüfung wurde einmal absichtlich gebrochen | **§13**: 22 Rotbeweise, jeder mit beiden Läufen in seiner Rohdatei, jeder an einem TESTFALL. |
...
| **E-09** (M-44/M-50) | **Die Blindreihenfolge ist eine eigene Zeile der Kandidatentransition, keine eigene Wirefamilie.** Dirigentenentscheid 2026-09-04 (Nacharbeit 3, Befund B7). Damit gilt Regel V2(c) aus §12 mit **fünf** Zeilenarten: `begin`, `kandidat`, `result`, `abort`, `verdraengt` — nicht mit sechs. | `neuer_kandidat` und `binde_reihenfolge` laufen in EINEM Befehlszweig (`experiment_candidate`) und erzeugen EINE Zeile in `experiment_events`; die Zeile trägt `blindreihenfolge_gebunden` im Zustand. Die Bindung bleibt append-only: der ERSTE Kandidat, der sie trägt, bindet sie, ein zweiter dreht sie nicht (M-44). Der Restore leitet die Transition `ReihenfolgeGebunden` aus derselben Zeile ab — sie ist im Log sichtbar, ohne im Store eine eigene zu brauchen. | A4-verdrahtung `export_traegt_seine_transitionen_auch_nach_dem_neustart` (die Kette enthält Passage, Begin, Kandidat und Reihenfolge). |
...
Thread `01a06b42-0f1c-7500-95d4-713850720677`, `HEAD` `2379ebf`) gab **keine
...
## 12. Nacharbeit 2 nach Wiederprüfung 1 (2026-09-04)

Append-only. Die Wiederprüfung 1 der Etappe 2 (Codex `gpt-5.6-sol`, Effort
**max**, lesend, Thread `01a06c5f-1704-7820-a245-3f02abe1dff8`, `HEAD`
`b69cd55`, Prüfbereich `5e978e7...b69cd55`) gab **keine URTEIL-Zeile** aus; der
Dirigent wertet sie als **NEEDS_WORK** und hat jeden der 33 Befunde an der
Quelle prüfen lassen: **31 Defekte**, **2 Lücken mit entschiedener Regel**, **0
widerlegt**. Auftrag `docs/beweise/roh/SONDE-013-etappe-2-wiederpruefung-1-auftrag.txt`,
Rohurteil `docs/beweise/roh/SONDE-013-etappe-2-wiederpruefung-1-b69cd55.txt`,
Nacharbeitsauftrag `docs/beweise/roh/SONDE-013-etappe-2-nacharbeit-2-auftrag.txt`.

**Der Kernbefund dieser Prüfung ist einer, kein Dutzend.** Die Runde 1 hat
Funktionen gebaut und Tests geschrieben, die das Modul direkt aufrufen. Der
Produktpfad — Nachricht rein → Coordinator → Store → Outbox → Subscriber — war
an vielen Stellen nicht geschlossen, und ein Test, der das Modul direkt ruft,
**kann nicht fallen**: er misst die Funktion, nicht den Weg. Diese Runde
schliesst die Produktpfade und misst sie dort. Der sichtbarste Beleg dafür ist
der neue Harnisch `HarnischMitStore` in `broker/tests/sonde013_verdrahtung.rs`:
mit ihm fährt jeder Fall, der eine Wirkung behauptet, gegen einen **echten
SQLite-Store** und liest sein Ergebnis dort. Ohne Store ist ein
persistenzpflichtiger P0-Befehl `abgelehnt/internal` — die Runde 1 sah ein
`command_ack` und hielt den Weg für gefahren, während die Wirkung nie lief.
...
| Ursachenklasse | Befunde | Commit | Beine |
|---|---|---|---|
| V4 Schema- und Vertragsbruch | R22, R23, R26, R27 | `725ab66` | A5, A8, A4-verdrahtung |
| V7 Autorisierung und Isolation | R09, R31 | `6631c36` | A4-verdrahtung |
| V6 Grenzen und Lebenszyklus | R02, R04, R05, R33 | `da00cc7` | A4-verdrahtung, B16, B23 |
| V1 Produktaufrufer + V2 Persistenz und Replay | R01, R03, R06, R21, R24; R07, R08, R10, R11, R12, R13, R15 | `fb92928` | A4-verdrahtung, A4-crash, B23, B16 |
| V5 Fachliche Rechenwege | R16–R20, R25, R29, R30 | `163b865` | A4-verdrahtung, A4-experiment |
| V3 Outbox und Zustellung | R14, R28, R32 | `14dea8c` | A4-verdrahtung, B13, A5, A8 |

`py -3.13 tools/dirigent/rundenbilanz.py --runden 9876822 14dea8c`:
**Produkt 42 Dateien +4774/−397 · Tests 9 Dateien +2836/−80 · Prüfwerkzeug 2
Dateien +173/−8 · Doku 33 Dateien +3306/−0.** Kein Konvergenz-Signal; jede der
sechs Klassen trägt echten Produktdiff. Zum Vergleich Runde 1 desselben
Tickets, in der drei der vier Commits ebenfalls Produktcode trugen — die Zahl
allein sagt hier nichts, die Prüfung sagte es: gebaut wurde, verdrahtet nicht.

**V1 und V2 liegen bewusst in EINEM Commit.** Sie teilen sich
`experiment_verdrahtung.rs` und `befehl.rs`; jede Trennung hätte einen
Zwischenstand erzeugt, der nicht übersetzt. Ein nicht übersetzender Commit ist
kein kleinerer Änderungssatz, sondern ein unbeweisbarer.

### 12.2 V1 — Der Produktaufrufer fehlt (R01, R03, R06, R21, R24)

Die Ursache ist eine: Runde 1 hat fünf Funktionen gebaut, die im Produkt
niemand ruft. Ein exportiertes Modul ohne Aufrufer ist kein Produktpfad.

| Nr. | Einordnung | Regel und Umsetzung | Test | Rotbeweis |
|---|---|---|---|---|
| R01 | DEFEKT M-61 | Der bestätigte Resync des Control-Links ruft `InterventionsRing::resync()` **und** setzt `interventionsRingUeberlauf` zurück. Vorher blieb das Sticky-Bit für immer stehen: ein einmal übergelaufener Ring meldete `intervention_state_unknown` bis zum Prozessende, und keine starke Evidenz kam je wieder durch. | **B23**::`R01 ueberlauf_sticky_wird_beim_bestaetigten_resync_geloescht` — gemessen am neu aufgebauten Control-Link, nicht am Ring | `roh/SONDE-013-rot-R01-r2.txt` |
| R03 | DEFEKT G4 | `FeatureEngine::setzePassagenfenster(start, ende, transportEpoche)` bekommt seinen Aufrufer: `merkeManuellePassage`/`vergissManuellePassage` legen den Wunsch in Atomics, der **Analyseworker** wendet ihn an. Kein Audio-Thread-Zugriff. Die Transportepoche steht in der Signatur — ein Fenster ohne sie wäre an keine Zeitachse gebunden. | **B23**::`R03 passagenfenster_erreicht_die_engine` — vom `PluginProcessor`-Aufruf bis zum in der Engine geführten Fenster | `roh/SONDE-013-rot-R03-r2.txt` |
| R06 | DEFEKT G2 | `Vergleichspegel` und `Blindvergleich` sind in den Experimentpfad des Plugins eingebunden; `nichtEndlicheSamples()` hat einen Leser, und die Zahl reist in den Wirezustand. Vorher stand die NaN-Ehrlichkeit als Funktion da, die niemand aufrief — M-07 war gebaut und unwirksam. | **B23**::`R06 vergleichspegel_und_blindvergleich_im_produktpfad` — mit dem Gegenbeispiel: sauberes Material zählt nachweislich KEIN nichtendliches Sample | `roh/SONDE-013-rot-R06-r2.txt` |
| R21 | DEFEKT M-44 | Die Blindreihenfolge wird in einem **eigenen Befehlszweig** gebunden, bevor jemand hört: die neue Familie `experiment_candidate` (Fassung 2) erfasst den Kandidaten und bindet die Reihenfolge append-only. Vorher band sie erst das Hörurteil — die Reihenfolge liess sich nachträglich zum Urteil passend erzählen. Der Fehlercode `reihenfolge_nicht_gebunden` benennt den Gegenfall. | **A4**-verdrahtung::`blindreihenfolge_wird_im_eigenen_befehlszweig_gebunden`; **A4**-crash::`produkt_coordinator_ackt_keine_wirkung_die_nicht_stattfindet` — der Sender erfährt `reihenfolge_nicht_gebunden` statt eines generischen `internal` | `roh/SONDE-013-rot-R21-r2.txt` |
| R24 | DEFEKT M-52…M-57 | Material- und Messpunktwechsel invalidieren **aus dem Descriptor- und Fingerprintvergleich heraus**, und `preview_begin`/`_renew`/`_end` invalidieren neben `persistenz_p0`. Vorher hatten `invalidierung_wegen_material` und `_messpunkt` keinen Aufrufer, und eine Vorschau färbte die Messung, ohne sie zurückzunehmen. | **A4**-verdrahtung::`preview_nimmt_die_evidenz_ihrer_sitzung_zurueck`, ::`messpunktwechsel_invalidiert_aus_dem_deskriptorvergleich`, ::`materialwechsel_invalidiert_aus_dem_fingerprintvergleich` | `roh/SONDE-013-rot-R24-r2.txt` |

### 12.3 V2 — Persistenz und Replay (R07, R08, R10–R13, R15)

| Nr. | Einordnung | Regel und Umsetzung | Test | Rotbeweis |
|---|---|---|---|---|
| R07 | DEFEKT G1(c) | Annahme und Store-Append sind **eine Einheit**: scheitert der Append, wird der Snapshot zurückgerollt (über seine `evidence_id`), abgelehnt und gezählt. Vorher galt er als angenommen, während er nirgends stand — nach einem Neustart war er fort, und niemand wusste davon. | **A4**-verdrahtung::`evidenz_ohne_erfolgreiche_ablage_ist_nicht_angenommen` | `roh/SONDE-013-rot-R07-r2.txt` |
| R08 | DEFEKT speichern↔wirken | Befehlsguard und Domänenereignis liegen in **einem** `store.append` (`persistenz_p0_mit_domaene`). Vorher committete der Guard vor der Wirkung: ein Crash dazwischen liess den Retry idempotent quittieren und die Wirkung **dauerhaft** überspringen. | **A4**-verdrahtung::`experimentbefehl_und_wirkung_sind_ein_append` — der Test liest nach dem Append im Store nach, nicht im Coordinator | `roh/SONDE-013-rot-R08-r2.txt` |
| R10 | DEFEKT M-59 | `Experimentstore::beginne` gibt die verdrängten IDs zurück; der Wrapper persistiert je Verdrängung das terminale `verdraengt` **und** schliesst dessen Taintintervalle. Vorher schloss der Bestandsdeckel intern, und der verdrängte Versuch liess seine Intervalle offen zurück. | **A4**-verdrahtung::`verdraengung_bekommt_ihr_eigenes_terminal` | `roh/SONDE-013-rot-R10-r2.txt` |
| R11 | DEFEKT M-40 | Der Begin-Payload trägt die vollständigen §43.1-Referenzen: Passage mit Fingerprint und Messpunktklassen, Upstream-Fingerprint, Alignment, Reproduzierbarkeit, Match-Gain. Ein Versuch ohne sie wäre später nicht mehr derselbe Versuch. | **A4**-verdrahtung::`experimenttransitionen_stehen_im_ereignisindex_mit_vollen_referenzen` | `roh/SONDE-013-rot-R11-r2.txt` |
| R12 | DEFEKT M-50 | `Coordinator::mit_store` liest die Projektionen `passages`, `experiments` und `evidence` und **rekonstruiert** daraus Experimentstore und Evidenzhistorie. (`experiment_events` ist der append-only Ereignisindex aus R13 und wird dafür nicht gebraucht — die Projektion trägt den gerechneten Zustand, der Index die Kette.) Vorher starteten beide leer; ein Brokerneustart löschte jeden offenen Versuch, ohne ihn zu beenden. Der Test erzeugt einen **zweiten** Coordinator auf demselben Store — der Restart-Test der Runde 1 tat das nicht und konnte deshalb nicht fallen. | **A4**-verdrahtung::`terminal_traegt_die_nutzerdaten_und_ueberdauert_den_neustart` | `roh/SONDE-013-rot-R12-r2.txt` |
| R13 | DEFEKT G1(a) | `experiment_events` bekommt seinen Writer: jede Transition steht als eigene Zeile im Ereignisindex — `begonnen`, `kandidat`, `ergebnis`, `abgebrochen`, `verdraengt`. Eine sechste für die Blindreihenfolge gibt es bewusst nicht: sie wird MIT dem Kandidaten gebunden (R21), und eine eigene Zeile für denselben Vorgang wäre eine zweite Wahrheit. Vorher lief alles als `session_snapshot` mit `event_type="experiment"` — der Index blieb leer. Migration 1 bleibt eingefroren; die Tabelle wird aus der bestehenden `experiments`-Projektion gefüllt. | **A4**-verdrahtung::`experimenttransitionen_stehen_im_ereignisindex_mit_vollen_referenzen` | `roh/SONDE-013-rot-R13-r2.txt` |
| R15 | DEFEKT M-49 | Der Terminal-Payload trägt `hoerurteil`, `blindreihenfolge`, `notiz`, `werkzeug` und `execution_mode` und **ersetzt den Begin-Zustand nicht**. Vorher überschrieb das Terminal ihn: die Referenzen des Begins waren nach dem Urteil fort. | **A4**-verdrahtung::`terminal_traegt_die_nutzerdaten_und_ueberdauert_den_neustart` | `roh/SONDE-013-rot-R15-r2.txt` |

### 12.4 V3 — Outbox und Zustellung (R14, R28, R32)

Die Regel dieser Klasse: **Nachricht rein → Outbox-Eintrag → Push → im
Plugin-Modell angekommen.** Eine Outbox-Schuld ohne Leser ist ein Defekt, kein
Zustand. Die ersten drei Glieder misst **A4**-verdrahtung im Broker, das vierte
**B13** auf der C++-Seite — gegen dasselbe Korpusfixture, nicht gegen einen im
Test erfundenen Text.

| Nr. | Einordnung | Regel und Umsetzung | Test | Rotbeweis |
|---|---|---|---|---|
| R14 | DEFEKT M-49 Zustellung | Der `session_snapshot` trägt `experimente` (`$defs/session_experiment`, Fassung 2); das Terminal macht seine Sitzung dirty und flusht sie. Vorher bekam der Terminal-Event **gar keine** `snapshot_ziele`, und der Snapshot besass kein Experimentfeld: ein gerechnetes Resultat erreichte Gen nie. Der Rückweg läuft über den bestehenden Outbox-/Snapshot-Pfad statt über eine weitere Familie — genau das sagt die Beschreibung von `experiment_manual_result` im Vertrag zu (§53.9). | **A4**-verdrahtung::`versuchsergebnis_erreicht_gen_ueber_outbox_und_push` (Push-Payload, Urteil, Reihenfolge, danach Outbox leer); **B13**::`R14_R32_experiment_and_pair_verdict_arrive_in_the_model` | `roh/SONDE-013-rot-R14-r2.txt` |
| R28 | DEFEKT G1(d) | Die Invalidierung wird über `SessionPush::snapshot_schreiben` **wirklich zugestellt** und danach kompaktiert; `SourcesModel::uebernehmeEvidenzruecknahme` ist ihr Leser in `EqCopilotProcessor::v3Antwort`. Vorher legte der Broker nur eine Outbox-Schuld an, die niemand las, und die Pluginseite kannte die Familie überhaupt nicht. Der Leser hält die geschlossene Gründemenge, wählt ihre Fassung aus dem Wire-Envelope, prüft den über `art` diskriminierten Umfang samt Bereichsordnung und wirkt fail-closed auf die Messachse. | **A4**-verdrahtung::`invalidierung_wird_dem_abonnenten_wirklich_zugestellt` (beide Umfänge: `ganze_sitzung` und `sample_range`); **B13**::`R28_evidence_invalidate_has_a_reader_and_an_effect`, ::`invalidate_reason_version_is_chosen_by_the_wire_envelope`, ::`invalidate_scope_discriminator_is_not_a_label` | `roh/SONDE-013-rot-R28-r2.txt` |
| R32 | DEFEKT G1(e) | Der `session_snapshot` trägt `paare` (`$defs/session_paar`). Vorher endete das Paarurteil in einer flüchtigen Map: weder `StoreEvent` noch Dirty-Snapshot noch Outboxziel. Ein eigener Flush steht bewusst **nicht** im Join — sein einziger Aufrufer ruft unmittelbar danach `heartbeat_kontakt` mitsamt Flush derselben Sitzung; ein zweiter wäre ein zweites Lock je Evidenzsnapshot auf dem heissen Empfangspfad, den kein Test von diesem unterscheiden könnte. | **A4**-verdrahtung::`paarurteil_erreicht_gen_ueber_outbox_und_push`; **B13**::`R14_R32_experiment_and_pair_verdict_arrive_in_the_model` | `roh/SONDE-013-rot-R32-r2.txt` |

### 12.5 V4 — Schema- und Vertragsbruch (R22, R23, R26, R27)

| Nr. | Einordnung | Regel und Umsetzung | Test | Rotbeweis |
|---|---|---|---|---|
| R22 | DEFEKT M-59 | `audible_intervention_begin` trägt innerhalb der Fassung 2 ein optionales `experiment_id`; `Coordinator::intervention_begin` reicht `art` und `experiment_id` durch. Dass es bei `art=experiment` Pflicht ist, **gehört dem Consumer**: die Keyword-Menge des v3-Vertrags ist geschlossen und kennt kein `if`/`then` — die Regel steht ausdrücklich in `schemas/v3/README.md`, nach demselben Muster wie `sample_start <= sample_end` bei `invalidate_bereich`. | **A4**-verdrahtung::`wire_intervention_traegt_art_und_experiment_bis_zum_terminal`, ::`wire_experimentintervall_ohne_id_wird_abgelehnt`, ::`wire_hoermarkierung_wird_von_keinem_experimentterminal_geschlossen` | `roh/SONDE-013-rot-R22-r2.txt` |
| R23 | DEFEKT G6 | `Passage` bekommt `messpunktklassen` als Feld parallel zu `aktive_quellen`; `passage_aus_wert` liest es. Vorher stand die Klasse im Wire und wurde verworfen — der Messpunktvergleich aus M-28/M-55 lief auf leeren Listen. | **A4**-verdrahtung::`passage_aus_dem_wire_traegt_ihre_messpunktklassen` | `roh/SONDE-013-rot-R23-r2.txt` |
| R26 | DEFEKT M-57 | `evidence_invalidate` benutzt `sample_start`/`sample_end` und trägt **kein** Top-Level `ausgeschlossen`. Vorher schrieb der Broker `von_sample`/`bis_sample` und ein zusätzliches Feld gegen `additionalProperties: false` — jeder erzeugte Payload war vertragswidrig, und kein Test validierte ihn. Seit dieser Runde läuft **jeder vom Broker erzeugte v3-Payload** durch die Vertragsengine. | **A4**-verdrahtung::`invalidierung_aus_dem_produktpfad_haelt_den_vertrag`, ::`invalidierung_serialisiert_ihre_drei_umfaenge`; **A5** über den Korpus | `roh/SONDE-013-rot-R26-r2.txt` |
| R27 | DEFEKT | Invalidierungs-, Experiment- und Paarereignisse haben **eigene** `StoreEvent`-Typen mit eigener Projektion; `sessions.state_jcs` bleibt ausschliesslich die `session_snapshot`-Projektion. Vorher ersetzte der Writer sie durch den Invalidierungspayload: beim nächsten Subscribe erwartete `subscription.rs` dort einen Sessionsnapshot, verwarf die Projektion und setzte das Routing fail-closed — eine Rücknahme von Evidenz nahm die ganze Sitzung mit. Die Invalidierungstests der Runde 1 liefen ohne Store und konnten das nicht auslösen. | **A4**-verdrahtung::`invalidierung_laesst_die_sessionprojektion_stehen`, ::`invalidierung_projiziert_ihren_grund_in_die_evidenzzeilen` | `roh/SONDE-013-rot-R27-r2.txt` |

### 12.6 V5 — Fachliche Rechenwege (R16–R20, R25, R29, R30)

| Nr. | Einordnung | Regel und Umsetzung | Test | Rotbeweis |
|---|---|---|---|---|
| R16 | LÜCKE, entschieden | **Mindestens ein nach dem Begin erfasster Kandidat ist Pflicht.** `hat_resultat()` prüft ihn; ohne Kandidat wird das Terminal mit `ohne_resultatmessung` abgelehnt, und die Abschlusswache des Stores trägt dafür ihren eigenen `Abschlussfehler::OhneKandidat`. Ohne Kandidat gibt es nichts, wogegen die Baseline stünde. Der Test der Runde 1 schrieb den Erfolg ohne Kandidat fest — er ist so umgeschrieben, dass er ohne Kandidat fällt. Erfasst wird der Kandidat über die neue Familie `experiment_candidate` (siehe R21). | **A4**-verdrahtung::`ergebnis_ohne_resultatmessung_wird_abgelehnt` | `roh/SONDE-013-rot-R16-r2.txt` |
| R17 | DEFEKT M-49 | `resultatmessung(experiment_id, session)` arbeitet ausschliesslich mit Evidenz **derselben Passage** (Fingerprintvergleich M-31), **derselben Quellen und Messpunktklassen**, **ab der Begin-Grenze** und **ohne Ausschlussgrund**: Baseline ist Evidenz vor dem Begin innerhalb der Passage, Resultat Evidenz nach dem erfassten Kandidaten. Vorher nahm sie die längste Historie **irgendeiner** Quelle und halbierte die Retention stumpf — vier vor dem Begin eingegangene Snapshots genügten für ein sofortiges Resultat. | **A4**-verdrahtung::`resultatmessung_ist_an_versuch_passage_und_grenze_gebunden` — je Ausschlussgrund einzeln geprüft | `roh/SONDE-013-rot-R17-r2.txt` |
| R18 | DEFEKT M-46 | `vergleichbarkeit::beurteile` läuft im Produktpfad **vor** dem Terminal; ohne Vergleichbarkeit gibt es kein Terminal mit Siegerurteil. Vorher blieb das Feld `Default::default()` — Gate 6 aus §49.2 war gebaut und wirkungslos. | **A4**-verdrahtung::`resultatmessung_ist_an_versuch_passage_und_grenze_gebunden` (R18-Zweig) | `roh/SONDE-013-rot-R18-r2.txt` |
| R19 | DEFEKT M-45 | Die Guardrails umfassen **Loudness, True Peak, Transient, Breite und geschützte Bereiche** zusätzlich zu Coverage und Klasse. Vorher waren es nur Coverage und Klasse: ein 6 dB lauteres Resultat sah bei unveränderter Coverage „stabil" aus. | **A4**-verdrahtung::`resultatmessung_ist_an_versuch_passage_und_grenze_gebunden` (R19-Zweig, die Achse FÄLLT) | `roh/SONDE-013-rot-R19-r2.txt` |
| R20 | DEFEKT M-45 | Der Block-Bootstrap zieht **zeitlich benachbarte Analysefenster je Band**, nie Bänder; die p-Werte kommen aus dem Vergleich mit der Bootstrapverteilung, nicht aus `exp(-\|d\|)`. Vorher war der p-Wert eine Formel ohne Stichprobe. | **A4**-verdrahtung::`bootstrap_zieht_zeitfenster_und_rechnet_echte_p_werte` — mit einer **kleinen, aber stabilen** Reihe (0,4 dB), die die alte Formel nie signifikant nennen konnte | `roh/SONDE-013-rot-R20-r2.txt` |
| R25 | DEFEKT M-52 | Der Begin-Stempel wird gespeichert; der invalidierte Bereich ist exakt Begin…Ende. Fehlt `project_sample_end`, wird **fail-closed die ganze Sitzung** invalidiert. Vorher begann der Bereich bei `i64::MIN/2`, und ohne Endstempel geschah gar nichts — der Marker hatte gefärbt, und niemand nahm es zurück. | **A4**-verdrahtung::`markerinvalidierung_trifft_genau_ihren_bereich`, ::`markerinvalidierung_ohne_projektzeit_ist_fail_closed` | `roh/SONDE-013-rot-R25-r2.txt` |
| R29 | DEFEKT M-52 | `ausschlussgrund` wird in `haelfte_aus_historie` **und** in `resultatmessung` gefiltert. Vorher wurde er gesetzt und von keiner Auswertung gelesen: zurückgenommene Evidenz bestimmte weiter Paarurteil und Experimentresultat — die Rücknahme war ein Etikett ohne Wirkung. | **A4**-verdrahtung::`prepost_filtert_ausschluss_und_rechnet_in_einer_konvention` | `roh/SONDE-013-rot-R29-r2.txt` |
| R30 | DEFEKT M-14 | **Eine Konvention:** `haelfte_aus_historie` übergibt lineare Amplituden (`10^(dB/20)`), und `relation_db` rechnet daraus dB. Vorher gingen dB-Werte als lineare Hüllkurven hinein, und die Relation war eine Zahl ohne Einheit. Derselben Einheitenfamilie entstammte der Fund während der Nacharbeit: `capture_s` stand auf `envelope_frames / audio_sample_rate` (≈0,00025 s), der Suchraum war 0 Frames, und PRE/POST konnte **nie** einen Lag finden — jetzt ist es `min(pre.aktiv_s, post.aktiv_s)` aus der Projektspanne. | **A4**-verdrahtung::`prepost_filtert_ausschluss_und_rechnet_in_einer_konvention` — mit einem realen dB-Wert | `roh/SONDE-013-rot-R30-r2.txt` |

### 12.7 V6 — Grenzen und Lebenszyklus (R02, R04, R05, R33)

| Nr. | Einordnung | Regel und Umsetzung | Test | Rotbeweis |
|---|---|---|---|---|
| R02 | DEFEKT M-58 | Der Nachlauf wird **aus der Uhr** abgeleitet (verstrichene Zeit × Samplerate); die Konstante `TAIL_SCHRITT_JE_TICK` entfällt. Sie war für 1 s gerechnet, der Tick läuft alle 100 ms: der Tail wurde **fünfmal zu schnell** frei, und starke Evidenz kam wieder durch, während der Eingriff noch nachklang. | **A4**-verdrahtung::`nachlauf_laeuft_in_echtzeit_ab` — der Test fällt bei 5× zu schnell | `roh/SONDE-013-rot-R02-r2.txt` |
| R04 | LÜCKE, entschieden | `setzePassagenfenster` leert **auch** `kurzZellen`, `kurzTpZellen`, `kurzRmsZellen`, `kurzStand`, `kurzGefuellt` und die laufende Zelle. Vorher enthielten die ersten LRA-, PSR- und Crest-Fenster einer mitten im Stück markierten Passage bis zu zwei Sekunden Audio **vor** ihr. | **B16**, Abschnitt „N2 – R04: das Kurzzeitfenster fällt mit dem Passagenanfang" | `roh/SONDE-013-rot-R04-r2.txt` |
| R05 | DEFEKT B09-Regel | `tp.nachlauf()` läuft **innerhalb** der Sampleschleife am Index `passBis`, bevor Post-Material verarbeitet wird. Vorher lief er nach der Schleife: endete die Passage mindestens zwölf Samples vor dem Blockende, wurden die verzögerten Ausgänge der letzten Passagensamples mit `imPassagenfenster = false` verbraucht und dem Passagenmaximum nicht zugeschlagen. | **B16**, Abschnitt „N3 – R05: der Nachlauf läuft am Index `passBis`" — Impuls in den letzten zwölf Samples einer mid-block endenden Passage | `roh/SONDE-013-rot-R05-r2.txt` |
| R33 | DEFEKT M-22 | `client_eviktieren_locked` entfernt die Paarurteile der Sitzung **und** stösst `evidenz_paare_bilden` an, damit die verbleibende Hälfte als getrennte Hälfte **mit Grund** geführt wird. Vorher blieb ein Urteil aus zwei Hälften stehen, von denen eine gar nicht mehr da war. | **A4**-verdrahtung::`eviction_raeumt_das_paarurteil_und_bildet_neu` | `roh/SONDE-013-rot-R33-r2.txt` |

### 12.8 V7 — Autorisierung und Isolation (R09, R31)

| Nr. | Einordnung | Regel und Umsetzung | Test | Rotbeweis |
|---|---|---|---|---|
| R09 | DEFEKT E-03/M-48 Isolation | `experiment_p0_vorpruefung` vergleicht `e.projektbindung` mit der Projektbindung der **Zielsession**; ein fremdes Projekt wird mit Konflikt abgelehnt. Vorher suchte die Vorprüfung global über die `experiment_id`: ein Main konnte den Versuch eines anderen Projekts beenden. | **A4**-verdrahtung::`experimentbefehl_bleibt_in_seinem_projekt` — zwei Projekte | `roh/SONDE-013-rot-R09-r2.txt` |
| R31 | DEFEKT M-13 | `evidenz_paare_bilden` gruppiert **je Sitzung**, dann je `pair_id`. Die `pair_id` kommt aus dem Deskriptor des Users und ist keine globale Kennung: vorher bildeten eine PRE-Hälfte aus Sitzung A und eine POST-Hälfte aus Sitzung B ein gemeinsames Paar. | **A4**-verdrahtung::`paare_bleiben_in_ihrer_sitzung` — zwei Sitzungen mit derselben `pair_id` | `roh/SONDE-013-rot-R31-r2.txt` |

### 12.9 Was diese Runde ausserdem gefunden hat

Fünf Funde, die kein Codex-Befund waren, aber in denselben Änderungssatz
gehören — sie sind beim Schliessen der Produktpfade sichtbar geworden.

1. **PRE/POST konnte nie einen Lag finden.** `capture_s` stand auf
   `envelope_frames / audio_sample_rate` — rund 0,00025 s. Der Suchraum war
   damit **0 Frames**, und die Restlag-Schätzung lief leer. Der Fund gehört zu
   R30: dieselbe Einheitenverwechslung, ein Feld weiter.
2. **`session_snapshot.experimente` hat `maxItems: 32`, der Bestandsdeckel
   M-48 deckelt aber nur die OFFENEN.** Abgeschlossene sammeln sich in
   derselben Ablage; eine lange Sitzung hätte einen vertragswidrigen Snapshot
   erzeugt, den der Leser **ganz** verwirft — der Rückweg wäre genau dann
   gerissen, wenn er am meisten trägt. Die Liste wird jetzt am **alten** Ende
   auf `N_GLOBAL` gekappt, die Anlegereihenfolge bleibt stehen. Gemessen:
   **A4**-verdrahtung::`der_snapshot_haelt_den_versuchsdeckel_des_vertrages`.
3. **A8 war seit dem V4-Commit rot.** Die dort von Hand angelegten Fixtures
   (`experiment_candidate`, `audible-intervention-begin-experiment` und ihre
   Negativzwillinge) hatten keinen Erzeuger in `erzeuge_v3_fixtures.py`; der
   Generator meldete sie als verwaist. Sie haben ihn jetzt, und der Korpus ist
   wieder bytegleich zur Neuerzeugung. **Ein von Hand geschriebenes Fixture
   ohne Erzeuger ist ein rotes Bein**, auch wenn beide Vertragsengines es
   annehmen.
4. **Der Abschlusskanon fiel an einem STACK OVERFLOW.** `EqCopAnalysisGoldenTest`
   (B5) starb mit `0xC00000FD` in `zwillingsprobe()`. Die Funktion legt ZWEI
   `FeatureEngine`-Objekte in ihren Rahmen, jedes rund ein halbes Megabyte, und
   sie wird aus `main` gerufen, dessen Rahmen schon mehrere davon trägt: das
   neue Feld `Passagenfenster::epoche` (R03) kippte den 1-MB-Vorgaberahmen von
   MSVC. Beide Engines liegen jetzt auf dem Heap — dieselben zwei Objekte,
   dieselben Aufrufe, nur nicht im Rahmen. Die Lehre gehört zu den
   Maschinen-Landminen: **ein Feld im falschen Objekt ist eine
   Übersetzungseinheit weiter ein Absturz**, und nur ein VOLLER Kanon findet
   ihn — die betroffenen Beine allein liefen grün.
5. **Drei Testhelfer der Runde 1 waren still falsch.** `capabilities()` meldete
   `cycle_derivation` (keine Vertrags-Capability) statt `remote_control`; ein
   Main mit `runtime`-Block im Heartbeat liess den Broker einen
   `probe_descriptor` mit `plugin_kind: "main"` ableiten — vertragswidrig; und
   `state_report` brauchte `dsp_schema_version`. Sichtbar wurden sie erst, als
   die Fälle gegen einen **echten Store** fuhren: ohne ihn quittierte jeder
   persistenzpflichtige Befehl `abgelehnt/internal`, und der Test hielt das für
   den Weg.

## 13. Nacharbeit 3 nach Wiederprüfung 2 (2026-09-04)

Append-only. Die Wiederprüfung 2 der Etappe 2 (Codex `gpt-5.6-sol`, Effort
**max**, lesend, Thread `01a06d88-46be-7d41-b9b7-9745128f6c44`, `HEAD` vor und
nach dem Lauf `eb115af`, Prüfbereich `9876822...eb115af`) gab **keine
URTEIL-Zeile** aus; der Dirigent wertet sie als **NEEDS_WORK** und hat jeden
der 28 Befunde an der Quelle prüfen lassen: **21 Defekte**, **1 Lücke mit
entschiedener Regel** (B3), **3 Lücken fürs Register** (B2, B12, C9), **1
Härtung** (B8), **1 widerlegt** (B15) und **1 durch Regelanpassung geschlossen**
(B7). Auftrag `docs/beweise/roh/SONDE-013-etappe-2-wiederpruefung-2-auftrag.txt`,
Rohurteil `docs/beweise/roh/SONDE-013-etappe-2-wiederpruefung-2-eb115af.txt`,
Nacharbeitsauftrag `docs/beweise/roh/SONDE-013-etappe-2-nacharbeit-3-auftrag.txt`.

**Der Kernbefund dieser Prüfung.** Die Runde 2 hat die Produktpfade verdrahtet,
aber an drei Stellen zu grob:

* **im Plugin** waren Passagenfenster und Vergleichspegel ein einzelner
  globaler, unsynchronisierter Slot ohne Passagen-, Zeit- und Epochenbindung;
* **im Broker** lagen Befehl und Wirkung an drei Stellen weiter in getrennten
  Appends, Rollbacks griffen zu weit oder zu kurz, und Restore, Zähler und ein
  Off-by-one verloren Evidenz;
* **Paarurteile** wurden nur bei NEUER Evidenz gebildet, nie bei Entzug,
  Eviction oder Restore.

Die Tests der Runde 2 fuhren jeweils den störungsfreien Erstaufruf mit einer
Passage, einer Session und mehreren Snapshots — genau die Konstellationen, in
denen der Fehler nicht sichtbar wird. Diese Runde misst mit den
Konstellationen, die fallen können: **zwei Passagen, zwei Sessions, Seek
zwischen Markierung und Worker, genau EIN Resultatsnapshot, injizierter
Append-Fehler, Neustart mit OFFENEM Versuch, Push-Fehler mit Reconnect.**

Je Befund steht unten: die umgesetzte Regel, der Commit, der messende Test und
der Rotbeweis. **Jeder Rotbeweis fällt an einem TESTFALL**, nicht an einem
Übersetzungsfehler — beide Läufe stehen in der Rohdatei.

### 13.1 Rundenbilanz

`py -3.13 tools/dirigent/rundenbilanz.py --runden 8c02520 899df16 71194a7 a3654f5 562c4cc 14e0d5c`:

| Klasse | Commit | Produkt | Tests |
|---|---|---|---|
| W1+W2 Plugin | `899df16` | 13 Dateien +2895/−1924 | 5 Dateien +771/−28 |
| W3 Atomarität | `71194a7` | 8 Dateien +594/−210 | 1 Datei +356/−2 |
| W4 Replay | `a3654f5` | 5 Dateien +258/−8 | 1 Datei +286/−0 |
| W5 Zustellung | `562c4cc` | 2 Dateien +96/−1 | 1 Datei +108/−0 |
| W6 Rechenwege | `14e0d5c` | 9 Dateien +278/−93 | 2 Dateien +301/−11 |

Über die ganze Runde `8c02520..14e0d5c`: **Produkt 25 Dateien +4120/−2235 ·
Tests 5 Dateien +1822/−41 · Prüfwerkzeug 1 Datei +26/−0 · Doku 22 Dateien
+3834/−0**. **Kein Konvergenz-Signal** — jede der fünf Klassen trägt echten
Produktdiff. Die Zahl allein trägt hier keine Aussage; was diese Runde von der
vorigen unterscheidet, ist die **Konstellation** der Messung, nicht ihre Menge.
Budget: **drei von drei** Nacharbeitsrunden verbraucht.

### 13.2 W1 — Passagenbindung und Nebenläufigkeit im Plugin (C2, C3, C4, C6, C7)

Commit `899df16` (gemeinsam mit W2, siehe §13.3).

| Befund | Regel und was gebaut wurde | Gemessen | Rotbeweis |
|---|---|---|---|
| C2 | Der Fensterwunsch trägt die beim MARKIEREN gelesene Transportepoche (`passagenfensterEpocheWunsch`); der Worker vergleicht gegen sie, nicht gegen `transportEpocheJetzt()`. Lehnt die Engine ab, bleibt KEIN Fenster stehen. | B23 `c2Markierungsepoche` — Seek erzeugt eine neue Epoche, der Wunsch mit der alten wird verworfen, der mit der neuen angenommen | `roh/SONDE-013-rot-C2-r3.txt` |
| C3 | Das Fenster gehört einer `passageId`. `vergissManuellePassage(id)` löst nur die eigene Bindung; `beginneVersuch(id)` hängt das Fenster auf DIESE Passage um und lehnt den Versuch ab, solange sie keinen eigenen Pegel hat. Eine Generation entwertet eine überholte Veröffentlichung des Workers. | B23 `c3ZweiPassagen` — zwei Passagen, `vergiss(A)` lässt B stehen, `beginne(A)` bindet A | `roh/SONDE-013-rot-C3-r3.txt` |
| C4 | Der Vergleichspegel nimmt nur den Blockausschnitt auf, dessen Projektzeit in `[start, ende)` der gebundenen Passage liegt — samplegenau —, und nur solange der Analyseworker das Fenster als intakt veröffentlicht (`pegelFensterAktiv`). | B23 `c4PegelNurImFenster` — 250 Blöcke ausserhalb erreichen die 400-ms-Schwelle nicht, Material darin schon | `roh/SONDE-013-rot-C4-r3.txt` |
| C6 | `Vergleichspegel` trägt den Ausschluss im TYP: ein CAS-Tor, das der Audiothread nur VERSUCHT (kein Warten, keine Allokation, kein Logging) und der Nachrichtenthread hält. Der Leser sieht nie einen halb aufgenommenen Block. | B26 `c6UebergabeAnDerBlockgrenze` — 13 686 Proben, 0 Risse; ohne das Tor 12,9 Mio. von 14,9 Mio. | `roh/SONDE-013-rot-C6-r3.txt` |
| C7 | Fingerprint, Passagenepoche und Frame werden in EINEM `externerAnalyseSteuerZug` gelesen (`engineabzugLesen`); `beginneVersuch` und `erfasseKandidat` nehmen ihn wie `passagenfensterInEngine`. | B23 `c7EngineZug` — der gezählte Steuerzug steigt um beide Aufrufe | `roh/SONDE-013-rot-C7-r3.txt` |

### 13.3 W2 — Wirezustand und Handschlag im Plugin (C1, C5, C8)

Commit `899df16`. **W1 und W2 liegen bewusst in EINEM Commit**: beide Klassen
ändern `PluginProcessor.cpp/.h` und `Sonde013PassageStateTest.cpp`, und jede
Trennung erzeugte einen nicht übersetzenden Zwischenstand — dieselbe
Begründung wie in §12 für V1+V2.

| Befund | Regel und was gebaut wurde | Gemessen | Rotbeweis |
|---|---|---|---|
| C1 | Nach jedem bestätigten `neutral_resync` sendet das Plugin die erste Intervention mit Sequenz **1**: der Zähler fällt beim Resync auf 0 zurück, und das Inkrement steht VOR dem Senden. Die Brokerseite bleibt unverändert (Basis 0 = „nächste ist 1"). | B23 `c1Sequenzhandschlag` **und** A4-verdrahtung `erste_intervention_nach_resync_wird_angenommen` — beide gegen `eq-copilot/fixtures/v3/handschlag-v1.json` | `roh/SONDE-013-rot-C1-r3.txt` (beide Hälften) |
| C5 | `nicht_endliche_samples` ist ein optionales Feld (`integer ≥ 0`) im Referenzobjekt der Fassung 2: Schema, Erzeuger, ein Positiv- und zwei Negativfixtures, A5-Korpus und Brokerleser im selben Änderungssatz. `None` heisst „der Sender kennt das Feld nicht", ausdrücklich nicht `Some(0)`. | B23 `c5ZaehlerImWirezustand` — geprüft wird das GESENDETE JSON, nicht der Getter | `roh/SONDE-013-rot-C5-r3.txt` |
| C8 | `urteileVersuch` ruft am Ende `blindvergleich.loeschen()` wie `brichVersuchAb`. | B23 `c8ZweiVersucheNacheinander` — zwei Versuche nacheinander beurteilt | `roh/SONDE-013-rot-C8-r3.txt` |

**Warum der Handschlag eine dritte Instanz braucht.** C1 lag ZWISCHEN den
Sprachen: der Broker setzte die Basis auf 0 und erwartete als nächste
Ereignissequenz 1, das Plugin sendete 0. Jede Seite war für sich grün; nur ihr
Verhältnis war falsch, und deshalb konnte kein Bein einer Seite fallen.
`eq-copilot/fixtures/v3/handschlag-v1.json` ist von Hand geschrieben und damit
die Ausgabe keiner der beiden Implementierungen — dieselbe Bauform wie
`MANIFEST.json` für den Fixturekorpus. Stimmen beide mit ihr überein, stimmen
sie transitiv miteinander überein.

### 13.4 W3 — Atomarität Befehl↔Wirkung und Rollback im Broker (B3, B4, B14, B16)

Commit `71194a7`.

| Befund | Regel und was gebaut wurde | Gemessen | Rotbeweis |
|---|---|---|---|
| B3 (Lücke, entschieden) | Der Rückweg eines Experimentbefehls ist `Ruecknahme` und fasst nur an, was DIESER Befehl geändert hat: den angelegten oder veränderten Eintrag, die Terminale der von ihm Verdrängten, seine neue Passage und die Länge seines Logs. Kein Klon, kein Ersatz der ganzen Map. | A4-verdrahtung `ruecknahme_trifft_nur_den_eigenen_versuch` — injizierter Append-Fehler, der bereits persistierte Versuch bleibt | `roh/SONDE-013-rot-B3-r3.txt` |
| B4 | `taint_intervalle_des_experiments_schliessen` schliesst in JEDER Sitzung, die ein Intervall dieses Versuchs hält — und erst NACH dem erfolgreichen Append. | A4-verdrahtung `verdraengter_taint_faellt_in_der_opfersitzung` — zwei Sitzungen desselben Projekts, Verdrängung am Projektdeckel | `roh/SONDE-013-rot-B4-r3.txt` |
| B14 | `preview_begin`, `preview_renew` und `preview_end` sowie der Materialwechsel nach `experiment_begin` tragen ihre Invalidierung als Domänenereignis im SELBEN `persistenz_p0_mit_domaene`. | A4-verdrahtung `preview_und_ruecknahme_liegen_in_einem_append` — der Abstand der `event_ord` ist exakt 1; der Retry liefert `idempotent_wiederholt` und lässt GENAU EINE Rücknahme stehen | `roh/SONDE-013-rot-B14-r3.txt` |
| B16 | `invalidierung_anwenden` liefert `Result`: scheitert der Append, wird der vorläufige lokale Ausschluss GENAU zurückgenommen und der Auslöser lehnt ab. `Ok(None)` bleibt dem Broker ohne Store, dessen flüchtiger Stand die ganze Wahrheit ist. | A4-verdrahtung `gescheiterter_append_laesst_keinen_ausschluss_stehen` — Evidenz unverändert, Log leer, nach Neustart identisch | `roh/SONDE-013-rot-B16-r3.txt` |

**Die Naht.** `StoreHandle::append_naht_setzen` lässt jeden Append verlässlich
scheitern — dieselbe Begründung wie `IdleCheckpointNaht` daneben: ein echter
SQLite-Fehler ist nicht deterministisch herstellbar. Sie liegt VOR dem
Writerkanal; Guards, Checkpoints und Kompaktierung bleiben heil. Ein Befehl,
dessen Append scheitert, bleibt **unbeantwortet**, und der Link gilt als
storeverweigert — das ist die härteste Form von „nicht angewandt", und ein
Bein, das eine Antwort erzwingt, könnte sie nicht messen.

### 13.5 W4 — Replay und Reihenfolge im Broker (B5, B6, B9)

Commit `a3654f5`.

| Befund | Regel und was gebaut wurde | Gemessen | Rotbeweis |
|---|---|---|---|
| B5 | `Coordinator::mit_store` liest `experiment_events` und baut die Transitionshistorie je Versuch aus den daran hängenden Domänenereignissen nach. | A4-verdrahtung `export_traegt_seine_transitionen_auch_nach_dem_neustart` — der Export ist Zeile für Zeile derselbe wie vor dem Neustart | `roh/SONDE-013-rot-B5-r3.txt` |
| B6 | `empfangsfolge` steht neben dem Snapshot in der `evidence`-Projektion, kommt beim Restore zurück, und `evidenz_folge` setzt bei max(restaurierte Folgen, persistierte Begin- und Kandidatengrenzen) + 1 fort. | A4-verdrahtung `evidenzreihenfolge_ueberdauert_den_neustart` — Neustart mit OFFENEM Versuch und erfasstem Kandidaten, der neue Beleg zählt als Resultat | `roh/SONDE-013-rot-B6-r3.txt` |
| B9 | **Die Konvention, hier benannt:** beide Grenzen sind der Wert, den der NÄCHSTE angenommene Snapshot tragen wird. Baseline ist `empfangsfolge < begin_folge`, Resultat ist `empfangsfolge >= kandidat_folge`. | A4-verdrahtung `genau_ein_snapshot_nach_dem_kandidaten_ist_ein_resultat` | `roh/SONDE-013-rot-B9-r3.txt` |

### 13.6 W5 — Zustellung im Broker (B17)

Commit `562c4cc`.

| Befund | Regel und was gebaut wurde | Gemessen | Rotbeweis |
|---|---|---|---|
| B17 | `resubscribe_snapshot_push` spielt nach dem absoluten Sessionsnapshot ALLE offenen Outbox-Zeilen dieses Ziels nach — jeden `object_key`, in Ereignisreihenfolge — und kompaktiert jede Schuld erst NACH ihrer Zustellung. `outbox_lesen` hat damit seinen Produktaufrufer; `StoreHandle::event_payload_lesen` liefert den Payload zur `event_ord`. | A4-verdrahtung `liegengebliebene_ruecknahme_wird_beim_subscribe_nachgespielt` — abgelehnter Push, Reconnect, Zustellung, Schuld getilgt | `roh/SONDE-013-rot-B17-r3.txt` |

Ein `object_key`, dessen Wireform dieser Broker nicht kennt, bleibt als Schuld
STEHEN; sie zu kompaktieren, ohne sie zugestellt zu haben, wäre die stille
Löschung einer Zusage. Die Testprobe kann den Push seit dieser Runde ABLEHNEN —
eine Probe, die immer annimmt, kann eine Zustellschuld gar nicht erzeugen.

### 13.7 W6 — Rechenwege und Paarbildung im Broker (B1, B10, B11, B13, B18, B19)

Commit `14e0d5c`.

| Befund | Regel und was gebaut wurde | Gemessen | Rotbeweis |
|---|---|---|---|
| B1 | Der Nachlauf kennt seinen START (`Taintstand::tail_seit`, `tail_samples_gesamt`); der Tick rechnet ab ihm statt ab dem vorigen Tick. Das globale `letzter_tail_tick` fällt. | A4-verdrahtung `nachlauf_rechnet_ab_seinem_start` — Tick, dann Tail mitten im Intervall | `roh/SONDE-013-rot-B1-r3.txt` |
| B10 | Die Passagenbelege beider Hälften entstehen aus den TATSÄCHLICH vertretenen Instanzen: Fingerprint der Baseline aus der Begin-Referenz, der Resultathälfte aus `kandidat.referenz`; aktive Quellen und Messpunktklassen je Hälfte aus den Snapshots DIESER Hälfte. | A4-verdrahtung `passagenbelege_kommen_aus_den_vertretenen_quellen` — Baseline nur von Quelle A, Resultat nur von Quelle B → `QuellenVerschieden` | `roh/SONDE-013-rot-B10-r3.txt` |
| B11 | Loudness und True Peak werden NICHT ersetzt. Beide Achsen sind `nicht gemessen` mit benanntem Grund (`guardrail_nicht_gemessen`), und die Guardrail-Achse fällt dann auf `NichtBeurteilbar` statt auf `Unveraendert`. | A4-verdrahtung `resultatmessung_ist_an_versuch_passage_und_grenze_gebunden` — beide sind `None` und nennen ihren Grund | `roh/SONDE-013-rot-B11-r3.txt` |
| B13 | `resultatmessung` befüllt `erste_haelfte`/`zweite_haelfte` aus der ZEITLICHEN Halbierung der Resultatfenster; `achsen()` liefert damit wieder eine Effektstabilität. | A4-verdrahtung `passagenbelege_kommen_aus_den_vertretenen_quellen` prüft `effekt_stabil` auf `Some` | `roh/SONDE-013-rot-B13-r3.txt` |
| B18 | Paarurteile werden bei JEDER Evidenzänderung neu gebildet: Append, Invalidierung, Kapazitäts-Eviction und Tombstone-Eviction. Der Merker `Stand::paare_neu_bilden` steht dort, wo die Evidenz fällt. | A4-verdrahtung `invalidierung_bildet_das_paarurteil_neu` — Rücknahme ohne neuen Snapshot, Urteil neu mit Grund | `roh/SONDE-013-rot-B18-r3.txt` |
| B19 | Und beim Neustart: der Restore rechnet sie aus der persistierten Evidenz, und der zurückkehrende Deskriptor bildet sie sofort — ohne neue Evidenz. Ein eigener `StoreEvent` ist dafür NICHT nötig (Dirigentenentscheid, §7.1 E-10). | A4-verdrahtung `paarurteil_entsteht_beim_neustart_aus_der_evidenz` | `roh/SONDE-013-rot-B19-r3.txt` |

### 13.8 Was NICHT Gegenstand dieser Runde war

Der Dirigent hat sechs der 28 Befunde ausdrücklich nicht zur Nacharbeit
gegeben. Sie stehen hier, damit ihr Zustand benannt ist und nicht als
übersehen gilt:

| Befund | Einordnung | Wohin |
|---|---|---|
| B7 „Speichere die Reihenfolgebindung als eigene Transition" | **Durch Regelanpassung geschlossen** — die Blindreihenfolge wird mit dem Kandidaten gebunden (R21), und die Kandidatenzeile in `experiment_events` trägt sie. Regel V2(c) gilt mit **fünf** Zeilenarten. | Keine Änderung; §7.1 E-09 |
| B8 „Gib Experimentterminalen einen eigenen Outbox-Schlüssel" | **Härtung** — der Vertrag (§53.9) führt das Ergebnis über den Sessionsnapshot; der Verlust setzt Offline-Lag über mehr als `N_GLOBAL` Experimente voraus. | Register |
| B2 „Rolle Nebenwirkungen abgelehnter Evidenz zurück" | **Lücke** — die Transportbruch-Invalidierung ist sachlich richtig, eine gemeinsame Transaktion mit dem Snapshot-Append ist nicht zugesagt. | Register |
| B12 „Lies Schutzbereiche statt das grösste Delta umzudeuten" | **Lücke** — woher adressierter Befund und Schutzbereiche kommen, definiert erst SONDE-014. | Register |
| C9 „Ändere lokalen Experimentzustand erst nach dem ACK" | **Lücke** — keine Zusage; gehört zur Gen-Zustandsmaschine des UI-Tickets. | Register |
| B15 „Beende Markerinvalidierung bei `project_sample_end`" | **Widerlegt** — M-52 und §34.2 nennen den Tail ausdrücklich als Teil des Bereichs. | — |

### 13.9 Was diese Runde ausserdem gefunden hat

Nebenbefunde, die kein Codex-Befund waren. Sie gehören datiert nach
`docs/offene-punkte.md`; der Dirigent zieht sie im Abschluss nach.

1. **`evidence_snapshot` trägt weder integrierte Lautheit noch True Peak.**
   M-45 verlangt beide als Guardrails; das Schema führt `baender`,
   `verteilung`, `ereignisse`, `stereo`, `konfidenz` und `transport` — keine
   der beiden Grössen. Diese Runde liefert deshalb `nicht gemessen` mit
   benanntem Grund statt einer Ersatzrechnung unter fremdem Namen. **Der
   fehlende Vertragsteil ist ein eigener Schritt** und gehört in ein Ticket,
   das die Fassung dafür öffnet — hier wäre er ein stiller Vertragsschritt
   gewesen.
2. **`resync_bestaetigen` hat zwei Leser mit derselben Zahl.** Der Handschlag
   aus C1 liegt jetzt als `eq-copilot/fixtures/v3/handschlag-v1.json` neben dem
   Fixturekorpus. Er ist **keine** Schemadatei: A5 liest ausschliesslich
   `fixtures/v3/gueltig/*.json` und `fixtures/v3/ungueltig/*.json`, und der
   Handschlag steht bewusst daneben. Wer künftig eine zweite solche Konstante
   braucht, legt sie dort ab und lässt beide Sprachen sie lesen.
3. **Ein Befehl mit gescheitertem Append bleibt unbeantwortet.** Das ist die
   bestehende Regel von `persistenz_p0` (`store_verweigert_fuer_link` und
   `None`), nicht etwas Neues — aber sie war in keinem Bein sichtbar. Sie ist
   es jetzt, und der betroffene Link ist danach tot: der Test in §13.4 fährt
   deshalb einen **Reconnect**, keinen zweiten Befehl auf demselben Link.
4. **`descriptor_setzen` bildet Paare.** Der Deskriptor ist das, was aus
   Evidenz eine Paarhälfte macht; nach einem Neustart kommt er zurück, die
   Evidenz ist schon da. Ohne diesen Aufruf fehlte das Urteil bis zur nächsten
   Messung — dieselbe Lücke wie B19, nur eine Stelle weiter.

### 13.10 Prüfliste §6 — wo diese Runde sie gemessen hat

| Zeile | Gemessen in dieser Runde |
|---|---|
| A · dieselbe Regel in ALLEN Sprachen, Test beidseitig | C1: `eq-copilot/fixtures/v3/handschlag-v1.json` wird von B23 **und** von A4-verdrahtung gelesen; beide Seiten fallen einzeln (`roh/SONDE-013-rot-C1-r3.txt`) |
| B · verbinden↔trennen, aktivieren↔abklingen im selben Satz | C8 (`urteileVersuch` leert wie `brichVersuchAb`), B4 (Verdrängung schliesst den Taint des Opfers), B18 (Eviction bildet neu) |
| C · NaN/Inf und Grenzwerte an jeder Zahl über den Draht | C5: `nicht_endliche_samples` mit Positiv- und zwei Negativfixtures (negativ, Bruchzahl); B11: keine Ersatzgrösse unter echtem Namen |
| D · fail-closed ohne Rohtextheuristik | C2 (Epochenvergleich verwirft das Fenster), B16 (Append-Fehler nimmt den Ausschluss zurück), B11 (`NichtBeurteilbar` statt `Unveraendert`) |
| E · Behauptung ≤ Messung; jede neue Prüfung einmal gebrochen | 22 Rotbeweise `roh/SONDE-013-rot-C1..C8-r3.txt` und `-B1,B3..B6,B9..B11,B13,B14,B16..B19-r3.txt`; jeder fällt an einem TESTFALL |
...
| Feld | Wert |
|---|---|
| Zeitpunkt | 2026-09-04 23:20:19 +02:00 |
| Rechner | SCHUBBINATOR200 \| Windows 10.0.26200.0 |
| Zweig | master |
| Commit | 043e98e SONDE-013 Etappe 2 Nacharbeit 3: Manifestkopf, Matrixvermerke, Dirigentenentscheide E-09 bis E-12 und Paragraph 13 |
| Commit (voll) | 043e98e6e764c701cc8d65b37b60fc13580b3dc9 |
| Arbeitsbaum | sauber |
| JUCE gepinnt | 8.0.9 |
