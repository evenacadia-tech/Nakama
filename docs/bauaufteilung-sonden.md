...
- **Stand:** 2026-09-02 (Kopf und §1.4/§3/§4/§6.4 mit PR2 nachgezogen; §1.4, §3 und §4
  am selben Tag in **PR2 Runde 1** nach der Codex-Erstprüfung nachgearbeitet — Befunde D5, D7,
  D12 und D13)
- **Gehört zu:** `docs/FL-Nakama-Sonden-Design-Entwurf.md` (**Fassung 0.5**,
  02.09.2026 — alle 17 Errata sind dort an ihrer Stelle eingearbeitet; die
  Fassung 0.4 liegt byteidentisch in
  `docs/archiv/FL-Nakama-Sonden-Design-Entwurf-0.4-2026-08-20.md`) und
  `docs/pruefbericht-sondenentwurf-2026-08-20.md`
- **Rangfolge seit PR2 (02.09.2026):** der Entwurf bindet Technik, Verträge,
  Phasen und Falsifikation; `design/docs/funktions-und-bedien-blueprint.md`
  bindet die Funktions- und Bedienarchitektur beider Apps. Über beiden stehen
  aktueller Vertrag und Code; ein Widerspruch wird als Zeile in
...
- **Nachgezogen 23.08. (Umschnitt, NAK-64):** Entscheid **„Gen wird
  ...
  **S28b und S31b sind neu**, Summe 37 → **39** Sessions;
  ...
  `../design/docs/ui-spielregeln-eq-zentrale.md` (44 Regeln).~~ **Seit dem
  Funktionsneustart der UI (User-Auftrag 31.08.2026) Verlauf, kein Prüfmaßstab
  mehr — umgestellt mit PR2 am 02.09.2026, siehe §1.4 und §3 P6–P7.**
...
`design/abnahmen/`; frühere Figma-Stände sind Verlauf. Beide Gen-Seiten
verwenden genau eine logische Arbeitsgröße von 950×538 (Entscheid
01.09.2026). Compact, Fenster-Resize und größenabhängiger Reflow sind bis nach
Fertigstellung dieser Standard-UI vertagt; UI-Skalierung rendert dieselbe
Geometrie größer. Der
...
`Nakama-Design` Node `6:2864` (260×84).~~ **Historisch seit dem 02.09.2026 (PR2 Runde 1,
Codex-Befund D12).** Der Satz schickte an drei Stellen zur Altquelle: NAK-65 ist am
02.09.2026 mit `894dce7` geschlossen (User-Entscheid 31.08.2026), Suna ist am
23.08.2026 als App entfallen (`../design/abnahmen/2026-08-23-gen-eq-zentrale.md`),
und Figma-Stände sind seit dem Funktionsneustart der UI Verlauf. Der Ort der
Probeeq-Rückfallfläche ist seitdem **S31b**, ihre Quelle der Blueprint plus eine
noch abzunehmende visuelle Spezifikation — siehe den Absatz direkt darunter.
**Quellen umgestellt 02.09.2026 (PR2, Blueprint §15 Punkt 11):** Prüfmaßstab
jeder UI-Fassung sind ab jetzt `../design/docs/funktions-und-bedien-blueprint.md`,
die datierten Abnahmen unter `../design/abnahmen/` (zuletzt 31.08. und
01.09.2026, darunter `2026-09-01-gen-nur-standardgroesse.md`) und eine **noch
abzunehmende visuelle Spezifikation**, die es heute nicht gibt.
`../design/docs/ui-spielregeln-eq-zentrale.md`, die Figma-Stände und die drei
Rework-Bilder vom 25.08.2026 sind seit dem Funktionsneustart der UI
(bindender User-Wortlaut 31.08.2026, zeichengetreu aus
`../design/docs/funktions-und-bedien-blueprint.md` §0:
„ignoriere bisherige design die entworfen wurden.“ — Zeichen nachgezogen am
02.09.2026, PR2 Runde 1, Codex-Befund D5)
**Verlauf** und kein Prüfmaßstab mehr; sie bleiben liegen und werden nicht
gelöscht. Der frühere Vorschlag, `SONDE-007b` nehme die Kachelgestaltung aus
dem Figma-Stand, ist damit gegenstandslos — der Vertragsteil (Zustände,
Pflichtinhalt) bleibt, die Gestaltung wartet auf die visuelle Spezifikation.
...
als Master-EQ in Gen, bedient wird zentral auf Gens zweiter Fläche. Der Fernweg
(`SONDE-016/017`) ist damit der **Hauptbedienweg**, nicht mehr der
Nebenpfad; die EQ-Bedien-UI entsteht **einmal** (S31b) statt je App.

**Nachgezogen 02.09.2026 (PR2, Design-Strang).** Die Abnahmen vom 31.08. und
01.09.2026 verlangen sechs Stücke Vertragsarbeit, die bisher keinen Planschritt
hatten. Sie sind jetzt zugeordnet — Wortlaut und Belege stehen in
`docs/plan/plan.json`:

| Vertragsarbeit | Schritt | Quelle |
|---|---|---|
...
| S29–31 | `SONDE-016` + `017` | Pairing/HMAC (**mit NAK-21-Reihenfolge!**), Lease, Apply/Revert, Active-Compare — als Hauptbedienweg: **Drei-Stufen-Geste** (Preview per Halten · Apply mit 10-s-Lease-Startwert · Confirm), volle manuelle Bereiche **ohne ±3-Cap** (U14), Schutz-Zonen als Ablehnungsgrund + harte Sperre der Bedienwege, **zweistufiger Mix** (ganze App ↔ gewählte Spur) — **dazu seit PR1 Runde 1 (30.08.2026, Befunde B3/B4):** `command_ack` bestätigt die **tatsächlich angewandten Werte**, nicht nur `state_hash` (Entwurf §33.4 verlangt es wörtlich; alle fünf Zweige tragen heute nur `command_id`, `ergebnis`, `state_revision`, `state_hash`, `code`), und der Betriebszustand des Sonden-EQ wird hier geführt, weil Gen ihn für die Fernsteuerung braucht — Technik ohne Anzeigezusage (`command_ack.applied_dsp` und `state_report.eq_enabled` sind am 02.09.2026 als reservierte Namen ohne Nutzlast gemessen). **Dazu seit PR2 (02.09.2026)** — Quellen `design/abnahmen/2026-08-23-interview-struktur.md`, `design/abnahmen/2026-09-01-technische-ui-architektur-fuenferblock-01.md` und Register NAK-120 (Quellen nachgetragen am 02.09.2026, PR2 Runde 1, Codex-Befund D13)**:** der **10-Sekunden-Kandidatenvertrag** (ID, Timer, Nachricht, ACK, Reconnect, Kill, Ablauf), die **direkte Edit-Transaktion** (Band-Drag, Textwert, Preset-Load, Curve-Copy auf Preview/Kandidat/Confirmed/Cancel/Revert, samt Remove-Undo aus S26–28), die **Zustands- und Umschaltsemantik des zweistufigen Mix** und die **Main-Führung nach Brokerneustart** (bestätigbarer Main-State-Ingress, NAK-120). Gen Fläche 1 übergibt ausschließlich einen unbestätigten Draft (`SEND DRAFT`) und startet keinen Kandidaten | T1+T2 |
| **G7** | — | **Gate:** `/security-review` + `/rust-review` + Codex + 10 000 Befehlsstress. Falsifikation: Gate 3, 4, 8 | **T3** |
| S31b | `SONDE-020` | **EQ-Zentrale-UI:** Gen Fläche 2; dazu die kompakte Status- und Rückfallfläche von Probeeq. **Quelle umgestellt 02.09.2026 (PR2, Blueprint §15 Punkt 11):** bindend sind `design/docs/funktions-und-bedien-blueprint.md` plus eine **noch abzunehmende visuelle Spezifikation**; ohne deren datierte Abnahme beginnt die sichtbare Umsetzung nicht. Weggefallen sind alle drei früheren Quellen — Figma-Stand (NAK-65), `design/assets/rework-basis-2026-08-25/gen-page-2-eq-center.png` und `ui-spielregeln-eq-zentrale.md`; sie bleiben als Verlauf liegen (bindender User-Wortlaut 31.08.2026, zeichengetreu aus `design/docs/funktions-und-bedien-blueprint.md` §0: „ignoriere bisherige design die entworfen wurden.“ — Zeichen nachgezogen am 02.09.2026, PR2 Runde 1, Codex-Befund D5). Lücken der neuen Spezifikation werden einzeln vorgelegt, nicht erraten. **Abgenommene Bedienarchitektur (31.08./01.09.2026, Quellen `design/abnahmen/2026-08-31-technische-ui-architektur-arbeitsmodus.md`, `…/2026-09-01-technische-ui-architektur-fuenferblock-01.md`, `…/2026-09-01-technische-ui-architektur-fuenferblock-02.md`, `…/2026-09-01-gen-nur-standardgroesse.md` — nachgetragen am 02.09.2026, PR2 Runde 1, Codex-Befund D13):** graphdominante Arbeitsfläche, Transaktionszeile kompakt unter dem Graphen und ohne offenen Draft unsichtbar, objektgebundenes Mini-Panel per Doppelklick (Typ als 2×3-Kurvenauswahl, Dynamic zweireihig, Kanalmodus und `ON/OFF` im selben Körper), freier Doppelklick belegt den nächsten der acht Slots, Leerklick schließt nicht, zweite ruhige Kontur als Aktivbeleg mit mitgehendem Bandpunkt, atomares Panel-Rebinding, `Remove Band` getrennt und wiederherstellbar, eine logische Größe 950×538. **Antwort U16 (30.08.2026, „Ja, erste Fassung"):** die Überdeckung zweier Quellen erscheint als farbige Zone im Kurvenbild, Teil der ersten nützlichen Fassung; sie zeigt nur, was S23–25 belegt, und setzt keine eigene Schwelle. **Antwort U17 (30.08.2026, Button für Button):** Band direkt aus dem Spektrum ziehen (U17.1 „Ja“ — erste Reaktion des Users war „VERSTEHE Die frage nicht“, das „Ja“ kam nach erneuter Erklärung), Spitzenwerte festhalten (U17.2 „Ja“), Anzeige einstellbar in Höhe, Feinheit, Geschwindigkeit und Neigung als **reine Darstellung** bei unverändertem Messraster (U17.3 „Ja“) — und **keine** Tonnamen, **keine** Klaviatur (U17.4 „Nein“), womit auch die FL-Oktavschreibweise keine Anzeigefrage mehr ist. **Antwort U18 (30.08.2026, „Beides“):** Preset-Liste (mitgelieferte und eigene, eigene sicher- und überschreibbar) und der Handgriff, eine fertige Kurve von einer Sonde auf eine andere zu übertragen — mit sichtbarer Quelle und sichtbarem Ziel, weil er den Klangzustand der Zielsonde überschreibt. Für das Kopieren gibt es keinen belegten Marktstandard (**Lücke L-3**) und keine Vorlage; fehlt die Fläche in der noch abzunehmenden visuellen Spezifikation, wird sie einzeln vorgelegt. **GEPARKT 12.09.2026** mit dem gesamten Designteil (`design/abnahmen/2026-09-12-designteil-geparkt.md`); der Schritt steht in `docs/plan/plan.json` am Planende hinter P8–P9 und beginnt erst nach neuem User-Wort | T1+T2 |
...
**Bis R2 (das erste wirklich nützliche Produkt): 29 Bau-Sessions + 6 Gates.**
**Bis R4 (voller Sondenkern): 41 + 10 = 51 Sessions** (23.08.: +2 durch den
...
Der Zwischenhalt **PR1** ist keine Bau-Session und zählt in dieser Tabelle nicht mit; dasselbe gilt
für **PR2**. **02.09.2026 (PR2 Runde 1, Codex-Befund D7): +1** durch den neuen Schritt **S19b**
(verhaltensneutrale Broker-Modulaufteilung, P3 nach G3) — P3 zählt seitdem 3 statt 2, Gesamt 41
statt 40, bis R2 29 statt 28.
...
4. **Fremde Dateien nicht anfassen.** Gemeint sind uncommittete Dateien
   ...
   **Der Entwurf gehört dem Repo.** Er trug bis zum 02.09.2026 einen
   Errata-Block (21.08.) mit Vorrang, unter dem der Text bewusst nicht
   umgeschrieben wurde, damit die Prüfer-Regel „Gate-Text aus dem Entwurf,
   nicht meine Zusammenfassung" (T2) einen stabilen Bezug behielt.
   **Abgelöst mit PR2 (02.09.2026):** Fassung 0.5 arbeitet jedes Erratum an
   seiner Stelle ein, der Block bleibt als Verlaufsabschnitt und Quelle
   stehen. Der stabile Bezug wird jetzt anders gehalten — **die
   §-Nummerierung ändert sich nie**: ein gestrichener Abschnitt behält
   Überschrift und Nummer und trägt eine Grabsteinzeile mit Datum, Grund und
   Entscheidquelle. Neue Entscheide kommen weiterhin nur mit Datum und
   Wortlaut des Users hinein, Befunde als NAK-Zeile.
