...
$kanon = @(
    ...
    [pscustomobject]@{
        ...
        AbPhase    = 'jetzt'
        Behauptung = 'AnalyseEngine deckt sich mit der eingefrorenen Offline-Referenz (Fixture-SHA-256 als Determinismus-Riegel). Seit NAK-380 Etappe 3: M1-Perzentile aus Teilbloecken zu acht Segmenten; Rauschen steht in jedem Band ruhig, eine bekannte Pegelbewegung wird innerhalb -0,5/+2,0 dB gelesen. Seit NAK-380 Etappe 6 (380/M-99, M-101, M-103 bis M-108, M-111, M-112): die vier M1-Stufen folgen der Fensterdauer (16 384/8 192/4 096/2 048 bei 44,1 und 48 kHz, das Doppelte bei 88,2/96 und das Vierfache bei 176,4/192 kHz, Kappe 65 536 bei 384 kHz gemessen) mit 24 B je Punkt; die Resonanzsuche findet 300 Hz bei 96 kHz sowie 500 und 60 Hz bei 192 kHz, meldet unter der Suchgrenze 17,30*fs/N_Bass (50,68 bzw. 46,57 Hz) keinen Kandidaten und nennt die Grenze im Satz jeder Karte; die interpolierten Werte bleiben im Snapshot, und Zonen-Ticks bleiben 30 in 30 s bei 48 und 96 kHz.'
    }
    [pscustomobject]@{
        Kuerzel    = 'A3'
    ...
    [pscustomobject]@{ Kuerzel='A13'; Name='pruefe_host_capabilities.py'; Art='python'; Argumente=@(); AbPhase='jetzt'; Behauptung='Capabilityreport FL: die zehn Bits aus §53.6 entsprechen der v3-Vertragsform und stehen so, wie die Rohdaten der Termine A und B und die benannten Kanonbeweise sie tragen; jedes supported hat einen Termin oder, nur ohne Hostanteil, einen Kanonbeweis, jedes unsupported seinen festen Fallback.' }
    ...
    # NAK-283 F13 (13.09.2026): der Selbsttest des Soak-Orakels, Muster A27.
